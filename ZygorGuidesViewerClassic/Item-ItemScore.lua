	...
	if not cached or SKIP_CACHE or force then
		...
		for i,line in ipairs(tooltip) do
			...
			if ZGV.IsRetail then -- early exists are only valid on retail. classic has normal stats as equip: effects
				--if line==" " then break end -- empty lines denote end of stats and beggining of extras. 2026-03-24 - not anymore. level req is now after uses
				if line:match("^"..ITEM_SET_BONUS_GRAY) then break end
				if line:match("^"..ITEM_SET_BONUS) then break end
				if (found_enchant or found_equip or found_proc or found_use or found_level) then break end
			end
...
local function ItemScore_SetTooltipData(tooltip)
	...
	if ZGV.IsRetail then
		for i=1,50 do
			...
			if ZGV.IsSecret(line) then return end -- why, blizz, why
			local linetext = line and line:GetText()
			if ZGV.IsSecret(linetext) then return end -- why, blizz, why
			
			if linetext then
				if linetext:match(AUCTION_HOUSE_BUCKET_VARIATION_EQUIPMENT_TOOLTIP) then return end
			else
				break
			end
		end
