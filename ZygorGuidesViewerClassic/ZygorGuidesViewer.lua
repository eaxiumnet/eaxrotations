...
local function _StartupThread()
	...
	local function run_startup_cycle(phase)
		...
		for i,startup in ipairs(self.startups) do if not startup.done then
			if (not startup.postcombat or phase=="postcombat" or not ZGV:IsPlayerInCombat()) and (not startup.after or all_done(startup.after)) then
	...
	if displayedInCombatWarning then ZGV.Pointer:ShowMapMarkers() end
...
function ZGV:FindFilteredGuides(filters,path)
	filters = filters or {}
	local filterstring = ""
	...
	if path and not FindFilteredGuidesContextCache[path] then
		...
		local group = ZGV:FindOrCreateGroup(ZGV.registered_groups,path,true)
		if group then
	...
	local context = ZGV.registeredguides
