...
function ZGV:GetItemInfo(id)
	...
	if live_result[1] then
		...
		if not live_result[16] then live_result[16]=-1 end -- setID is nilable, which will break unpack
...
function ZGV.Replacements:Startup()
	...
	if ZGV.IsClassicWOTLK or ZGV.IsClassicCATA or ZGV.IsClassicMOP then
		...
		ZGV.Replacements.ExpBar = MainMenuExpBar or MainStatusTrackingBarContainer
	end
