...
function GearFinder:IsValidDungeon(dungeon, instanceId, diff, mythicplus)
	local ident = dungeon
	...
	if dungeon.difficulty then
		if dungeon.difficulty==23 then
			if not (ZGV.db.profile["gear_23"] or (mythicplus and ZGV.db.profile["gear_8"])) then return false, false, ident, 0, false,false, "instance filtered out "..dungeon.difficulty end
		else
	...
	if maxScaleLevel < (ItemScore.playerlevel-GearFinder.PAST_DUNGEONS_LIMIT) and not mythicplus then return false, false, ident, 0, false,false, "outleveled "..maxScaleLevel..":"..(ItemScore.playerlevel-GearFinder.PAST_DUNGEONS_LIMIT) end
...
function GearFinder:ScoreDungeonItems()
	...
	for dungeon,dungeondata in pairs(ZGV.ItemScore.Items) do
		local valid, future, ident, maxscale, mythic, mythicplus, comment = GearFinder:IsValidDungeon(dungeondata.dungeon or dungeondata.dungeonmap, dungeondata.instanceId, dungeondata.diff, dungeondata.mythicplus)
		if valid then
				...
				if type(bossdata)=="table" and (not bossdata.phase or ZGV.Dungeons.Phases[bossdata.phase]) then
					...
					for _,itemlink in pairs(classdata) do
						if type(itemlink)=="number" then 
							itemlink = "item:"..itemlink .. (dungeondata.suffix or "")
						end
