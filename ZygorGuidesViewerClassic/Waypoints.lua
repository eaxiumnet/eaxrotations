			...
			if waypath then
				if waypath.loop then
					...
					if currentmapid ~= waypath.coords[1].map or currentmapfloor ~= waypath.coords[1].floor then
						...
						local nextway = ZGV.Pointer:PointToNextInPath("farm") -- it's looped, so it's a farm, OK
						
						-- but are we close enough?
						if nextway then
							local px,py,pm = ZGV.LibRover:GetPlayerPosition()
							local dist = ZGV.HBD:GetZoneDistance(pm,px,py,nextway.m,nextway.x,nextway.y) or 9999
							if dist > 500 then
								-- too far, use proper routing
								ZGV.Pointer:FindTravelPath(nextway)
							end
						end
					end
					break -------------------------------------------------------------------------------
