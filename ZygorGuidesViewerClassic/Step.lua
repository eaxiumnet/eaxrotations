...
function Step:IsComplete()
	...
	-- prepare statuseses
	local anyvisibilitychanged = false
	local anywascompletable = false
	for i,goal in ipairs(self.goals) do 
		local _,visibilitychanged = goal:UpdateStatus() 
		anyvisibilitychanged = anyvisibilitychanged or visibilitychanged
		if goal.status=="complete" or goal.status=="incomplete" then anywascompletable=true end
	end
	
	if anywascompletable then self.anywascompletable = true end
	
	if anyvisibilitychanged then ZGV:SendMessage("ZGV_GOAL_VISIBILITY_CHANGED") end
	...
	--return (completeable or self.anywascompletable) and complete and (not alloptional or self.anywascompletable), steppossible, "normal comp"
end
...
function Step:AreRequirementsMet(case,strict)
	...
	if self.condition_visible then
		--if not self.conditionOnEnterChecked or self:IsCurrentlySticky() then
			self.conditionOnEnterChecked = true 
...
function Step:OnEnter()
	...
	if not self.skipping then
		-- if we navigate to step with hidden completables by hand, unseet so that we will not automatically skip it
		 self.anywascompletable = false
	end
...
function Step:OnLeave()
	ZGV:Debug("&step_setup Step %d OnLeave",self.num)

	for i,goal in ipairs(self.goals) do 
		goal:UnregisterEvents()
	end
	
	self.conditionOnEnterChecked = false
end
