...
`service_gossip`, `mount_manager`, `corpse_loot`, the retired anti-detection shim,
...
| 7 | **Anti-cheat random jump** — every 10-25 s while navigating | §state_nav ~683-686 | `nav_state.run` | This live movement action is owned by `nav_state`; the former anti-detection delay/jitter/proximity members are retired and are not a second source of navigation behavior. |
