...
| 11 | `quest_interaction_sylvanas.lua:414` trainer | `1..num_services`, buys each affordable service | **UNPROVEN — not changed** | the docs (`quests.md:881-941`) never say whether purchasing removes the service from the list. If it does, this loop skips every service after the first purchase; if it does not, the current code is correct. Not provable without the client — see below |
...
1. **The trainer loop (#11)** — whether `buy_trainer_service(i)` removes the service from
   the list. This is the one candidate the documentation leaves open, so it is reported
   rather than changed; a fix would be a guess in either direction.
