-- EAX Rogue Combat | libraries/menu.lua | Project Sylvanas
local unified = require("EAX_Unified/menu")
if not unified then error("[EAX Rogue Combat] EAX_Unified/menu not found!") end

local ROTATION_KEY = "rogue_combat"
local menu = {}
    ...
    categories = {
        {
            name = "Rotation",
            settings = {
                { key = "use_sinister_strike",   type = "checkbox", label = "Sinister Strike",   default = true,  tooltip = "Primary combo builder" },
                { key = "use_backstab",          type = "checkbox", label = "Backstab",          default = true,  tooltip = "Behind-target builder" },
                { key = "use_riposte",           type = "checkbox", label = "Riposte",           default = true,  tooltip = "After dodge counter-attack" },
                { key = "use_shiv",              type = "checkbox", label = "Shiv",              default = true,  tooltip = "Instant offhand for poison/debuff" },
                { key = "use_rupture",           type = "checkbox", label = "Rupture",           default = true },
                { key = "rupture_combo_points",  type = "slider",   label = "Rupture Min CP",    default = 4, min = 1, max = 5 },
                ...
                { key = "use_blade_flurry",      type = "checkbox", label = "Blade Flurry",      default = true,  tooltip = "AoE cleave — auto-toggle" },
                { key = "blade_flurry_min_enemies",type = "slider", label = "Blade Flurry Min Enemies", default = 2, min = 2, max = 6 },
            },
        },
        {
            name = "Cooldowns",
            settings = {
                { key = "use_adrenaline_rush",   type = "checkbox", label = "Adrenaline Rush",   default = true,  tooltip = "Major energy CD" },
...
local callbacks = {
    on_enabled  = function() print("|cFFF5E642[EAX Combat]|r Rotation ENABLED") end,
    on_disabled = function() print("|cFFF5E642[EAX Combat]|r Rotation disabled") end,
}

unified.register_rotation("Rogue", "Combat", MENU_DEF, callbacks)
...
menu.use_riposte                = proxy("use_riposte",               true)
menu.use_shiv                   = proxy("use_shiv",                  true)
menu.use_rupture                = proxy("use_rupture",               true)
menu.rupture_combo_points       = proxy("rupture_combo_points",      4)
...
menu.use_blade_flurry           = proxy("use_blade_flurry",          true)
menu.blade_flurry_min_enemies   = proxy("blade_flurry_min_enemies",  2)
menu.use_adrenaline_rush        = proxy("use_adrenaline_rush",       true)
...
local _enabled_keybind  = core.menu.keybind(7, false, "eax_rogue_combat_enabled_kb")
local _toggle_key_elem  = core.menu.keybind(107, false, "eax_rogue_combat_toggle_key")
...
function menu.on_menu_render()
    _menu_tree:render("EAX Combat", function()
        _enabled_keybind:render("Enable EAX Combat", "Toggle rotation on/off")
