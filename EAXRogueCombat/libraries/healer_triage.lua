...
local function compare_priority(a, b)
    if a.hp_pct ~= b.hp_pct then
        return a.hp_pct < b.hp_pct
    end
...
local function is_covered(member, opts)
    return member.incoming_heal_pct >= opts.covered_incoming_pct
        or (member.hp_pct + member.incoming_heal_pct) >= opts.covered_total_pct
end
...
function healer_triage.select_target(_, units, opts)
    ...
    for index, member in ipairs(units) do
        ...
        if entry then
            ...
            if entry.hp_pct < opts.group_hp_threshold then
    ...
    for _, member in ipairs(snapshots) do
        if member.hp_pct < opts.triage_hp_threshold and not is_covered(member, opts) then
    ...
    if tank
        and tank.hp_pct <= opts.tank_hp_threshold
...
function healer_triage.should_cancel_overheal(target_snapshot, opts)
    ...
    local hp_pct = clamp01(target_snapshot.hp_pct)
    ...
    return hp_pct >= opts.cancel_hp_threshold
