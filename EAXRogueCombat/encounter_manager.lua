...
local function count_nearby_enemies(me, range_yards)
    ...
    if not ok_me_pos or not me_pos then
        return 0
    end

    local count = 0
    local max_range = range_yards or 10
    local objects = core.object_manager.get_all_objects()
    for i = 1, #objects do
...
function encounter_manager.is_movement_phase(me)
    ...
    if entry and entry.movement_phase then
        ...
        if ok_tgt and tgt and tgt:is_valid() then
            local ok_me_pos, me_pos = pcall(function() return me:get_position() end)
            local ok_tgt_pos, tgt_pos = pcall(function() return tgt:get_position() end)
            if ok_me_pos and me_pos and ok_tgt_pos and tgt_pos and entry.min_range then
                local threshold = entry.min_range + (tgt:get_bounding_radius() or 0)
                local sq_dist = me_pos:squared_dist_to_ignore_z(tgt_pos)
                if sq_dist < (threshold * threshold) then
                    return true
                end
            end
