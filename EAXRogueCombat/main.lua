...
local function try_blade_flurry(me, target)
    ...
    local ok_me, me_player = pcall(function() return core.object_manager.get_local_player() end)
    if not ok_me then me_player = nil end
    for i = 1, #objects do
        local obj = objects[i]
...
local function on_update()
    ...
    -- Track energy ticks
    local ok_energy, current_energy = pcall(function() if me and me.get_power then return me:get_power(3) end return 0 end)
    if not ok_energy then current_energy = 0 end
