--[[
    middleware_manager.lua | EAX Mage Arcane
     middleware for Arcane Mage rotation
...
local function resolve_spell_ids()
    spell_ids.arcane_power = utils.resolve_spell_id(spells.ARCANE_POWER)
    spell_ids.presence_of_mind = utils.resolve_spell_id(spells.PRESENCE_OF_MIND)
...
-- Arcane Power offensive CD
local function arcane_power_middleware(menu)
    return {
        name = "ArcanePower",
        ...
        is_defensive = false,
        is_gcd_gated = true,
        setting_key = "use_arcane_power",
        matches = function(ctx)
            if not ctx.in_combat then return false end
            if not ctx.target then return false end
            if not spell_ids.arcane_power then return false end
            if utils.has_buff(ctx.me, spells.BUFF_ARCANE_POWER) then return false end
            if not utils.can_cast_self(spell_ids.arcane_power, ctx.me) then return false end
            
            -- Only use in burn phase with sufficient mana
            local mana_pct = utils.get_mana_pct(ctx.me)
            local burn_threshold = get_menu_value(menu, "burn_mana_pct", 50)
            if mana_pct < burn_threshold then return false end
            
            return true
        end,
        execute = function(icon, ctx)
            if utils.cast_self_fast(spell_ids.arcane_power, ctx.me, "Arcane Power [MW]") then
                return true, "[MW] Arcane Power"
            end
            return false
        end,
    }
...
-- Presence of Mind middleware
local function presence_of_mind_middleware(menu)
    return {
        name = "PresenceOfMind",
        ...
        is_defensive = false,
        is_gcd_gated = true,
        setting_key = "use_presence_of_mind",
        matches = function(ctx)
            if not ctx.in_combat then return false end
            if not spell_ids.presence_of_mind then return false end
            if utils.has_buff(ctx.me, spells.BUFF_PRESENCE_OF_MIND) then return false end
            if not utils.can_cast_self(spell_ids.presence_of_mind, ctx.me) then return false end
            return true
        end,
        execute = function(icon, ctx)
            if utils.cast_self_fast(spell_ids.presence_of_mind, ctx.me, "Presence of Mind [MW]") then
                return true, "[MW] Presence of Mind"
            end
            return false
        end,
    }
...
local function cold_snap_middleware(menu)
    return {
        name = "ColdSnap",
        priority = PRIORITY.OFFENSIVE_CDS - 30,
        is_burst = true,
        is_defensive = false,
        is_gcd_gated = true,
...
function middleware_manager.init(menu)
    ...
    -- Register all Arcane-specific middleware
    ...
    _compat.register_middleware(arcane_power_middleware(menu))
    _compat.register_middleware(presence_of_mind_middleware(menu))
