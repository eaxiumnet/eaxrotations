...
- [x] **B6 — Friendly-target healing + per-spec predictive thresholds** (post-audit 3.1/3.2). ✅ DONE 2026-06-25 (all 5 healers). Manual-friendly-target healing so a healer with a selected friendly ally heals that target (top routine priority) instead of only auto-scanning lowest HP.
