...
- [x] **B6 — Friendly-target healing + per-spec predictive thresholds** (post-audit 3.1/3.2). ✅ DONE 2026-06-25 (all 5 healers + all 5 vanilla variants). Manual-friendly-target healing so a healer with a selected friendly ally heals that target (top routine priority) instead of only auto-scanning lowest HP.
  - **Shared helpers** (core/units.lua): `NS.has_friendly_target()`, `NS.get_friendly_target_entry(context)` → `{ unit, hp_pct, effective_hp, is_player }`. No native `is_friendly()` exists on game_object (verified apidocs/game-object.md); "friendly" = alive and not hostile via `NS.is_hostile_unit` (can_attack / is_enemy_with). Resolved at CALL time so install order is safe.
  - **5 healer specs** — each got a `FriendlyTarget` strategy placed AFTER life-critical emergency saves but BEFORE routine heals (so emergencies win, manual target wins over auto-lowest-scan for routine healing). Each gates on: combat-only, not moving, no active emergency on the auto-scanned lowest (per-spec threshold), mana/readiness, overheal gate. Opt-out setting + threshold slider per spec.
    - priest/holy_sylvanas.lua: GreaterHeal (cast_best_heal_rank), after EmergencyPWS/Preemptive/EmergencyFlashHeal; gate holy_emergency_hp(30); opts holy_use_friendly_target / holy_friendly_target_threshold(90).
    - priest/discipline_sylvanas.lua: GreaterHeal rank pick (GREATER_HEAL_MAX/CONSERVE/EFFICIENT), after emergency/preemptive tier; gate discipline_flash_hp(55); opts disc_use_friendly_target / disc_friendly_target_threshold(90).
    - paladin/holy_sylvanas.lua: HolyLight rank (choose_holy_light_rank), after emergency direct-heal tier (HolyShock/HolyLightEmergency/DivineFavorHolyLightFollowup); gate hp<=55; opts holy_use_friendly_target / holy_friendly_target_threshold(90).
    - druid/resto_sylvanas.lua: Regrowth (predictive_overheal), after emergency tier (Swiftmend/NS/Tranquility/HealingTouchMaxEmergency); gate resto_emergency_hp(35); opts resto_use_friendly_target / resto_friendly_target_threshold(90).
    - shaman/restoration_sylvanas.lua: HealingWave (gate_overheal + healing_wave_ready), after cooldown tier (EarthShield/NS/ManaTide/Bloodlust); gate restoration_emergency_hp(50); opts restoration_use_friendly_target / restoration_friendly_target_threshold(90).
  - **Tests**: 5 new dedicated suites (test_*_friendly_target.lua, 5-10 contracts each: match+cast, emergency override, opt-out, hostile, moving-gated, ordering) + bumped 2 strategy-count assertions (test_holy_priest_feature_gaps 31→32, test_discipline_feature_gaps 31→32). Gate green: 166 rotation + 11 leveling on Lua 5.1.
  - **Deferred (B6.2)**: per-spec *predictive* threshold sliders (post-audit 3.2) — the friendly_target_threshold sliders are HP-based; predictive (damage-rate) threshold sliders are a separate tuning layer, deferred.
  - **Vanilla healer variants** (holy_vanilla, discipline_vanilla, resto_vanilla, restoration_vanilla) NOT modified — B6 scoped to TBC/sylvanas variants per plan. Vanilla follow-up if desired.
