...
- [ ] **A4 — Decide pvp_burst DR/EnemyCD stubs.** `pvp_burst_window_sylvanas.lua` stubs return false/unknown (DR immunity always misses). Either (a) wire `arena_priority_sylvanas.lua`'s existing DR logic into the stubs, or (b) document as accepted PvP limitation. **Low priority — PvP only.**
...
- [ ] **C1 — Extract `core/ttd.lua`** from core_sylvanas. Re-export on `NS.*`. Verify `test_ttd_tracker.lua`, `test_ttd_normalization.lua`.
- [ ] **C2 — Extract `core/talents.lua`**. Verify `test_talent_context.lua`.
- [ ] **C3 — Extract `core/diagnostics.lua`** (log/log_warning/log_error/api_health/dump_class_spells).
- [ ] **C4 — (Optional, highest risk) Extract `core/casting.lua`** (`try_cast`, `action_execute`, `spell_ready`). Defer unless core_sylvanas still >1500 lines after C1-C3.
- [ ] **C5 — `core/init.lua` facade + size audit.** Target `core_sylvanas.lua` < 1500 lines; no caller changes.

### Track D — Data Cleanup (mechanical, low risk)

- [ ] **D1 — Strip 35 DEBUG/placeholder entries** from `wowhead_data_bridge_*.lua` (pattern `DEBUG`/`XXXX`/`ALEX BUG`/`QR XXXX`). Grep rotation files for removed IDs first.
- [ ] **D2 — Fix spell ID 28176 conflict** (Fel Armor / Spellstone) in `offensive_dispel_sylvanas.lua`. Verify against DBC.
