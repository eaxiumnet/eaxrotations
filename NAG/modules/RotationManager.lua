...
--- True when NAG AceConfig dialog is open; rotation intentionally skips updates (see Common.lua Update early return).
local function IsNAGOptionsDialogOpen()
    local d = ns.AceConfigDialog
    return d and d.OpenFrames and d.OpenFrames["NAG"] ~= nil
end
...
function RotationManager:ShouldRunAggressiveRecovery()
    ...
    if IsNAGOptionsDialogOpen() then
        return false
    end
    return true
end
...
function RotationManager:PrintRotationFreezeStatus(verbose)
    ...
    if not rec and secsSinceRotation and secsSinceRotation > 2 then
        if secsSinceCallback and secsSinceCallback > 2 then
            ...
            local phase = cycleDiagnostics and cycleDiagnostics.cyclePhase or nil
            local reason = lastEarlyReturnReason and tostring(lastEarlyReturnReason) or nil
            if phase == "execute_returned_false" and reason then
                rec = format("|cFFFFAA00[CALLBACK] Rotation not completing (execute_returned_false). Blocking gate: %s|r", reason)
            else
...
function RotationManager:EnsureRotationWatchdog()
    ...
    TimerManager:Create(
        ...
        function()
            ...
            -- Options open: rotation is paused on purpose; skip finish/cache stall warnings and recovery kicks.
            if IsNAGOptionsDialogOpen() then
                return
            end
