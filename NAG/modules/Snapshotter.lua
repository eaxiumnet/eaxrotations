...
local FOF_STACK_AURA_ID = 138760--138758 --138768
local DK_UNHOLY_STRENGTH = 53365
    ...
    if rawSpellID then
        ...
        if rawSpellID == 55095 or rawSpellID == 55078 then
            ...
            if rawSpellID == DK_BLOOD_PLAGUE then
                ...
                snapshot.perSpell.snapUnholyStrength = (NAG and NAG.AuraIsActive and NAG:AuraIsActive(DK_UNHOLY_STRENGTH, "player")) and true or false
                -- local g = (type(destGUID) == "string" and #destGUID > 8) and destGUID:sub(-8) or tostring(destGUID)
                -- print(string.format("[SnapFoF] BP snapshot: destGUID=%s fofStacks=%d", g, fofStacks))
            end
        end
...
function Snapshotter:GetSnapStacksFoF(unit)
    ...
    if not guid or guid == "" then
        -- print(string.format("[SnapFoF] GetSnapStacksFoF: unit=%s guid=nil/empty => 0", tostring(targetUnit)))
        return 0
    end
    ...
    if snapshot and snapshot.perSpell and type(snapshot.perSpell.snapStacksFoF) == "number" then
        local v = math.max(0, math.min(10, snapshot.perSpell.snapStacksFoF))
        -- print(string.format("[SnapFoF] GetSnapStacksFoF: unit=%s guid=%s (match) => %d", tostring(targetUnit), tostring(guid):sub(-8), v))
        return v
    end
    ...
    if inInstance and (instanceType == "party" or instanceType == "raid" or instanceType == "scenario") and isBoss then
        ...
        if bossSnapshot and bossSnapshot.perSpell and type(bossSnapshot.perSpell.snapStacksFoF) == "number" then
            local v = math.max(0, math.min(10, bossSnapshot.perSpell.snapStacksFoF))
            -- print(string.format("[SnapFoF] GetSnapStacksFoF: unit=%s boss=%s (byName) => %d", tostring(targetUnit), tostring(tName), v))
            return v
        end
    end

    -- local mapKeys = map and type(map) == "table" and next(map) and "has entries" or "empty/nil"
    -- print(string.format("[SnapFoF] GetSnapStacksFoF: unit=%s guid=%s no snapshot (map %s) => 0", tostring(targetUnit), tostring(guid):sub(-8), mapKeys))
    return 0
end

--- Get whether Unholy Strength (53365) was active when the last Blood Plague snapshot was captured.
--- @param unit? string Optional unit token (defaults to "target")
--- @return boolean True if Unholy Strength was snapped
function Snapshotter:GetSnapUnholyStrength(unit)
    local targetUnit = unit or "target"
    local guid = UnitGUID(targetUnit)
    if not guid or guid == "" then
        return false
    ...
    local map = self.state and self.state.snapshots and self.state.snapshots[DK_BLOOD_PLAGUE]
    local snapshot = type(map) == "table" and map[guid] or nil
    if snapshot and snapshot.perSpell and snapshot.perSpell.snapUnholyStrength == true then
        return true
    end
    ...
    if inInstance and (instanceType == "party" or instanceType == "raid" or instanceType == "scenario") and isBoss then
        ...
        local rec = byNameMap and tName and byNameMap[tName] or nil
        local bossSnapshot = rec and type(rec) == "table" and rec.snapshot or nil
        if bossSnapshot and bossSnapshot.perSpell and bossSnapshot.perSpell.snapUnholyStrength == true then
            return true
        end
    end
