...
--- - Debug: Per-module logging via DebugManager (global debugLevel, optional module debugEnabled)
...
function CoreModule:ResetModuleToDefaults()
    ...
    -- Reset each namespace on the module's own db handle.
    if type(moduleDB.global) == "table" then wipe(moduleDB.global) end
    if type(moduleDB.char) == "table" then wipe(moduleDB.char) end
    if type(moduleDB.profile) == "table" then wipe(moduleDB.profile) end
    if type(moduleDB.class) == "table" then wipe(moduleDB.class) end

    -- Re-register defaults if the module has them.
    if self.defaults and type(moduleDB.RegisterDefaults) == "function" then
...
function NAG:CreateModule(name, defaults, mixins)
    ...
    if mixins then
        ...
        -- Mix in remaining properties (debugCategories removed; ignore if present in old mixins)
        for k, v in pairs(mixins) do
            if k ~= "debugCategories" then
                module[k] = v
            end
        end
    else
...
-- ============================ USAGE EXAMPLES ============================
--[[
Developer debugging (devDebugModules / self.debug) and standard logging:
- if self.debug then self:Debug(msg, ...) end  -- extra verbosity when enabled for this module
- self:Debug / :Info / :Warn / :Error / :Trace  -- controlled by global debugLevel and module debugEnabled

Slash: /nag devdebug help | list | enable <module> | disable <module> | toggle <module> | clear
Global debug level: NAG options (Debug) or /nagdebug level <0-5>
Per-module override: /nagdebug module <name> or DevMode module overrides in options
]]
