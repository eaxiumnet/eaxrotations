...
-- ~~~~~~~~~~ CONTENT ~~~~~~~~~~
-- Create a logging context for namespace functions (DebugManager uses GetName for labels)
...
function ClassBase:ModuleEnable()
    ...
    -- Register slash commands for assignments that have slashCommand field
    self:RegisterAssignmentSlashCommands()
...
function ClassBase:GetAvailableRotations(specIndex, showAllSpecs)
    ...
    local function addRotationsFromSpec(sourceSpecID)
        ...
        for name, config in pairs(allRotations) do
            ...
            if showAllSpecs and (tonumber(sourceSpecID) or 0) > 0 then
    ...
    if showAllSpecs then
        ...
        -- Collect specIndexes from unified rotations (keys may be string after import/serialization)
        local allRotations = self.db.class.rotations
        if allRotations then
            for sourceSpecIndex in pairs(allRotations) do
                local numSpec = tonumber(sourceSpecIndex)
                if numSpec and numSpec > 0 then
                    specIndexes[sourceSpecIndex] = true -- keep original key for DB lookup
                end
            end
        end
...
--- Registers slash commands for assignments that have a slashCommand field.
--- Each registered command toggles the assignment on/off when invoked.
--- @param self ClassBase
function ClassBase:RegisterAssignmentSlashCommands()
    local assignments = self.defaults.class.classAssignments or {}
    local SlashCommandHandler = NAG:GetModule("SlashCommandHandler", true)
    if not SlashCommandHandler or not SlashCommandHandler.RegisterCommandDirect then
        return
    end

    for _, assignment in ipairs(assignments) do
        if assignment and assignment.slashCommand and assignment.slashCommand ~= "" then
            local assignmentId = assignment.id
            local assignmentName = assignment.name or assignmentId
            local cmd = assignment.slashCommand
            local classModule = self

            local ok = SlashCommandHandler:RegisterCommandDirect("nag", cmd, function()
                if not classModule or not classModule.ToggleAssignment then
                    return
                end
                local states = classModule:GetAssignmentStates()
                local current = states[assignmentId] == true
                classModule:ToggleAssignment(assignmentId, not current)
            end, format("Toggle %s assignment on/off", assignmentName), "Assignments", self:GetName())

            if ok then
                self:Debug("Registered assignment slash command: /nag %s", cmd)
            end
        end
    end
end
