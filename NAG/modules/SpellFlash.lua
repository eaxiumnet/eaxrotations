...
--- - /nagdebug spellflash why — diagnose missing bar glow (support)
---
...
function SpellFlash:ModuleInitialize()
    ...
    -- Template is missing on older Classic (Era, SoD, TBC, Wrath); skip attempt there to avoid CreateFrame errors.
    -- On Cata, Mists, and Retail we try once via pcall and cache the result.
    local function templateKnownUnavailable()
    ...
    if templateKnownUnavailable() then
...
-- Dominos: slots 13-24 and 73-168 use DominosActionButtonN (not only 1-12).
local DOMINOS_ACTION_BUTTON_MAX = 168
local BARTENDER_BUTTON_MAX = 180

--- Walk action bar buttons SpellFlash scans (visible only). Used for matching and diagnostics.
--- @param callback fun(button: table, buttonName: string)
function SpellFlash:ForEachTrackedBarButton(callback)
    ...
    for _, pattern in ipairs(buttonPatterns) do
        ...
        elseif pattern == "DominosActionButton%d" then
            for i = 1, DOMINOS_ACTION_BUTTON_MAX do
        ...
        elseif pattern == "BT4Button%d" then
            for i = 1, BARTENDER_BUTTON_MAX do
                local buttonName = string.format(pattern, i)
                local button = _G[buttonName]
                if button and button:IsVisible() then
                    callback(button, buttonName)
        ...
        elseif pattern == "ActionButton%d" then
            for i = 1, 12 do
                local buttonName = string.format(pattern, i)
                local button = _G[buttonName]
                if button and button:IsVisible() then
...
function SpellFlash:FindButtonsForSpell(spellID)
    ...
    self:ForEachTrackedBarButton(function(button)
        if seen[button] then
            return
        end
        seen[button] = true
        ...
        if buttonSpellID == spellID then
            foundButtons[#foundButtons + 1] = button
        end
    end)
...
--- Compact bar snapshot when a spell has no matching button (spell ID vs bar, macros, same-name different ID).
--- @param spellID number Recommended spell ID
--- @param maxParts number Max hint lines
--- @return string
function SpellFlash:FormatBarSnapshotForSpell(spellID, maxParts)
    maxParts = maxParts or 8
    local wantName = spellID and GetSpellInfo(spellID)
    local localCache, localTime = {}, {}
    local now = GetTime()
    local SNAP_CACHE_TTL = 60
    local parts = {}
    local seenBtn = {}
    ...
    self:ForEachTrackedBarButton(function(button, buttonName)
        if seenBtn[button] then
            return
        end
        seenBtn[button] = true
        if #parts >= maxParts then
        ...
        local actionType, rawId = GetActionInfo(button.action)
        local resolved = self:GetActionButtonSpell(button, localCache, localTime, now, SNAP_CACHE_TTL)

        if resolved and spellID and resolved ~= spellID and wantName and GetSpellInfo(resolved) == wantName then
            add(string.format("%s: same spell name, bar id %s != NAG id %s", buttonName, tostring(resolved), tostring(spellID)))
            return
        end

        if actionType == "macro" then
            if not resolved then
                add(string.format("%s: macro — could not resolve spell from body", buttonName))
            elseif spellID and resolved ~= spellID then
                add(string.format("%s: macro resolves to %s (%s), NAG wants %s (%s)",
                    buttonName,
                    tostring(resolved),
                    GetSpellInfo(resolved) or "?",
                    tostring(spellID),
                    wantName or "?"))
            end
            return
    ...
    if #parts == 0 then
        return "no visible tracked buttons with spell/macro/item mismatch (spell may be on unsupported bar or hidden)"
    end
    return table.concat(parts, " | ")
end

--- One-line description of raw NAG.nextSpell for diagnostics.
--- @return string
function SpellFlash:DescribeNextSpellRaw()
    if not NAG then
        return "NAG nil"
    end
    local n = NAG.nextSpell
    if n == nil then
        return "NAG.nextSpell = nil"
    end
    if type(n) == "number" then
        return string.format("NAG.nextSpell = %s (%s)", tostring(n), GetSpellInfo(n) or "?")
    end
    if type(n) == "table" then
        local rid = (NAG.GetActionResolveId and NAG:GetActionResolveId(n)) or n.spellId or n.itemId
        return string.format("NAG.nextSpell table spellId=%s itemId=%s resolveId=%s",
            tostring(n.spellId), tostring(n.itemId), tostring(rid))
    end
    return "NAG.nextSpell type=" .. type(n) .. " " .. tostring(n)
end

--- Print why flashing might be missing (chat).
function SpellFlash:PrintWhyDiagnostic()
    self:Print("|cffffcc00=== SpellFlash: why ===|r")
    self:Print("enabled=" .. tostring(self:IsEnabled()) .. " ShouldFlash=" .. tostring(self:ShouldFlash()) ..
        " inCombat=" .. tostring(UnitAffectingCombat("player")) .. " flashOOC=" ..
        tostring(self:GetSetting("char", "flashOutOfCombat", true)))
    self:Print(self:DescribeNextSpellRaw())

    local recommended = self:GetRecommendedSpells()
    if #recommended == 0 then
        self:Print("|cffff6666No resolved spell IDs (NAG.nextSpell / secondary).|r")
        return
    end

    for i, spellID in ipairs(recommended) do
        local name = GetSpellInfo(spellID) or "?"
        local role = (i == 1) and "primary" or "secondary"
        local suppressed = self:IsSpellSuppressed(spellID)
        local buttons = self:FindButtonsForSpell(spellID)
        self:Print(string.format("%s: %s id=%s suppressed=%s buttons=%d",
            role, name, tostring(spellID), tostring(suppressed), #buttons))
        if #buttons == 0 then
            self:Print("  " .. self:FormatBarSnapshotForSpell(spellID, 8))
        end
    end
end
...
--- Size/position glow on button.icon when present; else match Blizzard overlay (1.4x + insets).
--- @param glow Frame Glow root frame (child of the action button)
--- @param button Frame Action bar button
function SpellFlash:ApplyGlowFrameGeometry(glow, button)
    if not glow or not button then
        return
    end
    ...
    if icon and icon.GetObjectType and icon:GetObjectType() == "Texture" then
        ...
        if iw and ih and iw > 0 and ih > 0 then
            local scale = 1.35
            glow:ClearAllPoints()
            glow:SetSize(iw * scale, ih * scale)
            glow:SetPoint("CENTER", icon, "CENTER", 0, 0)
            return
        end
    end

    local fw, fh = button:GetSize()
    if not fw or not fh or fw <= 0 or fh <= 0 or fw >= 200 or fh >= 200 then
        fw, fh = 40, 40
    end

    glow:ClearAllPoints()
    glow:SetSize(fw * 1.4, fh * 1.4)
    glow:SetPoint("TOPLEFT", button, "TOPLEFT", -fw * 0.2, fh * 0.2)
    glow:SetPoint("BOTTOMRIGHT", button, "BOTTOMRIGHT", fw * 0.2, -fh * 0.2)
end

--- Blizzard ActionBarButtonSpellActivationAlert stacks spark, inner/outer IconAlert slices, and ants.
--- SpellFlash tints all of them the same color, which reads as nested squares (plus ants texture).
--- Keep a single ring: outerGlow sized to the glow frame; hide other template layers and ants.
--- @param glow Frame Frame created with ActionBarButtonSpellActivationAlert
function SpellFlash:ApplySingleRingSpellFlashTemplateVisuals(glow)
    if not glow or not glow.outerGlow or not glow.ants then
        return
    end
    ...
    if glow.innerGlowOver then
        glow.innerGlowOver:Hide()
    end
    if glow.outerGlowOver then
        glow.outerGlowOver:Hide()
    end
    glow.ants:Hide()

    glow.outerGlow:ClearAllPoints()
    glow.outerGlow:SetSize(w, h)
    glow.outerGlow:SetPoint("CENTER", glow, "CENTER", 0, 0)
    glow.outerGlow:SetAlpha(1)
    glow.outerGlow:Show()
end
...
function SpellFlash:CreateFallbackGlowFrame(glowName, button)
    local glow = CreateFrame("Frame", glowName, button)

    self:ApplyGlowFrameGeometry(glow, button)
    local frameWidth, frameHeight = glow:GetSize()
...
function SpellFlash:ShowCustomGlow(button, color)
    ...
    if not glow then
        -- Use template only when not known unavailable; on older Classic (Era, SoD, TBC, Wrath) use fallback
        -- to avoid CreateFrame errors where ActionBarButtonSpellActivationAlert is missing.
        if self._spellActivationTemplateAvailable == false then
            glow = self:CreateFallbackGlowFrame(glowName, button)
        else
    ...
    -- Refresh layout every show (UI scale / icon bounds can change)
    self:ApplyGlowFrameGeometry(glow, button)
    ...
    -- Template path: skip stock animIn (scales inner vs outer, leaves ants — doubled borders when recolored).
    if glow.ants then
        self:ApplySingleRingSpellFlashTemplateVisuals(glow)
    else
...
function SpellFlash:HandleSpellFlashCommand(msg)
    ...
    if command == "" or command == "help" then
        ...
        self:Print("  |cff88ff88/nagdebug spellflash why|r - Diagnose missing bar flash")

    elseif command == "why" then
        self:PrintWhyDiagnostic()
