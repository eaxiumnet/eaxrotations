...
function CustomBehaviorOverrides:RegisterSpellOverrides()
    ...
    -- Healing Touch (5185) - Druid overlay: show next (lowest-HP) target name for macros
    ns.EntityBehaviors.RegisterOverride("Spell", 5185, "SpellCanCast", function(self, tolerance)
        if not NAG:IsKnownSpell(self.id) then return NAG:IsReadySpell(self.id, tolerance) end
        if NAG.CLASS_FILENAME == "DRUID" then
            if NAG:IsReadySpell(self.id, tolerance) and NAG.HealingTouchOverlayTickPrevalidated then
                NAG:HealingTouchOverlayTickPrevalidated()
            end
            return NAG:IsReadySpell(self.id, tolerance)
        end
        return NAG:IsReadySpell(self.id, tolerance)
    ...
    local function isMoPShamanAscendanceActive()
        return NAG.CLASS_FILENAME == "SHAMAN" and Version:IsMoP() and
            (NAG:AuraIsActive(114049, "player", false) or NAG:AuraIsActive(114051, "player", false))
    end
    ...
    -- Lava Burst (51505) - MoP Elemental can cast this continuously during Ascendance.
    -- Treat it as ready while the Ascendance transformation aura is active so the APL can keep recommending it.
    ns.EntityBehaviors.RegisterOverride("Spell", 51505, "IsReady", function(self, tolerance)
        if isMoPShamanAscendanceActive() then
            return true
        end
