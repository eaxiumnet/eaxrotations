...
local function getProcessingEntitiesByType(entityType)
    if not entityType then
        return nil
    end

    local bucket = processingEntities[entityType]
    if not bucket then
        bucket = {}
        processingEntities[entityType] = bucket
    end

    return bucket
end

local function getProcessingEntity(entityType, id)
    local bucket = processingEntities[entityType]
    return bucket and bucket[id] or nil
end

local function setProcessingEntity(entityType, id, entry)
    local bucket = getProcessingEntitiesByType(entityType)
    if bucket then
        bucket[id] = entry
    end
end

local function clearProcessingEntity(entityType, id, entry)
    local bucket = processingEntities[entityType]
    if bucket and bucket[id] == entry then
        bucket[id] = nil
    end
end
...
ProcessorV2 = {
    ...
    process = function(self, id, path, rawEntry)
        local inProgressEntity = getProcessingEntity(self.entityType, id)
        if inProgressEntity then
            return inProgressEntity
        end

        local processingEntry = nil
        ...
        local success, result = pcall(function()
            ...
            setProcessingEntity(self.entityType, id, entry)
        ...
        if processingEntry then
            clearProcessingEntity(self.entityType, id, processingEntry)
        end
    ...
    ensureEntityExists = function(self, entityType, id, path, rawData)
        ...
        local entity = processor:read(id) or getProcessingEntity(entityType, id)
        if not entity then
...
function DataManager:NAG_VERSION_DATA_SELECTED(message)
    ...
    self.dataProcessed = false
    self:ClearIdentityCaches("NAG_VERSION_DATA_SELECTED")
...
function DataManager:Get(id, entityType, internal)
    ...
    if not internal and not self:IsDataReady() then
        -- Cold login often runs rotation/APL (snapshots, aura checks) before DataWalker
        -- finishes. Callers already treat nil as "use WoW API / spellbook fallbacks"; logging
        -- here produced hundreds of identical warns with stacks.
        return nil
    end
    ...
    if not result then
        result = getProcessingEntity(entityType, id)
    end
...
function DataManager:GetIdentity(id, context, opts)
    ...
    -- Do not cache identities while storage is still loading: early calls see nil entities;
    -- caching that would stick until the next ClearIdentityCaches event.
    if dataReady and self._identityCache[cacheKey] ~= nil then
