...
local function isAnySpellActive(ids)
    for i = 1, #ids do
        if NAG:IsActive(ids[i]) then
            return true
        end
    end
...
function PaladinRetTwistModule:HandleCLEUSpellCastSuccess(timestamp, subEvent, hideCaster, sourceGUID, sourceName,
    ...
    if spellId == 20375  -- Seal of Command (Rank 1, cast ID)
        ...
        or containsSpellId(RET_TWIST_SOB_FAMILY_IDS, spellId)
        or containsSpellId(RET_TWIST_SOM_FAMILY_IDS, spellId)
...
function PaladinRetTwistModule:MaybePrintSnapshot(reason)
    ...
    local soBActive = isAnySpellActive(RET_TWIST_SOB_FAMILY_IDS)
    local soMActive = isAnySpellActive(RET_TWIST_SOM_FAMILY_IDS)
