...
local function DecideHunterStateAction(context, stepInput, stateWindow)
    ...
    -- not appropriate. Only action: wait for windup.
    -- autoshot: requires 2+ consecutive autos with GCD ready and no action between autos,
    -- and only applies for slow enough swings (> 1.7s).
    -- ms_zero_backup: immediate (we missed a filler, already "lost").
    -- spell_sent_ms/ar and filler_cd_mismatch: corrective refreshes, show SS in gap or fillers.
    ...
    local autosWithGcdReadyCount = tonumber(stepInput.autosWithGcdReadyCount or 0) or 0
    local waitForWindupTrigger = (triggerSource == "autoshot" or triggerSource == "ms_zero_backup")
    local autoshotCountOk = (triggerSource == "ms_zero_backup") or (triggerSource == "autoshot" and rangedSpeed > FILLER_MIN_SWING_SECONDS and autosWithGcdReadyCount >= 2)
    local isState0Unprepared = waitForWindupTrigger and autoshotCountOk and (type(gcdLeftVal) == "number" and gcdLeftVal <= 0.05) and context.ssQueuedLastState ~= true
...
local function BuildHunterCycleStep(context, stepInput, result)
    ...
    local decision = DecideHunterStateAction(context, {
        ...
        autosWithGcdReadyCount = stepInput.autosWithGcdReadyCount,
...
local function EvaluateStateTimeline(snap, horizonSeconds, options)
    ...
    if computeTrigger == "ms_zero_backup" or computeTrigger == "spell_sent_ms" or computeTrigger == "spell_sent_ar" or computeTrigger == "filler_cd_mismatch" then
        steadyQueued = false
        queuedEndIn = 0
    end
    result.now = now
    ...
    local autosWithGcdReadyCount = tonumber(options and options.autosWithGcdReadyCount or 0) or 0
    local state0Decision = DecideHunterStateAction(state0Context, {
        now = now,
        gcdLeft = gcdLeft,
        triggerSource = computeTrigger,
        autosWithGcdReadyCount = autosWithGcdReadyCount,
        steadyCastSeconds = steadyCastSeconds,
        multiCastSeconds = multiCastSeconds,
        rangedSpeed = rangedSpeed,
        earlyGapDuration = earlyGapDuration,
