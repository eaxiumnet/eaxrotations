...
--- @param clipOverride table|nil Optional autoshot timing override for clip correction.
--- @return boolean refreshed
function HunterStateTimelineDebug:RefreshBarSnapshot(idleAtWindupOverride, triggerSource, clipOverride)
    ...
    if type(clipOverride) == "table" then
        local overrideNow = tonumber(clipOverride.now or 0) or 0
        local overrideRangedTimeToNext = tonumber(clipOverride.overrideRangedTimeToNext or 0) or 0
        local overrideRangedSpeed = tonumber(clipOverride.overrideRangedSpeed or 0) or 0
        if overrideNow > 0 and overrideRangedTimeToNext > 0 then
            timelineOptions.overrideNow = overrideNow
            timelineOptions.overrideRangedTimeToNext = overrideRangedTimeToNext
            if overrideRangedSpeed > 0 then
                timelineOptions.overrideRangedSpeed = overrideRangedSpeed
            end
        end
    end
    timelineOptions.triggerSource = tostring(triggerSource or "unknown")
    -- For autoshot: pass count of consecutive autos with GCD ready and no action between autos.
    if triggerSource == "autoshot" and steadyWeaveBar and steadyWeaveBar.state then
        timelineOptions.autosWithGcdReadyCount = tonumber(steadyWeaveBar.state.autosWithGcdReadyCount or 0) or 0
    end
