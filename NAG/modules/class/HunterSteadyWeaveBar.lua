...
local FILLER_MISMATCH_TIMER = "HunterSteadyBarFillerMismatch"
local MAX_FILLER_BACKUP_TIMERS = 5
local SPELL_SENT_DELAY_BASE_MS = 100
local SPELL_SENT_DELAY_LATENCY_CAP_MS = 100
local MS_ZERO_BACKUP_DELAY_BASE_SECONDS = 0.1   -- Base 100ms after filler gap
local MS_ZERO_BACKUP_LATENCY_CAP_MS = 100      -- Add up to 100ms based on latency (total 100-200ms)
local MS_ZERO_BACKUP_ACTIVE_WINDOW_SECONDS = 0.5  -- Skip if any compute in last 500ms (e.g. we cast MS/AR)
local FILLER_MISMATCH_READY_TOLERANCE_SECONDS = 0.05
local FILLER_MISMATCH_RECHECK_THROTTLE_SECONDS = 0.5
local AURA_SPEED_CHECK_TIMER = "HunterSteadyBarAuraSpeedCheck"
local CLIP_DETECTION_DEFAULT_TOLERANCE_SECONDS = 0.2
local CLIP_DETECTION_MIN_PRINT_OVER_TOLERANCE_MS = 10
local CLIP_DETECTION_GREEN_OVER_TOLERANCE_MS = 20
local CLIP_DETECTION_YELLOW_OVER_TOLERANCE_MS = 50
local CLIP_DETECTION_ORANGE_OVER_TOLERANCE_MS = 80
local CLIP_DETECTION_SNAPSHOT_MAX_AGE_SECONDS = 5.0
local CLIP_FEEDBACK_BASE_VISIBLE_SECONDS = 0.9
local CLIP_FEEDBACK_BASE_FADE_SECONDS = 0.3
local CLIP_FEEDBACK_Y_OFFSET = 30
local CLIP_FEEDBACK_SLIDE_PIXELS = 18
local CLIP_FEEDBACK_GCD_BUFFER_SECONDS = 0.2
...
local UpdateClipFeedbackVisual
...
--- Compute delayed spell-triggered recompute timing: 100ms + latency up to another 100ms.
--- @return number delayMs
local function GetSpellTriggeredComputeDelayMs()
    local latencyMs = (NAG and NAG.GetNetStats and NAG:GetNetStats()) or 0
    if type(latencyMs) ~= "number" or latencyMs < 0 then
        latencyMs = 0
    end
    return SPELL_SENT_DELAY_BASE_MS + min(SPELL_SENT_DELAY_LATENCY_CAP_MS, latencyMs)
end

--- @param spellId number
--- @return number|nil
local function GetLiveSpellReadyIn(spellId)
    if NAG and NAG.SpellTimeToReady then
        local readyIn = tonumber(NAG:SpellTimeToReady(spellId) or -1)
        if readyIn ~= nil and readyIn >= 0 then
            return readyIn
        end
    end
    if NAG and NAG.SpellIsReady and NAG:SpellIsReady(spellId) then
        return 0
    end
    return nil
end
...
--- @param actionName string
--- @return number|nil
local function GetFillerSpellIdForActionName(actionName)
    if actionName == "MS" then
        return MULTI_SHOT_ID
    end
    if actionName == "AR" then
...
--- @return boolean
local function IsSteadyOrAimedSpellId(spellId)
    if type(spellId) ~= "number" then
        return false
    end
    return spellId == STEADY_SHOT_ID or AIMED_SHOT_ID_SET[spellId] == true
end

--- @param snapshot table|nil
--- @param now number
--- @return number|nil
local function GetNextPredictedAutoAt(snapshot, now)
    local snapshotAt = tonumber(snapshot and snapshot.snapshotAt or 0) or 0
    local autoshotMoments = snapshot and snapshot.autoshotMoments
    if snapshotAt <= 0 or type(autoshotMoments) ~= "table" then
        return nil
    end
    for i = 1, #autoshotMoments do
        local candidateAt = snapshotAt + (tonumber(autoshotMoments[i] or 0) or 0)
        if candidateAt > now then
            return candidateAt
        end
    end
    return nil
...
--- @return number
local function GetClipDetectionToleranceSeconds()
    local tolerance = CLIP_DETECTION_DEFAULT_TOLERANCE_SECONDS
    local hunterAPI = NAG:GetModule("TBCHunterAPI", true)
    if hunterAPI and hunterAPI.db and hunterAPI.db.class then
        local configured = tonumber(hunterAPI.db.class.fillerClipToleranceSeconds or tolerance) or tolerance
        if configured >= 0 and configured <= 0.5 then
...
--- @param clipMs number
--- @param toleranceMs number
--- @return string|nil
local function GetClipDetectionColorCode(clipMs, toleranceMs)
    local overToleranceMs = (tonumber(clipMs or 0) or 0) - (tonumber(toleranceMs or 0) or 0)
    if overToleranceMs < CLIP_DETECTION_MIN_PRINT_OVER_TOLERANCE_MS then
        return nil
    end
    if overToleranceMs < CLIP_DETECTION_GREEN_OVER_TOLERANCE_MS then
        return "|cff33ff33"
    end
    if overToleranceMs < CLIP_DETECTION_YELLOW_OVER_TOLERANCE_MS then
        return "|cffffff33"
    end
    if overToleranceMs < CLIP_DETECTION_ORANGE_OVER_TOLERANCE_MS then
        return "|cffff9933"
    end
    return "|cffff3333"
end

--- @param overToleranceMs number
--- @return number, number, number
local function GetClipFeedbackColor(overToleranceMs)
    local overMs = tonumber(overToleranceMs or 0) or 0
    if overMs < CLIP_DETECTION_GREEN_OVER_TOLERANCE_MS then
        return 0.20, 1.00, 0.20
    end
    if overMs < CLIP_DETECTION_YELLOW_OVER_TOLERANCE_MS then
        return 1.00, 0.90, 0.20
    end
    if overMs < CLIP_DETECTION_ORANGE_OVER_TOLERANCE_MS then
...
--- @return number, number, number
local function GetClipFeedbackDurations()
    local visibleSeconds = CLIP_FEEDBACK_BASE_VISIBLE_SECONDS
    local fadeSeconds = CLIP_FEEDBACK_BASE_FADE_SECONDS
    local totalSeconds = visibleSeconds + fadeSeconds
    local gcdSeconds = (NAG and NAG.GCDTimeValue and tonumber(NAG:GCDTimeValue() or 1.5)) or 1.5
    if gcdSeconds <= 0 then
        gcdSeconds = 1.5
    end

    local maxTotalSeconds = gcdSeconds - CLIP_FEEDBACK_GCD_BUFFER_SECONDS
    if maxTotalSeconds <= 0.05 then
        maxTotalSeconds = 0.05
    end
    if totalSeconds > maxTotalSeconds then
        local compression = maxTotalSeconds / totalSeconds
        visibleSeconds = visibleSeconds * compression
        fadeSeconds = fadeSeconds * compression
        totalSeconds = visibleSeconds + fadeSeconds
    end
    return visibleSeconds, fadeSeconds, totalSeconds
end

--- @param offsetY number|nil
local function PositionClipFeedbackText(offsetY)
    if not clipFeedbackText or not greenCircle then
        return
    end
    clipFeedbackText:ClearAllPoints()
    clipFeedbackText:SetPoint("BOTTOM", greenCircle, "TOP", 0, CLIP_FEEDBACK_Y_OFFSET + (tonumber(offsetY or 0) or 0))
end

--- @param self HunterSteadyWeaveBar
--- @param clearState boolean|nil
local function HideClipFeedback(self, clearState)
    if clipFeedbackText then
        clipFeedbackText:Hide()
        clipFeedbackText:SetAlpha(1)
        PositionClipFeedbackText(0)
    end
    if clearState and self then
        self.state = self.state or {}
        self.state.clipFeedback = nil
    end
end

--- @param self HunterSteadyWeaveBar
function UpdateClipFeedbackVisual(self)
    local feedback = self and self.state and self.state.clipFeedback or nil
    if not feedback or not clipFeedbackText then
        return
    end

    clipFeedbackText:SetText(tostring(feedback.text or ""))
    clipFeedbackText:SetTextColor(
        tonumber(feedback.colorR or 1) or 1,
        tonumber(feedback.colorG or 1) or 1,
        tonumber(feedback.colorB or 1) or 1,
        1
    )

    local elapsed = GetTime() - (tonumber(feedback.shownAt or 0) or 0)
    if elapsed < 0 then
        elapsed = 0
    end

    local visibleSeconds = tonumber(feedback.visibleSeconds or 0) or 0
    local fadeSeconds = tonumber(feedback.fadeSeconds or 0) or 0
    local totalSeconds = tonumber(feedback.totalSeconds or 0) or 0
    if totalSeconds <= 0 or elapsed >= totalSeconds then
        HideClipFeedback(self, true)
        return
    end

    if elapsed <= visibleSeconds or fadeSeconds <= 0 then
        clipFeedbackText:SetAlpha(1)
        PositionClipFeedbackText(0)
        clipFeedbackText:Show()
        return
    end

    local fadeProgress = (elapsed - visibleSeconds) / fadeSeconds
    if fadeProgress < 0 then
        fadeProgress = 0
    elseif fadeProgress > 1 then
        fadeProgress = 1
    end

    clipFeedbackText:SetAlpha(1 - fadeProgress)
    PositionClipFeedbackText(CLIP_FEEDBACK_SLIDE_PIXELS * fadeProgress)
    clipFeedbackText:Show()
end
...
--- @param clipMs number
local function ReportPredictedClip(self, castGUID, clipMs)
    local db = self and self.db and self.db.class or nil
    if not db or db.showClipDetection ~= true then
        return
    end

    local now = GetTime()
    self.state = self.state or {}
    if castGUID and self.state.lastClipDetectionCastGUID == castGUID then
        return
    end

    local toleranceMs = GetClipDetectionToleranceSeconds() * 1000
    local colorCode = GetClipDetectionColorCode(clipMs, toleranceMs)
    if not colorCode then
        return
    end

    local roundedClipMs = math.floor((tonumber(clipMs or 0) or 0) + 0.5)
    local overToleranceMs = (tonumber(clipMs or 0) or 0) - toleranceMs
    local red, green, blue = GetClipFeedbackColor(overToleranceMs)
    local visibleSeconds, fadeSeconds, totalSeconds = GetClipFeedbackDurations()
    self.state.lastClipDetectionCastGUID = castGUID
    self.state.lastClipDetectionAt = now
    self.state.clipFeedback = {
        shownAt = now,
        visibleSeconds = visibleSeconds,
        fadeSeconds = fadeSeconds,
        totalSeconds = totalSeconds,
        text = string.format("+%dms", roundedClipMs),
        colorR = red,
        colorG = green,
        colorB = blue,
    }
    ...
    if clipFeedbackText then
        clipFeedbackText:SetText(self.state.clipFeedback.text)
        clipFeedbackText:SetTextColor(red, green, blue, 1)
        clipFeedbackText:SetAlpha(1)
        PositionClipFeedbackText(0)
        clipFeedbackText:Show()
    end
...
--- Detect when a predicted filler is impossible because live cooldown is still longer than its predicted press time.
--- Schedules one immediate recovery recompute per mismatch episode, without spamming.
--- @param self HunterSteadyWeaveBar
--- @param snapshot table
local function HandleWrongFillerPrediction(self, snapshot)
    self.state = self.state or {}

    local mismatchDetected = false
    if snapshot and type(snapshot.segments) == "table" then
        local elapsed = GetSnapshotElapsedSeconds(self, snapshot)
        for i = 1, #snapshot.segments do
            local segment = snapshot.segments[i]
            local actionName = tostring(segment and segment[1] or "")
            local spellId = GetFillerSpellIdForActionName(actionName)
            if spellId then
                local predictedAt = (tonumber(segment and segment[2] or 0) or 0) - elapsed
                if predictedAt < 0 then
                    predictedAt = 0
                end
                local liveReadyIn = GetLiveSpellReadyIn(spellId)
                if type(liveReadyIn) == "number" and liveReadyIn > (predictedAt + FILLER_MISMATCH_READY_TOLERANCE_SECONDS) then
    ...
    if not mismatchDetected then
        self.state.fillerPredictionMismatchActive = false
        return
    end

    local now = GetTime()
    local lastTriggeredAt = tonumber(self.state.fillerPredictionMismatchTriggeredAt or 0) or 0
    if self.state.fillerPredictionMismatchActive == true then
        return
    end
    if lastTriggeredAt > 0 and (now - lastTriggeredAt) < FILLER_MISMATCH_RECHECK_THROTTLE_SECONDS then
        return
    end

    self.state.fillerPredictionMismatchActive = true
    self.state.fillerPredictionMismatchTriggeredAt = now

    if TimerManager and TimerManager.Categories and TimerManager.Create and TimerManager.Cancel then
        TimerManager:Cancel(TimerManager.Categories.UI_NOTIFICATION, FILLER_MISMATCH_TIMER)
        TimerManager:Create(
            TimerManager.Categories.UI_NOTIFICATION,
            FILLER_MISMATCH_TIMER,
            function()
                if self and self.DoPredictionCompute then
                    self:DoPredictionCompute("filler_cd_mismatch")
                end
            end,
            0.001,
            false
...
local function RenderDebugSnapshotTimeline(self, snapshot)
    ...
    if snapshot then
        HandleWrongFillerPrediction(self, snapshot)
        snapshot = self.state and self.state.debugSnapshot
        if not snapshot then
            return false
        end
    end
...
function module:UpdateDisplay()
    ...
    if ShouldUseDebugSnapshot(self) then
        local snapshot = self.state and self.state.debugSnapshot
...
function module:CreateFrameUI()
    ...
    clipFeedbackText = frame:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    clipFeedbackText:SetJustifyH("CENTER")
    clipFeedbackText:SetJustifyV("MIDDLE")
    clipFeedbackText:SetText("")
    PositionClipFeedbackText(0)
    clipFeedbackText:Hide()
...
--- @param clipOverride table|nil Optional autoshot timing override for corrective recomputes.
function module:DoPredictionCompute(triggerSource, clipOverride)
    ...
    local stateDebug = NAG:GetModule("HunterStateTimelineDebug", true)
    if stateDebug and stateDebug.RefreshBarSnapshot then
        stateDebug:RefreshBarSnapshot(nil, triggerSource or "unknown", clipOverride)
    end
    self.state = self.state or {}
...
ScheduleFillerBackups = function(self, segments)
    ...
    for i = 1, #segments do
        ...
        if (actionName == "MS" or actionName == "AR") and t1 > 0 then
            ...
            local latencyAddSeconds = min(MS_ZERO_BACKUP_LATENCY_CAP_MS / 1000, max(0, latencyMs / 1000))
            local delay = t1 + MS_ZERO_BACKUP_DELAY_BASE_SECONDS + latencyAddSeconds
            if delay >= 0.05 then
...
function module:HandleAutoshotSwingStart(eventName, unitId, speed, expirationTime, hand)
    ...
    local gcdLeft = (NAG and NAG.GCDTimeToReady and tonumber(NAG:GCDTimeToReady() or 0)) or 0
    if gcdLeft < 0 then
        gcdLeft = 0
    end
    local currentRangedSpeed = tonumber(speed or 0) or 0
    if currentRangedSpeed <= 0 then
        currentRangedSpeed = GetCurrentRangedSpeed()
    end
    local hadActionSinceLastAuto = self.state.hadActionSinceLastAuto == true
    if currentRangedSpeed > FILLER_MIN_SWING_SECONDS and gcdLeft <= 0.05 and not hadActionSinceLastAuto then
        self.state.autosWithGcdReadyCount = (tonumber(self.state.autosWithGcdReadyCount or 0) or 0) + 1
    else
        self.state.autosWithGcdReadyCount = 0
    end
    self.state.hadActionSinceLastAuto = false
...
function module:ModuleDisable()
    ...
    self.state.autosWithGcdReadyCount = 0
    self.state.fillerPredictionMismatchActive = false
    self.state.fillerPredictionMismatchTriggeredAt = nil
    self.state.lastClipDetectionCastGUID = nil
    self.state.lastClipDetectionAt = nil
    self.state.clipFeedback = nil
    self.state.lastSnapshotSetAt = nil
...
--- Track any player spell press between autos; MS/AR still trigger delayed compute.
function module:UNIT_SPELLCAST_SENT(eventName, unitId, targetName, castGUID, spellIdOrName)
    ...
    self.state = self.state or {}
    self.state.hadActionSinceLastAuto = true
    self.state.autosWithGcdReadyCount = 0
    self.state.fillerPredictionMismatchActive = false
    if MULTI_SHOT_ID_SET[spellId] then
        ScheduleDelayedCompute(self, GetSpellTriggeredComputeDelayMs(), "spell_sent_ms")
    elseif ARCANE_SHOT_ID_SET[spellId] then
        ScheduleDelayedCompute(self, GetSpellTriggeredComputeDelayMs(), "spell_sent_ar")
    end
end
...
function module:UNIT_SPELLCAST_START(eventName, unitId, castGUID, spellId)
    ...
    if not IsSteadyOrAimedSpellId(numericSpellId) then
        return
    end

    self.state = self.state or {}
    local snapshot = self.state.debugSnapshot
    if not snapshot or type(snapshot.autoshotMoments) ~= "table" or #snapshot.autoshotMoments == 0 then
        return
    end

    local now = GetTime()
    local snapshotAt = tonumber(snapshot.snapshotAt or 0) or 0
    if snapshotAt <= 0 or (now - snapshotAt) > CLIP_DETECTION_SNAPSHOT_MAX_AGE_SECONDS then
        return
    end

    local nextAutoAt = GetNextPredictedAutoAt(snapshot, now)
    if not nextAutoAt then
        return
    end
    ...
    if tonumber(castingSpellId or 0) ~= numericSpellId or not endTimeMS then
        return
    end

    local castRemaining = (endTimeMS / 1000) - now
    if castRemaining <= 0 then
        return
    end

    local castEndAt = now + castRemaining
    local windupStartAt = nextAutoAt - windupSeconds
    local clipSeconds = castEndAt - windupStartAt
    if clipSeconds <= 0 then
        return
    end

    local clipMs = clipSeconds * 1000
    local toleranceMs = GetClipDetectionToleranceSeconds() * 1000
    if clipMs < (toleranceMs + CLIP_DETECTION_MIN_PRINT_OVER_TOLERANCE_MS) then
        return
    end

    local clipOverride = {
        now = now,
        overrideRangedTimeToNext = castRemaining,
        overrideRangedSpeed = tonumber(snapshot.rangedSpeed or 0) or 0,
    }

    self:DoPredictionCompute("ss_clip_detected", clipOverride)
    ReportPredictedClip(self, castGUID, clipMs)
end
            ...
            general = {
                ...
                args = {
                    ...
                    useDebugSnapshotWhenAvailable = OptionsFactory:CreateToggle("Use Debug Snapshot",
                        ...
                        { order = 3 }),
                    showClipDetection = OptionsFactory:CreateToggle("Show Clip Detection",
                        "Show floating Steady/Aimed clipping text above the zero marker when a started cast is predicted to push past the windup tolerance.",
                        function() return self:GetSetting("class", "showClipDetection", true) end,
                        function(_, value)
                            self:SetSetting("class", "showClipDetection", value)
                            if value ~= true then
                                HideClipFeedback(self, true)
                            end
                        end,
                    ...
                    alpha = OptionsFactory:CreateRange("Alpha", "Frame alpha.", function() return self:GetSetting("class", "alpha") end,
                        ...
                        { min = 0.1, max = 1.0, step = 0.05, order = 7 }),
                }
            },
            autoshotTicks = {
                type = "group",
