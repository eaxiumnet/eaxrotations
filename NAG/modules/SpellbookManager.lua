...
local combatResolutionCache = {}
local combatResolutionCacheActive = false
...
local function ClearCombatResolutionCache()
    wipe(combatResolutionCache)
    combatResolutionCacheActive = false
end
...
function SpellbookManager:ResolveEffectiveSpellID(aplSpellID)
    ...
    local inCombat = NAG and NAG.InCombat and NAG:InCombat() or false
    if inCombat then
        if not combatResolutionCacheActive then
            ClearCombatResolutionCache()
            combatResolutionCacheActive = true
        end

        local cachedCombatResult = combatResolutionCache[aplSpellID]
        if cachedCombatResult ~= nil then
            return cachedCombatResult == false and nil or cachedCombatResult
        end
    elseif combatResolutionCacheActive then
        -- Per-fight cache only: clear once we leave combat so the next battle
        -- starts with fresh spell resolution data.
        ClearCombatResolutionCache()
    end

    -- CRITICAL PERFORMANCE FIX: Check resolution cache first (O(1) lookup)
    local cachedResolution = resolutionCache[aplSpellID]
    if cachedResolution ~= nil then
        if inCombat then
            combatResolutionCache[aplSpellID] = cachedResolution
        end
    ...
    if spellName then
        ...
        if canonicalSpellID then
            ...
            if inCombat then
                combatResolutionCache[aplSpellID] = canonicalSpellID
            end
    ...
    if spellbookCache[aplSpellID] then
        ...
        if inCombat then
            combatResolutionCache[aplSpellID] = aplSpellID
        end
        return aplSpellID
    end
    ...
    if not spellName then
        if inCombat then
            combatResolutionCache[aplSpellID] = false
        end
        return nil
    end
    ...
    if effectiveSpellID then
        ...
        if inCombat then
            combatResolutionCache[aplSpellID] = effectiveSpellID
        end
    ...
    if spellInfo and spellInfo.name then
        ...
        if apiSpellID then
            ...
            if inCombat then
                combatResolutionCache[aplSpellID] = apiSpellID
            end
            return apiSpellID
        end
    end
...
function SpellbookManager:InvalidateResolutionCache()
    ...
    ClearCombatResolutionCache()
    lastResolutionCacheCleanup = 0
    if NAG and NAG.InvalidateRetTwistSealKnowledge then
        NAG:InvalidateRetTwistSealKnowledge()
    end
end
...
function SpellbookManager:HandleSpellAffectingEvent(event)
    self._handlingSpellAffectingEvent = true
    ...
    if event == "PLAYER_LOGIN" or event == "PLAYER_ENTERING_WORLD" then
        ...
        self._handlingSpellAffectingEvent = false
        return
    end
...
function SpellbookManager:IsHandlingSpellAffectingEvent()
    return self._handlingSpellAffectingEvent == true
end
