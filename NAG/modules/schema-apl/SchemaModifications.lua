...
-- Registry for APL values registered at definition site (e.g. from class/handler files).
-- Merged into GetAllModifications() so SchemaModifier sees them as normal modifications.
SchemaModifications.registeredAPLValues = {}

--- Build fields and field_order from a compact argsSpec for RegisterAPLValue.
--- @param argsSpec table Map of field_name (snake_case) -> "optional"|"required"
--- @param fieldOrder table|nil Optional ordered list of field names; if nil, keys of argsSpec are used (sorted)
--- @return table fields Table suitable for modification.fields
--- @return table field_order Array of field names
local function BuildFieldsFromArgsSpec(argsSpec, fieldOrder)
    local order = fieldOrder or {}
    if #order == 0 then
        for k in pairs(argsSpec) do
            order[#order + 1] = k
        end
        table.sort(order)
    end
    ...
    for i, name in ipairs(order) do
        local label = argsSpec[name] or "optional"
        fields[name] = {
            id = i,
            type = "double",
            label = label,
...
SchemaModifications.nagFunctionMappingsAdditions = {
    ...
    ["UnholyBlightCancelOverlayText"] = "APLValueUnholyBlightCancelOverlayText",
    ["DKHasFoFTrinketEquipped"] = "APLValueDKHasFoFTrinketEquipped",
    ["RogueDesiredSnDComboPoints"] = "APLValueRogueDesiredSnDComboPointsPredictive",
    ["RogueShouldCastAdaptiveSnD"] = "APLValueRogueShouldCastPredictiveSnD",
}
...
SchemaModifications.globalModifications = {
    ...
    -- Handler signature: NAG:ShowFesterblightCancelOverlay()
    -- DK Festerblight: shows UB cancel overlay with "Cancel in" (normal) and "Xs"/"NOW" (red, 40% bigger). No args.
    ["APLActionShowFesterblightCancelOverlay"] = {
        function_type = "APLAction",
        fields = {},
        oneofs = {},
        field_order = {},
        _metadata = {
            full_description = "Shows the Festerblight UB cancel overlay with prefix (normal) and suffix (red, 40% bigger). Always returns false.",
            short_description = "Shows Festerblight cancel overlay.",
            submenu = { "spell" },
            ui_label = "Show Festerblight Cancel Overlay"
        }
    },
...
--- Register an APL value at definition site so schema validation recognizes it.
--- Call this next to where the NAG:FunctionName handler is implemented (e.g. class or handler file).
--- The modification is merged into GetAllModifications() so SchemaModifier applies it like any other.
--- @param nagName string NAG function name in PascalCase (e.g. "RogueHasNearbyEnemyKnownTTDGreaterThan"). Proto type will be "APLValue" .. nagName.
--- @param argsSpec table Map of field_name (snake_case) -> "optional"|"required", or {} for parameterless.
--- @param metadata table|nil Optional. Can include: include_if, short_description, full_description, submenu, ui_label. Use field_order (array of field names) if argument order must not be alphabetical.
function SchemaModifications:RegisterAPLValue(nagName, argsSpec, metadata)
    if not nagName or type(nagName) ~= "string" then
        return
    end
    argsSpec = argsSpec or {}
    metadata = metadata or {}
    local messageType = "APLValue" .. nagName
    local fields, field_order
    if TableUtils and TableUtils.Size and TableUtils.Size(argsSpec) == 0 then
        fields = {}
        field_order = {}
    else
        local order = metadata.field_order
        local metaCopy = {}
        ...
        metadata = metaCopy
        fields, field_order = BuildFieldsFromArgsSpec(argsSpec, order)
    end
    local modification = {
        modification_type = "full",
        function_type = "APLValue",
    ...
    self.registeredAPLValues[messageType] = modification
end
...
function SchemaModifications:GetAllModifications()
    ...
    -- Merge APL values registered at definition site (e.g. from class/handler files)
    if self.registeredAPLValues then
        for messageType, modification in pairs(self.registeredAPLValues) do
            allModifications[messageType] = modification
        end
    end
...
function SchemaModifications:GetModification(messageType)
    ...
    if SchemaModifications.globalModifications[messageType] then
        return SchemaModifications.globalModifications[messageType]
    end

    if self.registeredAPLValues and self.registeredAPLValues[messageType] then
        return self.registeredAPLValues[messageType]
    end
