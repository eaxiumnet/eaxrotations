...
-- Settings export/import: delta only (overrides vs defaults), compressed+encoded string
local SETTINGS_PREFIX = "NAG_SETTINGS:1:"
local SETTINGS_PREFIX_RAW = "NAG_SETTINGS_RAW:1:" -- Debug option: raw JSON, unencoded/uncompressed (not re-importable)
local SETTINGS_HEADER_PATTERN = "^NAG_SETTINGS:1:(.+)$"
...
-- Keys we never export (license keys, rotations)
local EXCLUDE_GLOBAL_KEYS = { keys = true }
local EXCLUDE_CLASS_KEYS = { rotations = true }
local EXCLUDE_CHAR_KEYS = { selectedRotations = true }
-- Namespaces that store large/runtime data rather than user settings (excluded from settings export)
local EXCLUDE_NAMESPACES = { LearnedPredictionProvider = true }

-- Export only current character's char data under key "__current" so the string is portable (no "CharacterName - Realm" keys in char).
-- profileKeys are included for AceDB backup/restore. Char data: we export one blob (current toon) applied on import to current character.
local SETTINGS_EXPORT_CHAR_AS_CURRENT_ONLY = true

--- Deep-copy table for export, excluding license keys, rotations, selectedRotations. Strips functions.
--- Used for full (v1) export and as the clone step before stripping defaults.
local function copySvForExport(src, excludeParentKey, excludeKey)
    if type(src) ~= "table" then
        return src
    end
    excludeParentKey = excludeParentKey or ""
    excludeKey = excludeKey or ""
    local result = {}
    for k, v in pairs(src) do
        if excludeParentKey == "global" and k == "keys" then
            -- Never export license keys
        elseif excludeParentKey == "class" and k == "rotations" then
            -- Exclude rotations; use separate rotation import/export (NAG_ROTATION / NAG_BUNDLE)
        elseif excludeParentKey == "char" and k == "selectedRotations" then
            -- Exclude selected-rotation state; tied to rotations
        elseif type(v) == "function" then
            -- Strip functions
        elseif type(v) == "table" then
            result[k] = copySvForExport(v, k, nil)
            if result[k] and next(result[k]) == nil then
                result[k] = nil
            end
        else
            result[k] = v
...
--- Strip default values from a table (AceDB-style). Mutates the given table; use on a clone.
--- Only keys present in defaults are considered; values equal to the default are removed.
--- Empty tables after stripping are removed. Does not handle AceDB ["*"]/["**"] wildcards.
--- @param tbl table Table to strip (will be modified)
--- @param defaults table|nil Defaults for this section; nil = no stripping
local function stripDefaultsFromTable(tbl, defaults)
    if type(tbl) ~= "table" or not defaults or type(defaults) ~= "table" then
        return
    end
    for k, v in pairs(defaults) do
        if k == "*" or k == "**" then
            -- AceDB wildcard defaults; skip (NAG does not use these)
        elseif type(v) == "table" and type(tbl[k]) == "table" then
            stripDefaultsFromTable(tbl[k], v)
...
--- Build delta for one section: clone (with exclusions), strip defaults, return if non-empty.
--- @param src table Raw sv section (e.g. sv.global, sv.profiles[name])
--- @param sectionDefaults table|nil Defaults for this section
--- @param excludeSet table|nil Set of keys to omit from clone (e.g. EXCLUDE_GLOBAL_KEYS)
--- @return table|nil Delta table or nil if empty
local function sectionDelta(src, sectionDefaults, excludeSet)
    if not src or type(src) ~= "table" then
        return nil
    end
    local clone = {}
    for k, v in pairs(src) do
        if excludeSet and excludeSet[k] then
            -- skip
        elseif type(v) == "table" then
            clone[k] = copySvForExport(v, nil, nil)
            if clone[k] and next(clone[k]) == nil then
                clone[k] = nil
            end
        else
            clone[k] = v
    ...
    stripDefaultsFromTable(clone, sectionDefaults)
...
--- Build root-level delta payload from db.sv (only overrides, no defaults).
--- Excludes global.keys, class.rotations, char.selectedRotations.
--- For portability, char data is exported only as the current character's delta under key "__current"
--- (no "CharacterName - Realm" keys in char). profileKeys are included for AceDB backup/restore.
--- @param sv table db.sv
--- @param db table NAG.db (must have .defaults and .children)
--- @return table Payload suitable for JSON (delta only)
local function buildDeltaPayload(sv, db)
    local payload = {}
    local defaults = db.defaults or {}
    local keys = db.keys or {}

    if sv.global and type(sv.global) == "table" then
        local delta = sectionDelta(sv.global, defaults.global, EXCLUDE_GLOBAL_KEYS)
        if delta then
            payload.global = delta
        end
    end

    if sv.profiles and type(sv.profiles) == "table" then
        local profileDef = defaults.profile or {}
        local out = {}
        for profileKey, profileData in pairs(sv.profiles) do
            if type(profileData) == "table" then
                local delta = sectionDelta(profileData, profileDef, nil)
                if delta then
                    out[profileKey] = delta
                end
            end
        end
        if next(out) then
            payload.profiles = out
        end
    end

    if sv.profileKeys and type(sv.profileKeys) == "table" and next(sv.profileKeys) then
        payload.profileKeys = TableUtils and TableUtils.DeepClone(sv.profileKeys) or sv.profileKeys
    end

    if sv.char and type(sv.char) == "table" then
        local charDef = defaults.char or {}
        local out = {}
        local currentCharKey = keys.char
        if SETTINGS_EXPORT_CHAR_AS_CURRENT_ONLY and currentCharKey and sv.char[currentCharKey] and type(sv.char[currentCharKey]) == "table" then
            local d = sectionDelta(sv.char[currentCharKey], charDef, EXCLUDE_CHAR_KEYS)
            if d then out["__current"] = d end
        else
            for ck, cv in pairs(sv.char) do
                if type(cv) == "table" then
                    local d = sectionDelta(cv, charDef, EXCLUDE_CHAR_KEYS)
                    if d then out[ck] = d end
                end
            end
        end
        if next(out) then payload.char = out end
    end
    if sv.class and type(sv.class) == "table" then
        local classDef = defaults.class or {}
        local out = {}
        for ck, cv in pairs(sv.class) do
            if type(cv) == "table" then
                local d = sectionDelta(cv, classDef, EXCLUDE_CLASS_KEYS)
                if d then out[ck] = d end
            end
        end
        if next(out) then payload.class = out end
    end

    if sv.namespaces and type(sv.namespaces) == "table" then
        local children = db.children or {}
        local out = {}
        for nsName, nsSv in pairs(sv.namespaces) do
            if EXCLUDE_NAMESPACES[nsName] then
                -- Skip data-heavy namespaces (learned predictions, etc.); not user settings
            elseif type(nsSv) == "table" then
                local nsDb = children[nsName]
                local nsDefaults = (nsDb and nsDb.defaults) or {}
                local nsPayload = {}

                if nsSv.global and type(nsSv.global) == "table" then
                    local d = sectionDelta(nsSv.global, nsDefaults.global, EXCLUDE_GLOBAL_KEYS)
                    if d then nsPayload.global = d end
                end
                if nsSv.profiles and type(nsSv.profiles) == "table" then
                    local pdef = nsDefaults.profile or {}
                    local profs = {}
                    for pk, pv in pairs(nsSv.profiles) do
                        if type(pv) == "table" then
                            local d = sectionDelta(pv, pdef, nil)
                            if d then profs[pk] = d end
                        end
                    end
                    if next(profs) then nsPayload.profiles = profs end
                end
                if nsSv.char and type(nsSv.char) == "table" then
                    local charDef = nsDefaults.char or {}
                    local currentCharKey = keys.char
                    if SETTINGS_EXPORT_CHAR_AS_CURRENT_ONLY and currentCharKey and nsSv.char[currentCharKey] and type(nsSv.char[currentCharKey]) == "table" then
                        local d = sectionDelta(nsSv.char[currentCharKey], charDef, EXCLUDE_CHAR_KEYS)
                        if d then
                            nsPayload.char = nsPayload.char or {}
                            nsPayload.char["__current"] = d
                        end
                    else
                        for ck, cv in pairs(nsSv.char) do
                            if type(cv) == "table" then
                                local d = sectionDelta(cv, charDef, EXCLUDE_CHAR_KEYS)
                                if d then
                                    nsPayload.char = nsPayload.char or {}
                ...
                if nsSv.class and type(nsSv.class) == "table" then
                    local classDef = nsDefaults.class or {}
                    for ck, cv in pairs(nsSv.class) do
                        if type(cv) == "table" then
                            local d = sectionDelta(cv, classDef, EXCLUDE_CLASS_KEYS)
                            if d then
                                nsPayload.class = nsPayload.class or {}
                                nsPayload.class[ck] = d
                            end
                        end
                    end
                end
                if next(nsPayload) then
                    out[nsName] = nsPayload
                end
            end
        end
        if next(out) then
            payload.namespaces = out
        end
    end
...
--- Export all NAG settings as delta only (overrides vs AceDB defaults).
--- Excludes: global.keys, rotations, selectedRotations. Includes profileKeys. Char data is exported only as
--- current character under key "__current" for portability. Excludes data-heavy namespaces (e.g. LearnedPredictionProvider).
--- When debug option "Export settings as raw JSON" is enabled, returns unencoded/uncompressed JSON for testing.
--- @return string|nil Export string with prefix NAG_SETTINGS:1: (or NAG_SETTINGS_RAW:1: when raw debug option is on), or nil on failure
--- @return string|nil Error message if failed
function ImportExport:ExportAllSettings()
    ...
    if not sv or type(sv) ~= "table" then
        return nil, "SavedVariables not available"
    end

    local payload = buildDeltaPayload(sv, db)
    payload = stripFunctions(payload)
    if not payload then
        return nil, "Failed to prepare export data"
    end

    local converted = convertNilToJsonNull(payload)
    local serialized
    local ok, err = pcall(function()
        serialized = json.Serialize(converted)
    end)
    if not ok or not serialized then
        return nil, "JSON serialization failed: " .. tostring(err)
    end

    -- Debug: when "Export settings as raw JSON" is on, skip compression/encoding so user can inspect payload
    local optionsManager = NAG:GetModule("OptionsManager", true)
    if optionsManager and optionsManager.GetSetting and optionsManager:GetSetting("global", "exportSettingsRawForTesting") then
        return SETTINGS_PREFIX_RAW .. serialized
    end
    ...
    local encoded = encodeData(compressed)
    if not encoded then
        return nil, "Encoding failed"
    end

    return SETTINGS_PREFIX .. encoded
end

--- Import settings from a string produced by ExportAllSettings (delta merge into current sv).
--- @param importString string String with prefix NAG_SETTINGS:1:
--- @param options table|nil Optional: { resetFirst = true } to clear settings (except license keys) before merging
--- @return boolean success
--- @return string|nil Error message if failed
function ImportExport:ImportSettingsString(importString, options)
    if type(importString) ~= "string" or importString == "" then
        return false, "No import string provided"
    end
    local payloadEncoded = importString:match(SETTINGS_HEADER_PATTERN)
    if not payloadEncoded or payloadEncoded == "" then
        return false, "Not a settings string (expected NAG_SETTINGS:1:...)"
    end

    local decoded = decodeData(payloadEncoded)
    if not decoded then
        return false, "Failed to decode settings string"
    end

    local decompressed, decompErr = LibCompress:Decompress(decoded)
    if not decompressed then
        return false, "Failed to decompress: " .. tostring(decompErr)
    end

    local ok, data = pcall(json.Deserialize, decompressed)
    if not ok or type(data) ~= "table" then
        return false, "Failed to deserialize settings"
    end

    local db = NAG and NAG.db
    if not db or not db.sv then
        return false, "Database not available"
    end

    local sv = db.sv
    local currentCharKey = db.keys and db.keys.char
    local currentClassKey = db.keys and db.keys.class

    if options and options.resetFirst then
        local preservedKeys = (sv.global and type(sv.global.keys) == "table") and TableUtils.DeepClone(sv.global.keys) or nil
        if sv.global and type(sv.global) == "table" then wipe(sv.global) end
        if preservedKeys then
            sv.global = sv.global or {}
            sv.global.keys = preservedKeys
        end
        if sv.profiles and type(sv.profiles) == "table" then wipe(sv.profiles) end
        if sv.profileKeys and type(sv.profileKeys) == "table" then wipe(sv.profileKeys) end
        if sv.char and type(sv.char) == "table" then wipe(sv.char) end
        if sv.class and type(sv.class) == "table" then wipe(sv.class) end
        if sv.namespaces and type(sv.namespaces) == "table" then wipe(sv.namespaces) end
    end

    if data.global and type(data.global) == "table" then
        if not sv.global then sv.global = {} end
        local clone = TableUtils and TableUtils.DeepClone(data.global) or data.global
        if clone.keys then clone.keys = nil end
        TableUtils.DeepMerge(sv.global, clone)
    end
    if data.profiles and type(data.profiles) == "table" then
        sv.profiles = sv.profiles or {}
        for profileKey, delta in pairs(data.profiles) do
            if type(delta) == "table" then
                if not sv.profiles[profileKey] then sv.profiles[profileKey] = {} end
                TableUtils.DeepMerge(sv.profiles[profileKey], TableUtils and TableUtils.DeepClone(delta) or delta)
            end
        end
    end
    if data.profileKeys and type(data.profileKeys) == "table" then
        sv.profileKeys = sv.profileKeys or {}
        TableUtils.DeepMerge(sv.profileKeys, TableUtils and TableUtils.DeepClone(data.profileKeys) or data.profileKeys)
    end
    if data.char and type(data.char) == "table" then
        sv.char = sv.char or {}
        for ck, cv in pairs(data.char) do
            if type(cv) == "table" then
                local targetKey = (ck == "__current" and currentCharKey) or ck
                if targetKey then
                    if not sv.char[targetKey] then sv.char[targetKey] = {} end
                    local c = TableUtils and TableUtils.DeepClone(cv) or cv
                    if c.selectedRotations then c.selectedRotations = nil end
                    TableUtils.DeepMerge(sv.char[targetKey], c)
                end
            end
        end
        -- Apply one char delta to current character when no __current (old export format with character keys)
        if currentCharKey and next(data.char) and not data.char["__current"] then
            for _, cv in pairs(data.char) do
                if type(cv) == "table" then
                    if not sv.char[currentCharKey] then sv.char[currentCharKey] = {} end
                    local c = TableUtils and TableUtils.DeepClone(cv) or cv
                    if c.selectedRotations then c.selectedRotations = nil end
                    TableUtils.DeepMerge(sv.char[currentCharKey], c)
                    break
                end
            end
        end
    end
    if data.class and type(data.class) == "table" then
        sv.class = sv.class or {}
        for ck, cv in pairs(data.class) do
            if type(cv) == "table" then
                if not sv.class[ck] then sv.class[ck] = {} end
                local c = TableUtils and TableUtils.DeepClone(cv) or cv
                if c.rotations then c.rotations = nil end
                TableUtils.DeepMerge(sv.class[ck], c)
            end
        end
        -- Apply one class delta to current class so importer sees class-scoped settings
        if currentClassKey and next(data.class) then
            for _, cv in pairs(data.class) do
                if type(cv) == "table" then
                    if not sv.class[currentClassKey] then sv.class[currentClassKey] = {} end
                    local c = TableUtils and TableUtils.DeepClone(cv) or cv
                    if c.rotations then c.rotations = nil end
                    TableUtils.DeepMerge(sv.class[currentClassKey], c)
                    break
                end
            end
        end
    end
    if data.namespaces and type(data.namespaces) == "table" then
        sv.namespaces = sv.namespaces or {}
        for nsName, nsDelta in pairs(data.namespaces) do
            if type(nsDelta) == "table" then
                if not sv.namespaces[nsName] then sv.namespaces[nsName] = {} end
                local nsSv = sv.namespaces[nsName]
                if nsDelta.global and type(nsDelta.global) == "table" then
                    nsSv.global = nsSv.global or {}
                    local g = TableUtils and TableUtils.DeepClone(nsDelta.global) or nsDelta.global
                    if g.keys then g.keys = nil end
                    TableUtils.DeepMerge(nsSv.global, g)
                end
                if nsDelta.profiles and type(nsDelta.profiles) == "table" then
                    nsSv.profiles = nsSv.profiles or {}
                    for pk, pv in pairs(nsDelta.profiles) do
                        if type(pv) == "table" then
                            if not nsSv.profiles[pk] then nsSv.profiles[pk] = {} end
                            TableUtils.DeepMerge(nsSv.profiles[pk], TableUtils and TableUtils.DeepClone(pv) or pv)
                        end
                    end
                end
                if nsDelta.char and type(nsDelta.char) == "table" then
                    nsSv.char = nsSv.char or {}
                    for ck, cv in pairs(nsDelta.char) do
                        if type(cv) == "table" then
                            local targetKey = (ck == "__current" and currentCharKey) or ck
                            if targetKey then
                                if not nsSv.char[targetKey] then nsSv.char[targetKey] = {} end
                                local c = TableUtils and TableUtils.DeepClone(cv) or cv
                                if c.selectedRotations then c.selectedRotations = nil end
                                TableUtils.DeepMerge(nsSv.char[targetKey], c)
                            end
                        end
                    end
                    -- Apply one char delta to current character when no __current (old export format)
                    if currentCharKey and next(nsDelta.char) and not nsDelta.char["__current"] then
                        for _, cv in pairs(nsDelta.char) do
                            if type(cv) == "table" then
                                if not nsSv.char[currentCharKey] then nsSv.char[currentCharKey] = {} end
                                local c = TableUtils and TableUtils.DeepClone(cv) or cv
                                if c.selectedRotations then c.selectedRotations = nil end
                                TableUtils.DeepMerge(nsSv.char[currentCharKey], c)
                                break
                            end
                        end
                    end
                end
                if nsDelta.class and type(nsDelta.class) == "table" then
                    nsSv.class = nsSv.class or {}
                    for ck, cv in pairs(nsDelta.class) do
                        if type(cv) == "table" then
                            if not nsSv.class[ck] then nsSv.class[ck] = {} end
                            local c = TableUtils and TableUtils.DeepClone(cv) or cv
                            if c.rotations then c.rotations = nil end
                            TableUtils.DeepMerge(nsSv.class[ck], c)
                        end
                    end
                    -- Apply one class delta to current class so importer sees namespace class-scoped settings
                    if currentClassKey and next(nsDelta.class) then
                        for _, cv in pairs(nsDelta.class) do
                            if type(cv) == "table" then
                                if not nsSv.class[currentClassKey] then nsSv.class[currentClassKey] = {} end
                                local c = TableUtils and TableUtils.DeepClone(cv) or cv
                                if c.rotations then c.rotations = nil end
                                TableUtils.DeepMerge(nsSv.class[currentClassKey], c)
                                break
                            end
                        end
                    end
...
function ImportExport:ShowImportSettingsDialog()
    StaticPopup_Show("NAG_IMPORT_SETTINGS_STRING")
end

function ImportExport:ShowExportSettingsDialog()
    local str, err = self:ExportAllSettings()
    if not str then
        self:Error("Export failed: " .. tostring(err))
        return
    end
    StaticPopup_Show("NAG_EXPORT_SETTINGS_STRING", nil, nil, str)
end
...
StaticPopupDialogs["NAG_IMPORT_ROTATION_STRING"] = {
    ...
    OnAccept = function(dialog)
        ...
        -- Full settings import (v1 full replace or v2 delta merge)
        if strmatch(importString, "^NAG_SETTINGS:%d+:") then
            local ok, err = ImportExport:ImportSettingsString(importString)
            if not ok then
                NAG:Error("Failed to import settings: " .. tostring(err))
                return
            end
            AceConfigRegistry:NotifyChange("NAG")
            NAG:Info("Settings imported successfully. Reload UI to apply fully.")
            StaticPopup_Show("NAG_SETTINGS_IMPORTED_RELOAD")
            return
        end
...
-- When enterClicksFirstButton = true, Blizzard sets dialog:SetScript("OnKeyDown", StaticPopup_OnKeyDown).
-- That handler calls RunBinding("SCREENSHOT") for Print Screen (protected → ADDON_ACTION_FORBIDDEN)
-- and StaticPopup_OnClick for Enter. Use enterClicksFirstButton = false and handle Enter in
-- EditBoxOnEnterPressed (accept + Hide) so the dialog never gets OnKeyDown and no RunBinding runs.
StaticPopupDialogs["NAG_EXPORT_SETTINGS_STRING"] = {
    text = L["exportSettingsString"] or "Copy the string below to backup or share your settings (license keys are never included):",
    button1 = OKAY,
    hasEditBox = true,
    editBoxWidth = 350,
    maxLetters = 9999999,
    ...
    OnShow = function(dialog)
        ...
        if name then
            ...
            if btn1 then
                btn1:ClearAllPoints()
                btn1:SetPoint("BOTTOM", dialog, "BOTTOM", 0, 24)
            end
        end
    end,
...
StaticPopupDialogs["NAG_IMPORT_SETTINGS_STRING"] = {
    text = L["importSettingsString"] or "Paste the settings string below (from backup or another machine):",
    button1 = ACCEPT,
    button2 = CANCEL,
    hasEditBox = true,
    editBoxWidth = 350,
    ...
    OnShow = function(dialog)
        dialog:SetFrameStrata("FULLSCREEN_DIALOG")
        dialog:SetFrameLevel(200)
        -- Fix width so reused StaticPopup frame does not grow each time (content-based resize can accumulate)
        dialog:SetWidth(350)
        local editBox = dialog:GetEditBox()
        if editBox then
            editBox:SetFocus()
        end
        if not dialog.resetCheckbox then
            local check = CreateFrame("CheckButton", nil, dialog, "UICheckButtonTemplate")
            check:SetSize(22, 22)
            check:SetPoint("TOPLEFT", editBox, "BOTTOMLEFT", 0, -12)
            local label = check:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
            label:SetPoint("LEFT", check, "RIGHT", 4, 0)
            label:SetPoint("RIGHT", dialog, "RIGHT", -24, 0)
            label:SetWordWrap(true)
            label:SetNonSpaceWrap(true)
            label:SetText(L["resetSettingsBeforeImport"] or "Reset settings before import")
            check:SetHitRectInsets(0, -label:GetStringWidth() - 4, 0, 0)
            dialog.resetCheckbox = check
        end
        dialog.resetCheckbox:SetChecked(false)
        dialog.resetCheckbox:Show()
        -- Reposition Accept/Cancel to bottom of dialog (StaticPopup places them under content by default)
        local name = dialog:GetName()
        if name then
            local btn1 = _G[name .. "Button1"]
            ...
            if btn1 and btn2 then
                btn1:ClearAllPoints()
                btn2:ClearAllPoints()
                btn1:SetPoint("BOTTOM", dialog, "BOTTOM", -60, 24)
                btn2:SetPoint("BOTTOM", dialog, "BOTTOM", 60, 24)
            end
        end
    end,
    OnHide = function(dialog)
        if dialog.resetCheckbox then
            dialog.resetCheckbox:Hide()
        end
    end,
    EditBoxOnEnterPressed = function(editBox)
        local dialog = editBox:GetParent()
    ...
    OnAccept = function(dialog)
        ...
        local resetFirst = dialog.resetCheckbox and dialog.resetCheckbox:GetChecked()
        local ok, err = ImportExport:ImportSettingsString(importString, { resetFirst = resetFirst })
        if not ok then
            NAG:Error("Failed to import settings: " .. tostring(err))
            return
        end
...
StaticPopupDialogs["NAG_SETTINGS_IMPORTED_RELOAD"] = {
    text = L["settingsImportedReload"] or "Settings imported. Reload UI to apply changes?",
    button1 = YES,
    button2 = NO,
    OnAccept = function()
        ReloadUI()
