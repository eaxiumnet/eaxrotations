...
local LOG_WINDOW_LIMIT = 200 -- max logs per window for DEBUG and TRACE only (see log())
...
-- Opt-in mirror of DebugManager output to session NDJSON (docs/logging-infrastructure.md)
local MIRROR_WINDOW_SECONDS = 1.0
local MIRROR_WINDOW_LIMIT = 80
local mirrorWindowStart = 0
local mirrorWindowCount = 0

--- Append one formatted DebugManager line to NAG._debugSessionLog when global options allow.
--- @param moduleSelf table|string|nil
--- @param severity number
--- @param formattedMessage string
local function maybeMirrorToSessionLog(moduleSelf, severity, formattedMessage)
    if not ns.SessionDebugLogAppend then
        return
    end
    local OptionsManager = ns.OptionsManager
    if not OptionsManager or not OptionsManager.db then
    ...
    local ok, enabled = pcall(ns.DatabaseUtils.GetSetting, OptionsManager.db, "global", "debugMirrorToSessionLog", false)
    if (not ok) or not enabled then
        return
    end
    local ok2, maxLevel = pcall(ns.DatabaseUtils.GetSetting, OptionsManager.db, "global",
        "debugMirrorToSessionLogMaxLevel", ns.DEBUG_LEVELS.WARN)
    maxLevel = (ok2 and tonumber(maxLevel)) or ns.DEBUG_LEVELS.WARN
    if not severity or type(severity) ~= "number" then
        return
    end
    -- Mirror ERROR (1) through maxLevel inclusive on ns.DEBUG_LEVELS scale
    if severity > maxLevel then
        return
    end
    local now = GetTime()
    if (now - mirrorWindowStart) > MIRROR_WINDOW_SECONDS then
        mirrorWindowStart = now
        mirrorWindowCount = 0
    end
    mirrorWindowCount = mirrorWindowCount + 1
    if mirrorWindowCount > MIRROR_WINDOW_LIMIT then
        return
    end
    local modName = "Unknown"
    if moduleSelf and type(moduleSelf.GetName) == "function" then
        modName = moduleSelf:GetName() or modName
    elseif type(moduleSelf) == "string" then
        modName = moduleSelf
    end
    local text = formattedMessage
    if #text > 2000 then
        text = string.sub(text, 1, 2000) .. "..."
    end
    ns.SessionDebugLogAppend(modName .. ":DebugManager", "debugmanager_log", {
        severity = severityToCategory(severity),
        text = text,
    }, { subsystem = "debugmanager" })
end
...
do -- ~~~~~~~~~~ Public API ~~~~~~~~~~
    ...
    function DebugManager:log(moduleSelf, message, severity, printTrace)
        ...
        maybeMirrorToSessionLog(moduleSelf, severity, formattedMessage)
