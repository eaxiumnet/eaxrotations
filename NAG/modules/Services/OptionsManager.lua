...
local defaults = {
    global = {
        ...
        -- Opt-in: copy passing DebugManager lines to NAG._debugSessionLog (see docs/logging-infrastructure.md)
        debugMirrorToSessionLog = false,
        debugMirrorToSessionLogMaxLevel = ns.DEBUG_LEVELS.WARN, -- Mirror ERROR..this level (inclusive on 1=ERROR .. 5=TRACE scale)
        debugOutputChannel = 1,             -- Chat frame for debug output (1 = default, 2–10 = ChatFrame2–10)
        exportSettingsRawForTesting = false, -- When true, settings export is raw JSON (unencoded/uncompressed) for inspection
...
function OptionsManager:CreateSplashOptions()
    ...
    local splashOptions = {
        ...
        settingsBackupGroup = {
            type = "group",
            name = L["settingsBackup"] or "Settings backup",
            order = 4,
            inline = true,
            args = {
                exportSettings = {
                    type = "execute",
                    name = L["exportSettings"] or "Export settings",
                    desc = L["exportSettingsDesc"] or "Copy a string to backup or share your settings (license keys are never included)",
                    width = "normal",
                    order = 1,
                    func = function()
                        local ImportExport = NAG:GetModule("ImportExport", true)
                        if ImportExport and ImportExport.ShowExportSettingsDialog then
                            ImportExport:ShowExportSettingsDialog()
                        else
                            NAG:Error("Import/Export not available")
                        end
                    end,
                },
                importSettings = {
                    type = "execute",
                    name = L["importSettings"] or "Import settings",
                    desc = L["importSettingsDesc"] or "Paste a settings string from backup or another machine",
                    width = "normal",
                    order = 2,
                    func = function()
                        local ImportExport = NAG:GetModule("ImportExport", true)
                        if ImportExport and ImportExport.ShowImportSettingsDialog then
                            ImportExport:ShowImportSettingsDialog()
                        else
                            NAG:Error("Import/Export not available")
                        end
                    end,
...
--- DEBUG tab tree: Ace shows non-inline top-level groups in the left nav; `order` must be consistent.
--- Bands: built-in groups 10–65; module `optionsOrder` from IntegrateModuleOptions should stay >= 100
--- (and avoid GetOptions root `order` below 65 or that module sorts above Debug Tools).
...
function OptionsManager:CreateDebugOptions()
    return {
        ...
        order = 5,
        args = {
            -- Core logging first; per-level channel overrides are a separate group below (clearer tree).
            debugCoreGroup = {
                type = "group",
                name = L["debugCoreGroup"] or "Logging & chat output",
                desc = L["debugCoreGroupDesc"] or "Global severity, developer mode, session log mirror, and the default chat tab for debug lines.",
                order = 10,
                inline = true,
                args = {
                    ...
                    debugMirrorToSessionLog = OptionsFactory:CreateToggle(L["debugMirrorToSessionLog"] or "Mirror debug output to session log", L["debugMirrorToSessionLogDesc"] or "When enabled, DebugManager messages that pass filters are also appended to the in-memory session log (same buffer as /nag debuglog). Rate-limited (80/s). Does not bypass global Debug Level.",
                        function()
                            return self:GetSetting("global", "debugMirrorToSessionLog", false)
                        end,
                        function(_, value)
                            self:SetSetting("global", "debugMirrorToSessionLog", value)
                        end,
                        {
                            order = 2.25,
                            width = "full"
                    ...
                    debugMirrorToSessionLogMaxLevel = OptionsFactory:CreateSelect(L["debugMirrorToSessionLogMaxLevel"] or "Mirror up through level", L["debugMirrorToSessionLogMaxLevelDesc"] or "Cap mirrored severities (Error only … through Trace). Default Warning = ERROR+WARN. Raise global Debug Level if you choose Info/Debug/Trace or those lines are never emitted.",
                        function()
                            return self:GetSetting("global", "debugMirrorToSessionLogMaxLevel", ns.DEBUG_LEVELS.WARN)
                        end,
                        function(_, value)
                            self:SetSetting("global", "debugMirrorToSessionLogMaxLevel", value)
                        end,
                        {
                            order = 2.26,
                            width = "double",
                            values = {
                                [ns.DEBUG_LEVELS.ERROR] = "Error only",
                                [ns.DEBUG_LEVELS.WARN] = "Warning (errors + warnings)",
                                [ns.DEBUG_LEVELS.INFO] = "Info",
                                [ns.DEBUG_LEVELS.DEBUG] = "Debug",
                                [ns.DEBUG_LEVELS.TRACE] = "Trace (all severities)",
                            }
                        }
                    ),
                    ...
                    debugStatus = {
                        ...
                        func = function()
                            ...
                            local mirrorOn = self:GetSetting("global", "debugMirrorToSessionLog", false)
                            local mirrorMax = self:GetSetting("global", "debugMirrorToSessionLogMaxLevel", ns.DEBUG_LEVELS.WARN)
                            local mirrorLevelNames = {
                                [ns.DEBUG_LEVELS.ERROR] = "Error",
                                [ns.DEBUG_LEVELS.WARN] = "Warn",
                                [ns.DEBUG_LEVELS.INFO] = "Info",
                                [ns.DEBUG_LEVELS.DEBUG] = "Debug",
                                [ns.DEBUG_LEVELS.TRACE] = "Trace",
                            }
                            tinsert(status, format("  Session log mirror: |cff00ff00%s|r (through %s)",
                                mirrorOn and "On" or "Off",
                                mirrorLevelNames[mirrorMax] or tostring(mirrorMax)))
            ...
            -- Optional per-severity chat tab overrides (separate tree node from core logging).
            debugChannelsByLevelGroup = {
                type = "group",
            ...
            -- Performance Settings Group (before niche export testing — more commonly adjusted)
            ...
            -- Settings export testing (raw JSON) — niche; keep below performance
            settingsExportTestingGroup = {
                type = "group",
                name = L["settingsExportTestingGroup"] or "Settings Export (Testing)",
                desc = L["settingsExportTestingGroupDesc"] or "Options for testing settings export/import. When enabled, Export Settings produces raw JSON (unencoded, uncompressed) so you can inspect what is being exported.",
                order = 50,
                inline = true,
                args = {
                    exportSettingsRawForTesting = OptionsFactory:CreateToggle(
                        L["exportSettingsRawForTesting"] or "Export settings as raw JSON (unencoded)",
                        L["exportSettingsRawForTestingDesc"] or "When enabled, Export Settings will output raw JSON without compression or encoding. Use this to inspect the exported delta in testing. The resulting string cannot be re-imported; disable this for normal export.",
                        function() return self:GetSetting("global", "exportSettingsRawForTesting", false) end,
                        function(_, value) self:SetSetting("global", "exportSettingsRawForTesting", value) end,
                        { order = 1, width = "full" }
                    ),
                }
            },
...
-- Create module debug override list (single scrollable page; search applies to all modules).
-- Previously: name-heuristic buckets as non-inline Ace groups became a nested tree (one search scope per tab).
function OptionsManager:CreateModernModuleDebugOptions()
    ...
    args.moduleSearch = OptionsFactory:CreateInput(L["moduleSearch"] or "Search Modules", L["moduleSearchDesc"] or "Filter the list below (all modules on this page). Plain text, not patterns.",
        function()
            return self.moduleSearchTerm or ""
        end,
        function(info, value)
            self.moduleSearchTerm = value
    ...
    args.overridesDesc = {
        type = "description",
        name =
        "Per-module: when enabled, this toggle turns OFF debug output for that module (sets module namespace debugEnabled = false). ERROR lines still pass through DebugManager.",
        order = 11,
        fontSize = "medium",
        width = "full",
    }
    ...
    local moduleNames = {}
    for moduleName in NAG:IterateModules() do
        tinsert(moduleNames, moduleName)
    end
    sort(moduleNames)

    local term = strlower(self.moduleSearchTerm or "")
    local listArgs = {}
    local visOrder = 1
    ...
    if term ~= "" then
        ...
        for _, moduleName in ipairs(moduleNames) do
            if strfind(strlower(moduleName), term, 1, true) then
                anyMatch = true
                break
            end
        end
        if not anyMatch then
            listArgs.moduleSearchNoMatch = {
                type = "description",
                name = L["moduleSearchNoMatch"] or "No module names contain that text.",
                order = 0,
                fontSize = "medium",
                width = "full",
            }
    ...
    for _, moduleName in ipairs(moduleNames) do
        if term == "" or strfind(strlower(moduleName), term, 1, true) then
            listArgs[moduleName .. "Override"] = OptionsFactory:CreateToggle(
                moduleName,
                "Disable debug output for " .. moduleName .. " (non-ERROR severities)",
                function()
                ...
                function(_, value)
                    ...
                    if moduleDB and moduleDB.global then
                        moduleDB.global.debugEnabled = not value
                    end
                end,
                -- AceConfigDialog: "half"/"double"/default are *fixed px* (width_multiplier=170), not % of panel.
                -- "half" = 85px → Flow packs many per row on wide windows. Use relative width for real 2-col layout.
                { order = visOrder, width = "relative", relWidth = 0.5 }
            )
            visOrder = visOrder + 1
        end
    end

    args.moduleOverrideList = {
        type = "group",
        name = "",
        order = 20,
        inline = true,
