...
do
    ...
    if SchemaModifications and SchemaModifications.RegisterAPLValue then
        local includeDruid = { class_specific = true, classes = { "Druid" }, prepull_only = false, combat_only = false, spec_specific = false }
        SchemaModifications:RegisterAPLValue("IsFeralShredAssignmentEnabled", {}, { include_if = includeDruid, ui_label = "Is Feral Shred Assignment Enabled", submenu = { "druid" }, short_description = "True when feral Shred assignment is effectively enabled.", full_description = "Returns whether the dedicated feral Shred assignment is effectively enabled. Defaults to ON while grouped when unset." })
        SchemaModifications:RegisterAPLValue("FeralCanLikelyShred", {}, { include_if = includeDruid, ui_label = "Feral Can Likely Shred", submenu = { "druid" }, short_description = "True when Shred is likely usable (e.g. behind target).", full_description = "Returns true when Shred is likely usable. Heuristic: grouped, target not targeting you, not primary target." })
        SchemaModifications:RegisterAPLValue("ShouldRefreshMangleForFeral", {}, { include_if = includeDruid, ui_label = "Should Refresh Mangle For Feral", submenu = { "druid" }, short_description = "True when to refresh Mangle early for feral bleed damage.", full_description = "Returns true when another feral druid is in group and Mangle should be refreshed to protect bleed damage." })
        SchemaModifications:RegisterAPLValue("GetFeralManglePosition", {}, { include_if = includeDruid, ui_label = "Get Feral Mangle Position", submenu = { "druid" }, short_description = "Display position for Feral Mangle based on group composition.", full_description = "Returns BELOW when guardian or another feral is present, else PRIMARY." })
        SchemaModifications:RegisterAPLValue("ShouldMangle", {}, { include_if = includeDruid, ui_label = "Should Mangle", submenu = { "druid" }, short_description = "True if Feral should cast Mangle and position for display.", full_description = "Determines whether Feral should cast Mangle and where to show it. Priority-style rules based on TBC feral rotation logic." })
        SchemaModifications:RegisterAPLValue("ManglePosition", {}, { include_if = includeDruid, ui_label = "Mangle Position", submenu = { "druid" }, short_description = "Spell position for Mangle when ShouldMangle is true.", full_description = "Returns the position (e.g. PRIMARY, BELOW) for Mangle from ShouldMangle." })
        SchemaModifications:RegisterAPLValue("CastPowershiftPlaceholder", {}, { include_if = includeDruid, ui_label = "Cast Powershift Placeholder", submenu = { "druid" }, short_description = "Cast Cat Form placeholder and show Powershift overlay.", full_description = "Casts the Cat Form placeholder and shows a Powershift overlay. Returns true if placeholder was triggered." })
        SchemaModifications:RegisterAPLValue("GetFeralDebugSnapshot", {}, { include_if = includeDruid, ui_label = "Get Feral Debug Snapshot", submenu = { "druid" }, short_description = "Debug snapshot for TBC feral finisher/powershift gating.", full_description = "Returns a table of diagnostic fields for powershift, rip, and bite gates (mirrors Feral DPS rotation checks)." })
        SchemaModifications:RegisterAPLValue("CastMangleIfNeeded", {}, { include_if = includeDruid, ui_label = "Cast Mangle If Needed", submenu = { "druid" }, short_description = "Cast Mangle if rotation rules say we should.", full_description = "Casts Mangle with appropriate position when ShouldMangle returns true. Returns true if Mangle was triggered." })
    end
end
