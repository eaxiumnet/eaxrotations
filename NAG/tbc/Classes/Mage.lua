...
defaults.class.specSpellLocations = {
    [1] = { -- Arcane
        ...
        [116] = NAG.SPELL_POSITIONS.AOE,       -- Frostbolt (secondary filler, AOE slot)

        -- PRIMARY spells (default, not explicitly set)
        -- Arcane Blast (16067) - main nuke, max DPS filler
    },
    [2] = { -- Fire
        -- ABOVE spells (Buffs, maintenance)
...
local arcaneRotation = {
    ...
    -- AoE: Frostbolt when 2+ targets at range (Arcane Explosion requires melee)
    or (NAG:NumberTargets() >= 2 and NAG:CountEnemiesInRange(10) < 2 and NAG:SpellCanCast(116)) and
        NAG:Cast(116, nil, NAG.SPELL_POSITIONS.AOE)

    -- Main filler: Arcane Blast (max DPS, no mana conservation)
...
Core Mechanics:
- Arcane Mages are burst DPS casters. Always prioritize maximum DPS output.
- Arcane Blast stacks up to 3 times, increasing damage and reducing cast time.
- Arcane Blast is the main filler at all times (no mana conservation mode).
- Use Evocation in combat when mana is very low (<15%).

Rotation Priority (Single Target):
1. Maintain Mage Armor (mana regeneration).
2. If in combat and mana is very low, Evocation (bottom position).
...
6. Cast Arcane Blast as main filler (max DPS).
...
- Arcane Blast: Main nuke and filler - always use for max DPS.
- Frostbolt: AOE slot only (e.g. 2+ targets at range when Arcane Explosion not in melee).
- Mage Armor: Provides passive mana regeneration - always maintain it.

Movement:
- Fire Blast: Instant cast fire damage - use while moving.
- Minimize movement during cooldown windows to maximize Arcane Blast spam.

AoE Rotation:
- Arcane Explosion on 2+ targets when they are in melee range.
- Frostbolt (AOE slot) when 2+ targets at range (Arcane Explosion not in melee).
