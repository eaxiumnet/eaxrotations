...
--capture state machine output and call proper actions
-- In-combat CS/seal/judge/filler sequencing is intentionally owned by the
-- state-machine bridge (RetStatemachineAPLBridge) so we keep one source of truth.
or NAG:RetStatemachineAPLBridge()
...
or (
    -- Plain SoC fallback: if SoC is needed here, do not keep checking for Judge.
    (not NAG:RetTwistIsSoCActive())
    and (not NAG:RetTwistShouldPreferSoBForSwing(0.4, 3, 0.05, 0.2, 0.3))
    and (
        ...
        or (
            ...
            and (not NAG:RetTwistShouldBlockSoCSetupForCS(0.4, "next_swing_pre_div"))
            and NAG:RetTwistSoCSetupHold(0.4)
            and (NAG:RetTwistDebugBranchWinner("soc_hold", "soc_fallback_hold_no_judge") or true)
        )
    )
)
