...
do
    ...
    if SchemaModifications and SchemaModifications.RegisterAPLValue then
        local includeWarrior = { class_specific = true, classes = { "Warrior" }, prepull_only = false, combat_only = false, spec_specific = false }
        SchemaModifications:RegisterAPLValue("DualWieldWarriorShouldCheckWeapons", {}, { include_if = includeWarrior, ui_label = "Dual Wield Warrior Should Check Weapons", submenu = { "warrior" }, short_description = "True when dual-wield warrior should show weapon speed alert.", full_description = "Returns true when MH and OH have the same speed (within 0.1s), indicating a bad dual-wield setup that should show an alert." })
    end
end
