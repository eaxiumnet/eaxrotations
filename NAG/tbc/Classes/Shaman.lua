...
local elementalRotation = {
    ...
    or (NAG:UnitIsMoving() and NAG:SpellCanCast(8050) and
        ...
        NAG:Cast(8050)
    -- Fire Nova Totem (AoE) if enemies are in melee range
    or (NAG:CountEnemiesInRange(10) > 0 and (not NAG:IsTotemActiveResolved(1535)) and NAG:SpellCanCast(1535)) and
        NAG:Cast(1535)
    -- Magma Totem for sustained AoE when multiple enemies are in melee range
    or (NAG:CountEnemiesInRange(10) > 1 and (not NAG:IsTotemActiveResolved(1535)) and (not NAG:IsTotemActiveResolved(8187)) and
...
3. Drop Magma Totem while Fire Nova is on cooldown for sustained AoE.
...
local enhancementRotation = {
    ...
    -- Cast Fire Nova Totem if no Fire Elemental and no Fire Nova is active (any rank)
    or (not NAG:IsTotemActiveResolved(2894)) and NAG:CountEnemiesInRange(10) > 0 and
        ...
        (not NAG:IsFireNovaTotemActiveAnyRank()) and NAG:SpellCanCast(1535) and NAG:Cast(1535)
    -- Cast Magma Totem if no Fire Elemental and 2+ enemies in range (skip if Fire Nova active)
    or (not NAG:IsTotemActiveResolved(2894)) and NAG:CountEnemiesInRange(10) > 1 and
        (not (NAG:IsAssignmentEnabled("shaman_totem_twist_force") and NAG:WFNeedsRefresh() and NAG:CanCastWFTotem())) and
        (not NAG:IsFireNovaTotemActiveAnyRank()) and (not NAG:IsTotemActiveResolved(8187))
        and NAG:SpellCanCast(8187) and NAG:Cast(8187)
    -- Cast Magma Totem on single targets if enabled (skip if Fire Nova active)
    or (not NAG:IsTotemActiveResolved(2894)) and NAG:CountEnemiesInRange(10) <= 1 and
        (not (NAG:IsAssignmentEnabled("shaman_totem_twist_force") and NAG:WFNeedsRefresh() and NAG:CanCastWFTotem())) and
        (not NAG:IsFireNovaTotemActiveAnyRank()) and (not NAG:IsTotemActiveResolved(8187)) and
    ...
    or (not(NAG:IsTotemActiveResolved(2894)) and
        ...
        (NAG:FireTotemSlotNeedsFill() or
            ((not NAG:IsFireNovaTotemActiveAnyRank()) and (not NAG:IsTotemActiveResolved(8187)) and NAG:SearingNeedsRefresh()))
...
AoE Rotation:
1. Use Fire Nova Totem (AoE damage explosion)
2. Maintain Magma Totem (continues AoE damage after Fire Nova Totem explodes)
