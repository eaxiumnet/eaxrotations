        ...
        if NAG and NAG.modules and NAG.modules[moduleSelf] then
            ...
            if NAG and NAG.IsDevModeEnabled and NAG:IsDevModeEnabled() then
                if NAG.DebugManager and NAG.DebugManager.Warn then
                    NAG.DebugManager:Warn(NAG, "GetModuleTypeColor: Unknown module name passed: %s", tostring(moduleSelf))
                end
            end
            return defaultColor
        end
...
function ns.checkBase() -- Common.lua - base check (license/capital) without safeholder override
    ...
    if ns.Version then
        ...
        elseif (ns.Version.IsClassicEra and ns.Version:IsClassicEra()) or
            ...
            licenseValid = (ns[renderingProfileClassFlag] and (ns[renderingProfileCoreFlag] or ns[renderingProfileGodFlag])) or false
        end
    end
    if licenseValid or capital then flagOk = true end
    ...
    if flagOkMsg then
        ...
        if not flagOk and coreFlagForMsg and math.random() < 0.3 and
           ((ns.Version and ns.Version.IsMists and ns.Version:IsMists()) or
            (ns.Version and ((ns.Version.IsClassicEra and ns.Version:IsClassicEra()) or
                             (ns.Version.IsTBC and ns.Version:IsTBC()) or
                             (ns.Version.IsWrath and ns.Version:IsWrath()) or
                             (ns.Version.IsCata and ns.Version:IsCata())))) then
            flagOkMsg = false
            C_Timer.After(
                20,
        ...
        -- License flags are expansion-specific - check expansion before using flags
        if coreFlagForExpire and ns.d <= 3 and flagExpireMsg and
           ((ns.Version and ns.Version.IsMists and ns.Version:IsMists()) or isClassicExpansion) then
            flagExpireMsg = false
            --local n = 33 - ns.d
            C_Timer.After(
                20,
...
local function GetCapitalLicenseOverride()
    ...
    return (ns.IsTrainingDummy and ns.IsTrainingDummy()) or
...
function ns.check()
    if ns.RenderingProfileStabilizer and ns.RenderingProfileStabilizer.IsStabilized and ns.RenderingProfileStabilizer:IsStabilized() then
...
function ns.deleteRenderingCacheEntryLegacy(key)
    local keys = NAG.db.global.keys
    if not keys or not keys.classic then return end
...
function ns.deleteWrathKey(key)
    local keys = NAG.db.global.keys
    if not keys or not keys.wrath then return end
...
function ns.GetAllCacheStats()
    ...
    return {
        api = apiStats,
        aura = auraStats,
        totalHits = (apiStats.spellHits or 0) + (apiStats.itemHits or 0) + (apiStats.actionHits or 0) + (apiStats.macroHits or 0) + (auraStats.hits or 0),
        totalMisses = (apiStats.spellMisses or 0) + (apiStats.itemMisses or 0) + (apiStats.actionMisses or 0) + (apiStats.macroMisses or 0) + (auraStats.misses or 0)
    }
end
