...
local function normalizeSecondarySpellEntry(entry)
    ...
    if type(entry) == "table" and (entry.spellId or entry.itemId) then
        local normalized = normalizeActionIdentity(entry, {
            context = "secondary_normalize",
            itemOnlyWhenItem = true,
        })
        if normalized then
            normalized.queueSlot = tonumber(entry.queueSlot) or nil
            normalized.overlayText = type(entry.overlayText) == "string" and entry.overlayText or nil
        end
        return normalized
    end
    return nil
...
--- Schedules OG Ret shadow compare on the next engine tick so it does not consume rotation script time.
--- Coalesces to a single pending callback per frame when UpdateWithFunction runs multiple times.
--- @param nagSelf NAG
local function scheduleOGRetShadowCompareDeferred(nagSelf)
    if not nagSelf then
        return
    end
    if nagSelf._ogRetShadowCompareTimerScheduled then
        return
    end
    nagSelf._ogRetShadowCompareTimerScheduled = true
    if not (C_Timer and C_Timer.After) then
        nagSelf._ogRetShadowCompareTimerScheduled = nil
        local shadowSuccess, shadowErr = pcall(function()
            runOGRetShadowCompare(nagSelf)
        end)
        if not shadowSuccess and nagSelf.IsDevModeEnabled and nagSelf:IsDevModeEnabled() then
            nagSelf:Warn("UpdateWithFunction: OGRet shadow compare failed: %s", tostring(shadowErr))
        end
        return
    end
    C_Timer.After(0, function()
        ...
        if not shadowSuccess and nagSelf.IsDevModeEnabled and nagSelf:IsDevModeEnabled() then
            nagSelf:Warn("UpdateWithFunction: OGRet shadow compare failed (deferred): %s", tostring(shadowErr))
        end
    end)
end
...
local function getNextIconData()
    ...
    iconData.itemId = nil
    iconData.overlayResolveId = nil
    iconData.overlayDisplayId = nil
    iconData.queueSlot = nil
    iconData.overlayText = nil
    return iconData
end
...
--#region agent log: rotation session diagnostics (SessionDebugLog — /nag debuglog)
-- Verbose: logs every rotation outer cycle and every inner APL run (high volume). Default false; set true locally when diagnosing rotation timing.
-- When false: logs inner runs only on failure or when elapsed >= ROTATION_EXECUTION_WARN_MS (same threshold as chat Warn).
local ROTATION_SESSION_DEBUG_VERBOSE = false

--- Reused SessionDebugLog meta tables (stable pointers); filter in viewer with subsystem "rotation".
local LOG_META_ROTATION = {
    cycle_begin = { subsystem = "rotation", area = "cycle_begin" },
    outer = { subsystem = "rotation", area = "outer" },
    inner_function = { subsystem = "rotation", area = "execute_function" },
    inner_visitor = { subsystem = "rotation", area = "execute_visitor" },
    error = { subsystem = "rotation", area = "execute_error" },
}

local function rotationSessionAppend(location, message, data, meta)
    if ns.SessionDebugLogAppend then
        ns.SessionDebugLogAppend(location, message, data, meta)
    end
end

local function truncateForSessionLog(s, maxLen)
    if not s or type(s) ~= "string" then
        return ""
    end
    maxLen = maxLen or 480
    if #s <= maxLen then
        return s
    end
...
--- Inner APL timing (function or visitor path): always logged on error or slow run; every run if verbose.
--- @param pathKey string "function"|"visitor"
--- @param metaTable table
--- @param elapsedMs number
--- @param success boolean
--- @param errRaw any|nil pcall error object when success is false
local function logRotationInnerRun(pathKey, metaTable, elapsedMs, success, errRaw)
    local verbose = ROTATION_SESSION_DEBUG_VERBOSE
    if not verbose then
        if success and (not elapsedMs or elapsedMs < ROTATION_EXECUTION_WARN_MS) then
            return
        end
    end
    local diag = ensureRotationCycleDiagnostics()
    -- debugprofilestop deltas are fractional ms; do not floor (floor made most inner runs log as 0).
    local data = {
        path = pathKey,
        elapsedMs = elapsedMs or 0,
        ok = success,
        cycleId = tostring(diag.cycleId),
        phase = tostring(diag.cyclePhase or ""),
    }
    if not success and errRaw ~= nil then
        local errMsg = ExtractErrorMessage(errRaw)
        data.error = truncateForSessionLog(errMsg, 480)
        if errMsg and strfind(errMsg, "script ran too long") then
            data.scriptTimeout = true
        end
    end
    local loc = (pathKey == "visitor") and "Common.lua:UpdateWithVisitor" or "Common.lua:UpdateWithFunction"
    rotationSessionAppend(loc, "rotation_inner_run", data, metaTable)
end

--- Outer NAG:Rotation xpcall wrapper timing (ExecuteRotationBody).
--- @param elapsedMs number
--- @param xpcallOk boolean
--- @param result any second return from xpcall (rotationExecuted boolean when ok, or handler return when not)
local function logRotationOuterRun(elapsedMs, xpcallOk, result)
    local verbose = ROTATION_SESSION_DEBUG_VERBOSE
    if not verbose then
        if xpcallOk and (not elapsedMs or elapsedMs < ROTATION_EXECUTION_WARN_MS) then
            return
        end
    end
    local diag = ensureRotationCycleDiagnostics()
    ...
    if xpcallOk then
        data.rotationExecuted = result and true or false
    else
        data.handlerReturn = result
    end
    rotationSessionAppend("Common.lua:NAG:Rotation", "rotation_outer_run", data, LOG_META_ROTATION.outer)
end

--- Errors that propagate through xpcall(ExecuteRotationBody) (rare vs inner pcall).
--- @param useVisitorPattern boolean
--- @param err any
local function logRotationExecuteBodyError(useVisitorPattern, err)
    local errMsg = ExtractErrorMessage(err)
    local diag = ensureRotationCycleDiagnostics()
    local data = {
        useVisitorPattern = useVisitorPattern and true or false,
        cycleId = tostring(diag.cycleId),
        error = truncateForSessionLog(errMsg, 480),
    }
    if errMsg and strfind(errMsg, "script ran too long") then
        data.scriptTimeout = true
    end
    rotationSessionAppend("Common.lua:HandleRotationError", "rotation_execute_body_error", data, LOG_META_ROTATION.error)
end
--#endregion
...
local function HandleRotationError(self, useVisitorPattern, err)
    ...
    logRotationExecuteBodyError(useVisitorPattern, err)
...
do
    ...
    function NAG:Rotation()
        ...
        if ROTATION_SESSION_DEBUG_VERBOSE then
            local diagExec = ensureRotationCycleDiagnostics()
            rotationSessionAppend("Common.lua:NAG:Rotation", "rotation_execute_begin", {
                cycleId = tostring(diagExec.cycleId),
                visitor = useVisitorPattern and true or false,
                inCombat = UnitAffectingCombat("player") and true or false,
            }, LOG_META_ROTATION.cycle_begin)
        end
        rotationErrorContext.self = self
        rotationErrorContext.useVisitorPattern = useVisitorPattern
        local outerProfileStart = debugprofilestop()
        local success, result = xpcall(ExecuteRotationBody, RotationErrorHandler, self, useVisitorPattern)
        logRotationOuterRun(debugprofilestop() - outerProfileStart, success, result)
    ...
    function NAG:UpdateWithVisitor(rotationAST)
        ...
        logRotationInnerRun("visitor", LOG_META_ROTATION.inner_visitor, rotationElapsedMs, success, result)
    ...
    function NAG:UpdateWithFunction(rotation)
        ...
        local rotationElapsedMs = debugprofilestop() - rotationStartMs
        logRotationInnerRun("function", LOG_META_ROTATION.inner_function, rotationElapsedMs, success, foundSpell)
        if rotationElapsedMs > ROTATION_EXECUTION_WARN_MS then
            self:Warn("Rotation run exceeded %d ms (%.0f ms) - may cause timer lag or heartbeat stalls. Run /nag freeze and report if this persists.", ROTATION_EXECUTION_WARN_MS, rotationElapsedMs)
        end
        NAG.lastRotationExecutionMs = rotationElapsedMs
        ...
        -- Shadow-only OG Ret helper: compute + compare after rotation evaluation (deferred to next tick).
        -- This must never affect live recommendation selection.
        if success then
            scheduleOGRetShadowCompareDeferred(self)
        end
...
--- @return table Array of { norm, resolveId, displayId, preferItemFirst, position, queueSlot, overlayText }
        ...
        if norm then
            ...
            if resolveId and displayId then
                ...
                out[#out + 1] = {
                    ...
                    preferItemFirst = preferItemFirst,
                    position = position,
                    queueSlot = norm.queueSlot,
                    overlayText = norm.overlayText,
                }
            end
        end
    end
            ...
            if not norm or not resolveId or not id then
                ...
                if tinkerItemId and seenTinkerItemIds[tinkerItemId] then
                    ...
                    if icon then
                        ...
                        iconData.queueSlot = proj.queueSlot
                        iconData.overlayText = proj.overlayText
                        if norm.itemId then
...
--- Canonical shape: { spellId = number|nil, itemId = number|nil, position = string|nil, queueSlot = number|nil, overlayText = string|nil }.
--- @param entryOrId any
--- @param opts table|nil
--- @return table|nil
...
function NAG:NormalizeSecondaryAction(entryOrId, position, opts)
    ...
    if type(entryOrId) == "table" then
        normalized.queueSlot = tonumber(entryOrId.queueSlot) or nil
        normalized.overlayText = type(entryOrId.overlayText) == "string" and entryOrId.overlayText or nil
    end
    return normalized
end
