...
local SEAL_IDS = {
    ...
    348700, -- Seal of the Martyr (Alliance alt/client variant)
...
-- ============================ DRUID HEALING TOUCH TARGET (LOWEST HP) ============================
-- Same pattern as Monk Renewing Mist: detect lowest-HP friendly, show name on overlay, assign for macros.
-- Range filter uses LibRangeCheck-3.0 (40 yd for Healing Touch) only.

local HEALING_TOUCH_RANGE = 40 -- yards
local _htTargetCache = { ts = 0, name = nil }
local _htOverlayState = { expireAt = 0, lastUpdate = 0, lastName = nil }

--- Scans party/raid for the alive, connected friendly with the lowest HP percent within Healing Touch
--- range (LibRangeCheck). Cached for 0.85 seconds. Druid-only.
--- @usage local name = NAG:NextHealingTouchTargetName()
--- @return string|nil The unit name, or nil if none found or not a Druid
function NAG:NextHealingTouchTargetName()
    local now = GetTime()
    local cache = _htTargetCache
    if cache and (now - (cache.ts or 0)) < 0.85 then
        return cache.name
    end

    if NAG.CLASS_FILENAME ~= "DRUID" then
        _htTargetCache.name = nil
        _htTargetCache.ts = now
        return nil
    end

    local htChecker = RC and RC.GetFriendMaxChecker and select(1, RC:GetFriendMaxChecker(HEALING_TOUCH_RANGE))
    local function inRange(unit)
        return not htChecker or htChecker(unit)
    end
    ...
    if self:PlayerIsInRaid() then
        for i = 1, 40 do
            local unit = "raid" .. i
            local frac = getUnitHPFraction(unit)
            if frac and frac < lowestFrac and inRange(unit) then
                lowestFrac = frac
                lowestName = UnitName(unit)
            end
        end
    elseif self:PlayerIsInGroup() then
        local frac = getUnitHPFraction("player")
        if frac and frac < lowestFrac and inRange("player") then
            lowestFrac = frac
            lowestName = UnitName("player")
        end
        for i = 1, 4 do
    ...
    _htTargetCache.name = lowestName
    _htTargetCache.ts = now
    return lowestName
end

--- Shows an overlay on the NAG frame with the next Healing Touch target's name.
--- @usage NAG:ShowNextHealingTouchTargetOverlay()
--- @return nil
function NAG:ShowNextHealingTouchTargetOverlay()
    local name = self:NextHealingTouchTargetName()
    if not name then return end

    for _, anchor in ipairs(DisplayManager:GetActivePrimaryFrames()) do
        local overlay = OverlayManager:ShowOverlay(anchor, "pooling", nil, function()
            local exp = _htOverlayState and _htOverlayState.expireAt or 0
            return GetTime() <= exp
        end)
        if overlay then
            overlay:SetFrameStrata("HIGH")
        ...
        if overlay and overlay.text then
            ...
            local textOverlay = OverlayManager:ShowTextOverlay(anchor, name, nil, function()
                local exp = _htOverlayState and _htOverlayState.expireAt or 0
                return GetTime() <= exp
            end, { id = "NextHealingTouchTarget", fontSize = 9 })
            if textOverlay then
                textOverlay:SetFrameStrata("HIGH")
                textOverlay:SetFrameLevel(anchor:GetFrameLevel() + 10)
            end
...
--- Updates Healing Touch overlay when spell is castable; call when 5185 is already validated as ready.
--- @usage NAG:HealingTouchOverlayTickPrevalidated()
--- @return boolean Always returns false
function NAG:HealingTouchOverlayTickPrevalidated()
    if NAG.CLASS_FILENAME ~= "DRUID" then return false end

    local now = GetTime()
    if (now - (_htOverlayState.lastUpdate or 0)) <= 0.4 then
        _htOverlayState.expireAt = now + 0.5
    elseif _htOverlayState.expireAt <= now then
        _htOverlayState.expireAt = now + 0.5
    end
    _htOverlayState.lastUpdate = now

    local name = self:NextHealingTouchTargetName()
    if not name then return false end

    self.nextHealingTouchTargetName = name
    if name ~= _htOverlayState.lastName then
        self:ShowNextHealingTouchTargetOverlay()
        _htOverlayState.lastName = name
    end
    return false
end

--- Assigns the next Healing Touch target by name for downstream use (e.g., macros/UI).
--- Stores on self.nextHealingTouchTargetName.
--- @usage local name = NAG:AssignNextHealingTouchTarget()
--- @return string|nil The selected unit name or nil
function NAG:AssignNextHealingTouchTarget()
    local name = self:NextHealingTouchTargetName()
    self.nextHealingTouchTargetName = name
    return name
end
