...
--- Returns whether Unholy Strength (53365) was active when the last Blood Plague snapshot was captured.
--- @param targetUnit? string Optional unit token (defaults to "target")
--- @return boolean True if Unholy Strength was snapped
function NAG:SnapUnholyStrength(targetUnit)
    if not Snapshotter or not Snapshotter.GetSnapUnholyStrength then
        return false
    end
    return Snapshotter:GetSnapUnholyStrength(targetUnit or "target")
end
