...
local function getDisplayIconFrames()
    if not DisplayManager or type(DisplayManager) ~= "table" then
        return nil
    end

    local iconFrames = DisplayManager.iconFrames
    if not iconFrames or type(iconFrames) ~= "table" then
        return nil
    end
...
function NAG:IsSpellAtPosition(spellId, position)
    ...
    local iconFrames = getDisplayIconFrames()
    if not iconFrames then
        return false
    end

    local iconFrame = iconFrames[position]
...
function NAG:GetSpellPosition(spellId)
    ...
    for position, iconFrame in pairs(iconFrames) do
