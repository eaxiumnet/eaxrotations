...
-- Windfury Totem temporary weapon enchant IDs (from GetWeaponEnchantInfo when WF Totem buff is on player).
-- Used as fallback when UnitAura does not return the weapon buff (e.g. some client versions).
-- These are distinct from Windfury Weapon imbue IDs (WF_WEAPON_ENCHANT_IDS).
local WIND_FURY_TOTEM_ENCHANT_IDS = {
...
function NAG:FireTotemSlotNeedsFill()
    return (not NAG:IsTotemActiveResolved(2894))
        ...
        and (not NAG:IsFireNovaTotemActiveAnyRank())
...
--- Checks both UnitAura (spell IDs) and GetWeaponEnchantInfo (enchant IDs) since weapon buffs
--- may appear in either depending on client version.
--- @return boolean
function NAG:HasWindfuryTotemWeaponBuff()
    -- Primary: check aura by spell ID (works when weapon buff appears in UnitAura)
    for buffId, _ in pairs(WIND_FURY_TOTEM_BUFF_IDS) do
        if self:IsActiveAura(buffId, "player", false, true) then
            return true
        end
    end
    -- Fallback: check GetWeaponEnchantInfo for Windfury Totem enchant IDs
    -- (weapon buffs may not appear in UnitAura in some TBC/Classic clients)
    if GetWeaponEnchantInfo then
        local mhHas, _, _, mhEnchantId, ohHas, _, _, ohEnchantId = GetWeaponEnchantInfo()
        if (mhHas and mhEnchantId and WIND_FURY_TOTEM_ENCHANT_IDS[mhEnchantId]) or
           (ohHas and ohEnchantId and WIND_FURY_TOTEM_ENCHANT_IDS[ohEnchantId]) then
            return true
        end
    end
    return false
...
function NAG:AuraIsActive(id, sourceUnit, includeReactionTime, exactIdMatch)
    ...
    if not entity then
        -- Fallback for trinket proc auras and other spell IDs not yet in DataManager (e.g. 138760 FoF stacks)
