...
NAG:RegisterEvent("UNIT_SPELLCAST_SENT", function(_, unit, target, castGUID, spellIdOrName)
    ...
    elseif FILLER_IDS_AR[spellId] then
        -- HunterSteadyWeaveBar handles delayed spell-sent recomputes itself.
    end
end)
...
function NAG:ResolveEffectiveSpellId(spellId)
    ...
    if SpellbookManager and SpellbookManager.ResolveEffectiveSpellID then
        resolvedSpellId = SpellbookManager:ResolveEffectiveSpellID(spellId) or spellId
    end
...
local RET_TWIST_SOB_KNOWN_IDS = {
    31892, -- Seal of Blood (Horde)
    53720, -- Seal of the Martyr (Alliance legacy/client variant)
    348700, -- Seal of the Martyr (Alliance client variant)
}
local TBC_BLOOD_ELF_RACE_ID = PaladinTwistConstants.TBC_BLOOD_ELF_RACE_ID or 10
local TBC_TWIST_SEAL_BLOOD_ID = PaladinTwistConstants.TBC_TWIST_SEAL_BLOOD_ID or 31892
local TBC_TWIST_SEAL_MARTYR_ID = PaladinTwistConstants.TBC_TWIST_SEAL_MARTYR_ID or 53720
...
local retTwistSealKnowledge = {
    ...
    twistSealId = nil,
    soRSealId = nil,
    soCrusaderSealId = nil,
}

--- Direct spellbook/API knowledge check for Ret seal selection.
--- Avoids `NAG:SpellIsKnown()` so seal priming cannot recurse into DataManager registration.
--- @param spellId number|nil
--- @return boolean
local function retTwistIsSpellKnownLowLevel(spellId)
    if not spellId then
        return false
    end
    ...
    if SpellbookManager and SpellbookManager.ResolveEffectiveSpellID then
        effectiveSpellId = SpellbookManager:ResolveEffectiveSpellID(spellId) or spellId
    end

    if WoWAPI and WoWAPI.IsSpellKnown then
        if WoWAPI.IsSpellKnown(effectiveSpellId) or WoWAPI.IsSpellKnown(spellId) then
            return true
        end
    end

    if IsPlayerSpell then
        if IsPlayerSpell(effectiveSpellId) or IsPlayerSpell(spellId) then
            return true
        end
    end
...
--- @return number|nil
local function getRetTwistPlayerRaceId()
    local state = NAG and NAG.state and NAG.state.player
    local raceInfo = state and state.raceInfo or nil
    if raceInfo and raceInfo.id then
        return raceInfo.id
    end

    if NAG and NAG.RACEID then
        return NAG.RACEID
    end
...
--- @return number|nil
local function getCanonicalTBCPaladinTwistSealId()
    if not (Version and Version.IsTBC and Version:IsTBC()) then
        return nil
    end

    local playerClass = (NAG and NAG.CLASS_FILENAME) or (UnitClassBase and UnitClassBase("player")) or nil
    if playerClass ~= "PALADIN" then
        return nil
    end

    local raceId = getRetTwistPlayerRaceId()
    if raceId == TBC_BLOOD_ELF_RACE_ID then
        return TBC_TWIST_SEAL_BLOOD_ID
    end

    return TBC_TWIST_SEAL_MARTYR_ID
end

local function refreshRetTwistSealKnowledge(self)
    if retTwistSealKnowledge.refreshing then
        return
    end

    retTwistSealKnowledge.refreshing = true
    retTwistSealKnowledge.soCSealId = nil
    retTwistSealKnowledge.twistSealId = nil
    retTwistSealKnowledge.soRSealId = nil
    retTwistSealKnowledge.soCrusaderSealId = nil

    for i = #RET_TWIST_SOC_IDS, 1, -1 do
        local id = RET_TWIST_SOC_IDS[i]
        if id and retTwistIsSpellKnownLowLevel(id) then
            retTwistSealKnowledge.soCSealId = id
            break
        end
    end

    -- TBC paladins deterministically map to SoB for Blood Elves and SoM for everyone else.
    -- Keep the existing SoR fallback below if that canonical twist seal is not actually known yet.
    local canonicalTwistSealId = getCanonicalTBCPaladinTwistSealId()
    if canonicalTwistSealId and retTwistIsSpellKnownLowLevel(canonicalTwistSealId) then
        retTwistSealKnowledge.twistSealId = canonicalTwistSealId
    else
        for i = 1, #RET_TWIST_SOB_KNOWN_IDS do
            local id = RET_TWIST_SOB_KNOWN_IDS[i]
            if id and retTwistIsSpellKnownLowLevel(id) then
                retTwistSealKnowledge.twistSealId = id
                break
            end
        end
    end
    ...
    for i = #RET_TWIST_SOR_IDS, 1, -1 do
        ...
        if id and retTwistIsSpellKnownLowLevel(id) then
            retTwistSealKnowledge.soRSealId = id
            break
        end
    end
    ...
    for i = #RET_TWIST_SOCRUSADER_IDS, 1, -1 do
        ...
        if id and retTwistIsSpellKnownLowLevel(id) then
            retTwistSealKnowledge.soCrusaderSealId = id
            break
        end
    end

    retTwistSealKnowledge.dirty = false
    retTwistSealKnowledge.initialized = true
    retTwistSealKnowledge.refreshing = false
end

local function ensureRetTwistSealKnowledge(self)
    if not retTwistSealKnowledge.initialized then
        refreshRetTwistSealKnowledge(self)
        return
    end

    if retTwistSealKnowledge.dirty and (not self.InCombat or not self:InCombat()) then
        refreshRetTwistSealKnowledge(self)
    end
end

function NAG:InvalidateRetTwistSealKnowledge()
    retTwistSealKnowledge.dirty = true
end

function NAG:PrimeRetTwistSealKnowledge()
...
local function retTwistHasAnyKnownSpellId(self, spellIds)
    for i = 1, #spellIds do
        if retTwistIsSpellKnownLowLevel(spellIds[i]) then
            return true
        end
    end
    return false
...
--- @param spellIds number[]
--- @param preferredId number|nil
--- @return boolean
local function retTwistIsAnyKnownSpellActive(self, spellIds, preferredId)
    if preferredId and self:IsActive(preferredId) then
        return true
    end

    for i = 1, #spellIds do
        local id = spellIds[i]
        if id and id ~= preferredId and self:IsActive(id) then
            return true
        end
    end
...
function NAG:RetTwistGetSoCSealId()
    ensureRetTwistSealKnowledge(self)
    if retTwistSealKnowledge.soCSealId then
        return retTwistSealKnowledge.soCSealId
    end
...
function NAG:RetTwistIsSoCActive()
    ensureRetTwistSealKnowledge(self)
    local soCSealId = retTwistSealKnowledge.soCSealId
    return retTwistIsAnyKnownSpellActive(self, RET_TWIST_SOC_IDS, soCSealId)
end
...
function NAG:RetTwistIsSoBActive()
    ensureRetTwistSealKnowledge(self)
    local twistSealId = retTwistSealKnowledge.twistSealId
    if retTwistIsAnyKnownSpellActive(self, RET_TWIST_SOB_IDS, twistSealId) then
        return true
    end

    -- SoR is a valid twist seal: leveling fallback OR player choice (e.g. Prince).
    local soRSealId = retTwistSealKnowledge.soRSealId
    if retTwistIsAnyKnownSpellActive(self, RET_TWIST_SOR_IDS, soRSealId) then
        return true
    end
...
function NAG:RetTwistGetSoCrusaderSealId()
    ensureRetTwistSealKnowledge(self)
    if retTwistSealKnowledge.soCrusaderSealId then
        return retTwistSealKnowledge.soCrusaderSealId
    end
    -- Fallback to Rank 1
    return 20162
end
...
--- For TBC paladins this prefers the race-based canonical seal family:
--- Blood Elf -> Seal of Blood, otherwise Seal of the Martyr.
...
function NAG:RetTwistGetTwistSealId()
    ensureRetTwistSealKnowledge(self)
    if retTwistSealKnowledge.twistSealId then
        return retTwistSealKnowledge.twistSealId
    end

    -- Leveling fallback: if SoB/SoM are not learned yet, use Seal of Righteousness.
    if retTwistSealKnowledge.soRSealId then
        return retTwistSealKnowledge.soRSealId
    end
...
--- HOLD helper for state-machine-driven SoC timing.
--- Shows SoC with the standard Ret hold countdown/swipe using an explicit wait time.
--- @param holdSeconds number Required hold duration in seconds.
--- @return boolean
function NAG:RetTwistSoCHoldForSeconds(holdSeconds)
    if self:RetTwistIsSoCActive() then
        clearRetTwistSoCHoldVisuals(self)
        return false
    end
    ...
    registerRetTwistSoCHoldSwipe(self, soCId, holdSeconds, plannedEndAt)
    local overlayText = formatRetTwistHoldOverlay(holdSeconds)
    -- Use high display tolerance when on CD: state machine has decided SoC is next, so show it regardless of CD
    local displayTolerance = math.max(holdSeconds, 999)
    return self:CastSpell(soCId, displayTolerance, ns.SPELL_POSITIONS.PRIMARY, nil, overlayText)
end
...
function NAG:RetTwistJudgeHold(holdSeconds)
    ...
    -- Use high display tolerance when on CD: state machine has decided Judge is next, so show it regardless of CD
    local displayTolerance = math.max(holdSeconds, 999)
    return self:CastSpell(20271, displayTolerance, ns.SPELL_POSITIONS.PRIMARY, nil, overlayText)
end
...
function NAG:RetTwistJudgeSetupCanDisplay(maxToleranceSeconds, twistWindowSeconds, hysteresisSeconds, safetySeconds, fillerBufferSeconds)
    ...
    local committedJudgeDisplayGraceSeconds = 0.20
    local keepFallbackHoldVisuals = (self._retTwistJudgeHoldSeconds and self._retTwistJudgeHoldSeconds > 0)
        or (self.castOverlayTexts and type(self.castOverlayTexts[20271]) == "string"
            and string.find(self.castOverlayTexts[20271], "", 1, true) ~= nil)
    ...
    local committedJudgeDisplay = keepFallbackHoldVisuals
        or (self._retTwistJudgeHoldPhaseActive == true)
        or (gcdSnapshot and gcdSnapshot.canDisplay == true)
        or self:RetTwistJudgeSetupRecentlyDisplayable(committedJudgeDisplayGraceSeconds)
    ...
    elseif isHoldDisplay then
        local displayTolerance = castTolerance
        if committedJudgeDisplay then
            displayTolerance = displayTolerance + committedJudgeDisplayGraceSeconds
        end
        canDisplay = self:SpellCanCast(20271, displayTolerance)
    end
    if not canDisplay then
        ...
        if gcdLeft > 0 and self.IsDevModeEnabled and self:IsDevModeEnabled() then
            ...
            local judgePrimaryVisible = self.IsSpellAtPosition and self:IsSpellAtPosition(20271, ns.SPELL_POSITIONS.PRIMARY) or false
            local socAoeVisible = soCId and self.IsSpellAtPosition and self:IsSpellAtPosition(soCId, ns.SPELL_POSITIONS.AOE) or false
            print(string.format(
                "[JUDGE_HOLD_GATE][ver=%s] canDisplay=false gcdLeft=%.3f holdSeconds=%s reason=%s committed=%s judgePrimary=%s socAOE=%s keepFallback=%s recent=%s gcdSnap=%s",
                JUDGE_HOLD_LOG_VERSION,
                gcdLeft or 0,
                tostring(holdSeconds),
                blockReason,
                tostring(committedJudgeDisplay),
                tostring(judgePrimaryVisible),
                tostring(socAoeVisible),
                tostring(keepFallbackHoldVisuals),
                tostring(self:RetTwistJudgeSetupRecentlyDisplayable(committedJudgeDisplayGraceSeconds)),
                tostring(gcdSnapshot and gcdSnapshot.canDisplay == true)
            ))
        end
        if judgeHoldDebug and self:IsDevModeEnabled() and self._retTwistJudgeHoldPhaseActive then
...
--- @param options table|nil Optional display config (`centered`, `textColor`, `textSize`, `festerblight`).
---   When `festerblight` = { prefix, suffix }, renders prefix (normal) and suffix (red, 40% bigger) on two lines.
--- @return boolean Always returns false
function NAG:ShowCustomOverlay(text, options)
    ...
    local festerblight = options.festerblight

    -- For group display fallback: use full text when festerblight format
    if festerblight and type(festerblight) == "table" and festerblight.prefix and festerblight.suffix then
        self._customOverlayText = festerblight.prefix .. "\n" .. festerblight.suffix
    else
    ...
    if overlay then
        ...
        local suffixText = nil
        local suffixFontSize = fontSize

        if festerblight and type(festerblight) == "table" and festerblight.prefix and festerblight.suffix then
            displayText = festerblight.prefix
            suffixText = festerblight.suffix
            suffixFontSize = math.floor(fontSize * 1.4 + 0.5)
        end
        ...
        if suffixText then
            if not overlay.suffixText then
                overlay.suffixText = overlay:CreateFontString(nil, "OVERLAY")
            end
            local suffixSuccess = fontPath and overlay.suffixText:SetFont(fontPath, suffixFontSize, "OUTLINE")
            if not suffixSuccess then
                overlay.suffixText:SetFont("Fonts\\FRIZQT__.TTF", suffixFontSize, "OUTLINE")
            end
            overlay.suffixText:SetText(suffixText)
            overlay.suffixText:SetTextColor(1, 0, 0, 1)
            overlay.suffixText:SetAlpha(1.0)
            overlay.suffixText:ClearAllPoints()
            overlay.suffixText:SetPoint("TOP", overlay.text, "BOTTOM", 0, 0)
            overlay.suffixText:SetJustifyH("CENTER")
            overlay.suffixText:SetJustifyV("MIDDLE")
            overlay.suffixText:Show()
        elseif overlay.suffixText then
            overlay.suffixText:Hide()
        end
...
--- @param extraData table|nil Optional metadata for queueed secondaries (e.g. queueSlot, overlayText).
--- @return boolean True if the spell was already present; false if added or if spell/item is not usable.
--- @local Internal utility function for spell management
function NAG:AddSecondarySpell(ID, position, itemId, extraData)
    ...
    if type(extraData) == "table" then
        candidate.queueSlot = tonumber(extraData.queueSlot) or nil
        candidate.overlayText = type(extraData.overlayText) == "string" and extraData.overlayText or nil
    end
    ...
    for i = 1, #self.secondarySpells do
        ...
        if entry == nil then
            ...
            if match then
                local candidateQueueSlot = tonumber(candidate.queueSlot) or nil
                local existingQueueSlot = tonumber(norm.queueSlot) or nil
                if candidateQueueSlot or existingQueueSlot then
                    match = candidateQueueSlot == existingQueueSlot
                end
            end
            if match then
                if type(entry) == "table" then
                    ...
                    entry.queueSlot = candidate.queueSlot
                    entry.overlayText = candidate.overlayText
                else
...
function NAG:CastSpell(id, tolerance, position, hideWhileCasting, overlayText)
    ...
    local classModule = NAG:GetClassModule()
    local autoHideWhileCasting = false
    ...
    -- 1) cast/channel time, and 2) a cooldown longer than that cast time.
    -- This keeps repeat-cast spells visible while they are still effectively chain-castable
    -- (for example, when a buff removes or compresses the normal cooldown window).
    ...
    if hideWhileCasting == nil and type(id) == "number" then
        local castTime = self:SpellCastTime(id) or 0
        local hasCastTime = castTime > 0
    ...
    if hideWhileCasting then
        ...
        if (cId and cId == id) or (chId and chId == id) or
           ...
           (chName and spellName and chName == spellName) then
            -- For auto-hide on cooldown spells, keep showing the current cast if it is already
            -- predicted to be ready again by the next action window (e.g. MoP Elemental Ascendance).
            if autoHideWhileCasting and self:IsReadySpell(id, tolerance) then
                hideWhileCasting = false
            end
        end

        if hideWhileCasting and ((cId and cId == id) or (chId and chId == id) or
           (cName and spellName and cName == spellName) or
           (chName and spellName and chName == spellName)) then
...
local function ResolveArrowFrames(position)
    ...
    if ns.DisplayManager and ns.DisplayManager.GetFrameForPosition then
        local visibleFrame = ns.DisplayManager:GetFrameForPosition(resolvedPosition)
...
function NAG:CastPlaceholderWithArrow(id, position, orientation, durationSeconds)
    ...
    if type(id) == "number" then
        -- Defer arrow until after UpdateIcons runs; the target frame may be hidden
        -- until the display is updated with the new secondary spell.
        local spellId, pos, orient, dur = id, position, orientation, durationSeconds
        C_Timer.After(0, function()
            if NAG and NAG.ShowArrowOverlay then
                NAG:ShowArrowOverlay(spellId, pos, orient, dur, false)
            end
        end)
    end
    return result
...
function NAG:CastPlaceholder(spellId, tolerance, position, extraData)
    -- Direct parameter assignment (no polymorphic parsing)
    tolerance = tolerance or 0
    local overridePosition = position
    ...
    local queueSlot = nil
    local queueOverlayText = nil
    if type(extraData) == "table" then
        queueSlot = tonumber(extraData.queueSlot) or nil
        queueOverlayText = type(extraData.overlayText) == "string" and extraData.overlayText or nil
    end
    if type(spellId) == "table" then
        local normalizedInput = self:NormalizeSecondaryAction(spellId, overridePosition, {
            context = "cast_placeholder_input",
            itemOnlyWhenItem = true,
        })
        if not normalizedInput then
            return false
        end
        if queueSlot == nil then
            queueSlot = tonumber(normalizedInput.queueSlot) or nil
        end
        if not queueOverlayText or queueOverlayText == "" then
            queueOverlayText = normalizedInput.overlayText
        end
        spellId = normalizedInput.spellId or normalizedInput.itemId
    end
    ...
    if finalPosition and finalPosition ~= ns.SPELL_POSITIONS.PRIMARY then
        ...
        for i = 1, #self.secondarySpells do
            ...
            if entry == nil then
                ...
                local norm = NAG.NormalizeSecondarySpellEntry and NAG:NormalizeSecondarySpellEntry(entry) or nil
                local matched = norm and (norm.spellId == spellId or (norm.itemId and norm.itemId == spellId))
                if matched then
                    local existingQueueSlot = tonumber(norm.queueSlot) or nil
                    if queueSlot or existingQueueSlot then
                        matched = queueSlot == existingQueueSlot
                    end
                end
                if matched then
                    if type(entry) == "table" then
                        entry.position = finalPosition
                        entry.queueSlot = queueSlot
                        entry.overlayText = queueOverlayText
                    else
                        -- Defensive: malformed entry; replace with canonical shape
                        self.secondarySpells[i] = self:NormalizeSecondaryAction(spellId, finalPosition, {
                            context = "cast_placeholder_secondary",
                        })
                        if type(self.secondarySpells[i]) == "table" then
                            self.secondarySpells[i].queueSlot = queueSlot
                            self.secondarySpells[i].overlayText = queueOverlayText
                        end
                    end
                    alreadyPresent = true
                    break
        ...
        if not alreadyPresent then
            ...
            if normalizedSecondary then
                normalizedSecondary.queueSlot = queueSlot
                normalizedSecondary.overlayText = queueOverlayText
