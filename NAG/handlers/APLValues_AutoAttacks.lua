...
function NAG:RetTwistDebugBranchWinner(cluster, detail)
    ...
    local judgeId = 20271
    local nextIdentity = self.NormalizePrimaryAction and self:NormalizePrimaryAction(self.nextSpell) or self.nextSpell
    local nextResolveId = (self.GetActionResolveId and self:GetActionResolveId(nextIdentity)) or tonumber(nextIdentity) or 0
    local judgePrimary = self.IsSpellAtPosition and self:IsSpellAtPosition(judgeId, ns.SPELL_POSITIONS.PRIMARY) or false
    local socPrimary = soCId and self.IsSpellAtPosition and self:IsSpellAtPosition(soCId, ns.SPELL_POSITIONS.PRIMARY) or false
    local socAoe = soCId and self.IsSpellAtPosition and self:IsSpellAtPosition(soCId, ns.SPELL_POSITIONS.AOE) or false
    local gcdLeft = (self.GCDTimeToReady and self:GCDTimeToReady()) or 0
    if gcdLeft < 0 then
        gcdLeft = 0
    end
    local holdSeconds = tonumber(self._retTwistJudgeHoldSeconds or 0) or 0
    local recentJudgeDisplay = self.RetTwistJudgeSetupRecentlyDisplayable
        and self:RetTwistJudgeSetupRecentlyDisplayable(0.20)
        or false
    local gcdSnapshot = self._retTwistJudgeSetupGcdSnapshot
    local gcdSnapshotActive = gcdSnapshot and gcdSnapshot.canDisplay == true or false
    print(string.format(
        "[RET_BRANCH_WIN][ver=%s] cluster=%s detail=%s remTime=%.3f next=%s nextId=%s judgePrimary=%s socPrimary=%s socAOE=%s judgeHold=%.3f gcdLeft=%.3f judgeRecent=%s gcdSnap=%s",
        JUDGE_HOLD_LOG_VERSION,
        tag,
        info,
        remTime,
        tostring(nextIdentity),
        tostring(nextResolveId),
        tostring(judgePrimary),
        tostring(socPrimary),
        tostring(socAoe),
        holdSeconds,
        gcdLeft,
        tostring(recentJudgeDisplay),
        tostring(gcdSnapshotActive)
    ))
    return true
end
...
function NAG:RetTwistJudgeSetupWaitTolerance(maxToleranceSeconds, twistWindowSeconds, hysteresisSeconds, safetySeconds, fillerBufferSeconds)
    ...
    local gcdLeft = self:GCDTimeToReady() or 0
    if gcdLeft < 0 then gcdLeft = 0 end
    -- Diagnostic: Dev Mode only (same as Ret SM compute prints).
    local shouldPrint = (self.IsDevModeEnabled and self:IsDevModeEnabled())
        and (gcdLeft > 0)
...
function NAG:RetTwistTimeToTwistWindow(windowSeconds)
    ...
    local currentTwistWindowOpenIn = rawTimeToSwing - window
    if gcdLeft > rawTimeToSwing or gcdLeft > currentTwistWindowOpenIn then
        -- We can't use this swing's twist window (GCD misses swing or window-open); use next cycle.
        local swingSpeed = self:AutoSwingTime() or 0
        if swingSpeed <= 0 then
            return 0
        end
...
--- Returns true when at least one real Ret filler is currently valid under the same
--- final gates used by the rotation string.
--- This prevents Judge/SoC setup logic from deferring to a purely theoretical filler gap.
--- @param windowSeconds number|nil Default 0.4
--- @return boolean
function NAG:RetTwistHasActualFillerOpportunity(windowSeconds)
    local window = windowSeconds or 0.4
    local fillerGcd = self:GCDTimeValue() or 1.5
    if fillerGcd < 0 then
        fillerGcd = 0
    end

    if self:RetTwistShouldBlockFillersInSoBPreDiv(window, fillerGcd) then
        return false
    end

    local autoSwing = self:AutoSwingTime() or 0
    if autoSwing <= (1.75 * fillerGcd) then
        return false
    end

    local targetInMelee = self:TargetInMeleeRange(5)
    local targetCount = self:NumberTargets(5, false) or 0
    local remTime = self:RemainingTime() or 0
    local currentManaPercent = self:CurrentManaPercent() or 0
    local targetIsUndeadOrDemon = self:TargetMobType(NAG.Types.MobType.Undead) or self:TargetMobType(NAG.Types.MobType.Demon)
    local exoReady = self:SpellIsReady(879)
    local exoCanCast = self:SpellCanCast(879)
    local consCanCast = self:SpellCanCast(26573)
    local consBeforeTwist = consCanCast and self:RetTwistCanCastFillerBeforeTwist(26573, window, 0.1)
    local consBeforeCS = consCanCast and self:RetTwistCanCastFillerBeforeCS(26573, 35395, 0.1)
    local exoBeforeTwist = exoCanCast and self:RetTwistCanCastFillerBeforeTwist(879, window, 0.1)
    local exoBeforeCS = exoCanCast and self:RetTwistCanCastFillerBeforeCS(879, 35395, 0.1)
    local exoCompetesWithCons = (not targetIsUndeadOrDemon) or (not exoReady)
    local canCheckAoEConsecBeforeSoC = (self:TimeUntilSoC(window) == -1) or self:RetTwistCanCheckFillersBeforeSoC(window)

    local aoeConsecAllowed = canCheckAoEConsecBeforeSoC
        and (targetCount >= 2)
        and targetInMelee
        and exoCompetesWithCons
        and (currentManaPercent >= 0.3)
        and (remTime >= 5)
        and consBeforeTwist
        and consBeforeCS

    if aoeConsecAllowed then
        return true
    end

    local exoAllowed = targetIsUndeadOrDemon
        and exoCanCast
        and exoBeforeTwist
        and exoBeforeCS
    ...
    local stConsecAllowed = targetInMelee
        and exoCompetesWithCons
        and (currentManaPercent >= 0.3)
        and (remTime >= 5)
        and consBeforeTwist
...
function NAG:RetTwistShouldDeferJudgeSetupToFillers(windowSeconds, maxToleranceSeconds, hysteresisSeconds, safetySeconds, fillerBufferSeconds)
    ...
    -- Only defer if we DO have a Judge setup plan, the wait tolerance reports "0",
    -- and a real filler actually passes the final APL-style gates right now.
    ...
    return self:RetTwistHasActualFillerOpportunity(window)
end
...
function NAG:RetTwistSoCSetupHoldSeconds(windowSeconds)
    ...
    -- only allow filler checks to suppress SoC HOLD when a real filler is actually valid.
    local allowFillerGap = canRetTwistDeferNextSwingHoldForFillers(rawSwingNow, swingSpeed, window, self:GCDTimeValue() or 0)
    if allowFillerGap and self:RetTwistHasActualFillerOpportunity(window) then
