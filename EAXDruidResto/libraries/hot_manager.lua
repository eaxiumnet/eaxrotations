...
---Check if Lifebloom should be allowed to bloom for mana return
---@param unit game_object|nil The unit to check
---@param me game_object|nil The local player
---@param mana_threshold number|nil Mana percentage threshold (default: 30)
---@param time_threshold number|nil Seconds remaining threshold (default: 1.5)
---@return boolean True if should allow bloom
function hot_manager.should_bloom_lifebloom(unit, me, mana_threshold, time_threshold)
    if not unit or not me then
        return false
    end
    
    mana_threshold = mana_threshold or 30
    time_threshold = time_threshold or 1.5
    
    -- Check if Lifebloom has 3 stacks and is about to expire
    local success, buff_data = pcall(function()
        return buff_manager:get_buff_data(unit, LIFEBLOOM_ID)
    end)
    ...
    -- Must have 3 stacks for bloom to be worth it
    local stacks = buff_data.stacks or 0
    if stacks < 3 then
        return false
    end
    ...
    -- Check remaining duration
    local remaining = buff_data.remaining or 0
    if remaining > time_threshold then
        return false
    end
    
    -- Check mana threshold
    local my_mana_pct = 100
    if me.get_mana_percentage then
        local ok, pct = pcall(function() return me:get_mana_percentage() end)
        if ok then my_mana_pct = pct end
    end
    
    if my_mana_pct >= mana_threshold then
        return false
    end
...
---Convenience wrapper for should_bloom_lifebloom with default thresholds
---@param unit game_object|nil The unit to check
---@param me game_object|nil The local player
---@return boolean True if should allow bloom
function hot_manager.should_allow_bloom(unit, me)
    return hot_manager.should_bloom_lifebloom(unit, me, 30, 1.5)
end
