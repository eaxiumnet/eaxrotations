...
## v2.7.8 — Druid Low-Level Feral & Bear Fixes (July 16, 2026)

> **Release zip:** [EaxRotations-v2.7.8.zip](https://github.com/eaxiumnet/eaxrotations/releases/tag/v2.7.8)
> **Game:** The Burning Crusade Classic (2.5.5) + Classic Era (1.15)
> **Tested with:** 274 rotation suites + 18 leveling suites (all passing)

---

### Druid (Feral) — Low-Level Rotation Unlocked

If you are leveling a Feral Druid below level 50, your core abilities now fire correctly:

- **Shred** no longer waits for the Mangle (Cat) debuff before Mangle is even learned.
- **Rip** tracks all rank IDs, so low-level Rip casts no longer silently fail.
- **Ravage** and **Ferocious Bite** work as expected in the level 42 range.

### Druid (Bear) — Smarter Low-Level Rage Spending

- **Maul** now uses a level-scaled rage threshold when Mangle (Bear) is not yet learned, so low-level bears spend rage earlier instead of banking to 50.
- **Swipe** cleave works before Lacerate is available.
- **Demoralizing Roar** no longer wastes a GCD on a single mob that is about to die.
