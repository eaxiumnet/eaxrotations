...
-- Cache of spell name -> ID built from get_spells(), populated on first use
local _spell_name_cache = nil
local function get_spell_name_cache()
    if _spell_name_cache then return _spell_name_cache end
    _spell_name_cache = {}
    local ok, spells_map = pcall(function() return core.spell_book.get_spells() end)
    if ok and spells_map then
        for spell_id, _ in pairs(spells_map) do
            local ok2, name = pcall(function() return core.spell_book.get_spell_name(spell_id) end)
            if ok2 and name and name ~= "" then
                -- Store highest ID per name (highest rank)
                if not _spell_name_cache[name] or spell_id > _spell_name_cache[name] then
                    _spell_name_cache[name] = spell_id
                end
            end
        end
    end
    return _spell_name_cache
end

-- Known spell names for name-based fallback resolution
local SPELL_NAMES = {
    [5221]  = "Shred", [8992]  = "Shred", [8993]  = "Shred", [27001] = "Shred", [27002] = "Shred",
    [1822]  = "Rake",  [27003] = "Rake",
    [1079]  = "Rip",   [27008] = "Rip",
    [33982] = "Mangle (Cat)", [33876] = "Mangle (Cat)", [33983] = "Mangle (Cat)",
    [48565] = "Mangle (Cat)", [48566] = "Mangle (Cat)",
    [9846]  = "Tiger's Fury", [9845] = "Tiger's Fury", [6793] = "Tiger's Fury", [5217] = "Tiger's Fury",
    [27006] = "Pounce", [9005] = "Pounce", [9004] = "Pounce", [8998] = "Pounce",
    [9867]  = "Ravage", [9866] = "Ravage", [6785] = "Ravage", [3242] = "Ravage",
...
function utils.resolve_spell_id(rank_table)
    ...
    -- First pass: direct ID lookup
    for i = 1, #rank_table do
        if core.spell_book.is_spell_learned(rank_table[i]) then
            return rank_table[i]
        end
    end
    -- Second pass: name-based fallback for servers with non-standard spell IDs
    -- Find the canonical name for this spell table
    local canonical_name = SPELL_NAMES[rank_table[1]]
    if canonical_name then
        local cache = get_spell_name_cache()
        local found_id = cache[canonical_name]
...
-- Checks all paths including raw cache scan and name-based fallback.
function utils.is_prowling(unit, prowl_ids)
    if not unit or not unit:is_valid() then return false end
    -- Build lookup set for ID matching
    local id_set = {}
    if type(prowl_ids) == "table" then
        for _, id in ipairs(prowl_ids) do id_set[id] = true end
    elseif type(prowl_ids) == "number" then
        id_set[prowl_ids] = true
    end
    -- Fast path: direct data lookups via buff_manager
    ...
    if data and data.is_active then return true end
    -- Cache scan with name fallback
    local function scan_cache(cache)
        if not cache then return false end
        for _, entry in ipairs(cache) do
            if entry.is_active then
                if id_set[entry.buff_id] then return true end
                local n = entry.buff_name
                if n and (n == "Prowl" or n == "prowl") then return true end
            end
        end
        return false
    end
    local ok1, bc = pcall(function() return buff_manager:get_buff_cache(unit, 50) end)
    if ok1 and scan_cache(bc) then return true end
    local ok2, dc = pcall(function() return buff_manager:get_debuff_cache(unit, 50) end)
    if ok2 and scan_cache(dc) then return true end
    local ok3, ac = pcall(function() return buff_manager:get_aura_cache(unit, 50) end)
    if ok3 and scan_cache(ac) then return true end
    -- Last resort: scan unit's buff list directly via game object API
    local ok4, buffs = pcall(function() return unit:get_buffs() end)
    if ok4 and buffs then
        for _, b in ipairs(buffs) do
            if b then
                if id_set[b.id or b.spell_id or b.buff_id or 0] then return true end
                local n = b.name or b.buff_name
                if n and (n == "Prowl" or n == "prowl") then return true end
            end
        end
    end
...
-- Returns true if the unit is in cat form OR prowling.
-- Checks all buff/aura paths including name-based fallback.
function utils.is_in_cat_form(unit, spells_ref)
    ...
    local function scan_cache(cache)
        ...
        for _, entry in ipairs(cache) do
            if entry.is_active then
                if id_set[entry.buff_id] then return true end
                local n = entry.buff_name
                if n and (n == "Cat Form" or n == "cat form") then return true end
            end
        end
        return false
    end
    local ok1, bc = pcall(function() return buff_manager:get_buff_cache(unit, 50) end)
    if ok1 and scan_cache(bc) then return true end
    local ok2, ac = pcall(function() return buff_manager:get_aura_cache(unit, 50) end)
    if ok2 and scan_cache(ac) then return true end
    -- Last resort: scan unit's buff list directly via game object API
    local ok3, buffs = pcall(function() return unit:get_buffs() end)
    if ok3 and buffs then
        for _, b in ipairs(buffs) do
            if b then
                if id_set[b.id or b.spell_id or b.buff_id or 0] then return true end
                local n = b.name or b.buff_name
