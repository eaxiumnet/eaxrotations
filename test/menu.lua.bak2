...
-- Controls
-- key_checkbox does not expose :get_state() and has no readable toggle-state
-- getter — get_main_checkbox_state() is stuck at its initial value and never
-- changes, and get_keybind_state() only works while the key is physically held.
-- Solution: self-track the toggle state in Lua and flip it via a key listener.
do
    local _kc = core.menu.key_checkbox(7, false, true, true, 1, "eaxdruidferal_enabled")

    -- Seed from initial_toggle_state arg (false = off at load)
    local _toggle_state = false
    local _last_key_down = false  -- edge-detect so one press = one flip
        ...
        __index = function(_, k)
            if k == "get_state" then
                return function(self)
                    local ok, mode = pcall(function() return _kc:get_mode() end)
                    if ok and mode == 2 then return true end   -- always on
                    if ok and mode == 0 then
                        -- Hold: true only while key is physically down
                        local ok2, v = pcall(function() return _kc:get_keybind_state() end)
                        return ok2 and v or false
                    end
                    -- Toggle (mode 1): use our self-tracked state
                    -- Update it here each frame: detect key edge and flip
                    local ok2, held = pcall(function() return _kc:get_keybind_state() end)
                    if ok2 then
                        if held and not _last_key_down then
                            _toggle_state = not _toggle_state
                            pcall(function() _kc:set_toggle_state(_toggle_state) end)
                        end
                        _last_key_down = held
                    end
            ...
            elseif k == "set" then
                return function(self, val)
                    _toggle_state = val
                    pcall(function() _kc:set_toggle_state(val) end)
                end
            end
            -- Forward everything else (render, get_key_code, get_mode, etc.)
            local v = _kc[k]
            if type(v) == "function" then
                return function(self, ...) return _kc[k](_kc, ...) end
            end
            return v
        end,
    }
...
menu.toggle_key                          = menu.enabled  -- alias so existing code doesn't break
...
function menu.render()
    ...
    root_tree:render("  Eax's Druid Feral", function()
        ...
        bear_tree:render("  Bear Form (DPS)", function()
            ...
            ps.header("Defensive")
            menu.use_frenzied_regeneration:render("Frenzied Regen", "Emergency self-heal — auto-shifts to bear if needed")
            menu.frenzied_regeneration_hp_pct:render("Frenzied Regen HP %", "Trigger threshold")
            ps.header("Threat")
        ...
        shared_tree:render("  Shared Abilities", function()
            ps.header("Movement & Utility")
            menu.use_faerie_fire:render("Faerie Fire (Feral)", "Armor reduction — works in any form")
            menu.use_feral_charge:render("Feral Charge / Dash", "Bear charge or cat dash to close gaps")
            menu.use_travel_form:render("Travel Form", "Auto Travel Form when out of combat")
            menu.use_root_escape:render("Root Escape", "Drop form to break roots automatically")
            ps.header("Crowd Control")
            menu.use_bash:render("Bash", "Bear stun — also used as interrupt")
            menu.use_cyclone:render("Cyclone", "Last-resort CC — caster form, requires Bash on CD")
            ...
            ps.header("Defensive")
            menu.use_barkskin:render("Barkskin", "Emergency damage reduction — usable in any form")
            menu.barkskin_hp_pct:render("Barkskin HP %", "Use below this HP %")
            ps.header("Out of Combat")
            menu.use_innervate:render("Innervate", "Self-cast when low on mana out of combat")
            menu.innervate_mana_pct:render("Innervate Mana %", "Trigger threshold")
            menu.use_ooc_self_heal:render("OOC Self-Heal", "Healing Touch when HP is low out of combat")
            ...
            menu.use_remove_curse:render("Remove Curse", "Dispel curses from self or party")
            menu.use_natures_grasp:render("Nature's Grasp", "Auto-root attackers in caster form")
            ps.header("Advanced")
            menu.use_berserk:render("Berserk", "Burst CD — reduces Mangle CD in both cat and bear form")
            menu.use_powershift:render("Powershift", "Cat form energy reset (requires Wolfshead Helm or Natural Shapeshifter)")
        end)
