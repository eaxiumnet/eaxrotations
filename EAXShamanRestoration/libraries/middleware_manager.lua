-- EAXShamanRestoration Middleware Manager
-- Integrates libraries/middleware with Restoration Shaman-specific registrations
...
-- Spell IDs for TBC Restoration Shaman
local SHAMAN_SPELLS = {
    -- Racials
    BLOOD_FURY = 33697,      -- Orc spell power racial
...
--- Initialize and register all Restoration Shaman-specific middleware
...
function middleware_manager.initialize(menu)
    ...
    local use_mana_tide = (menu.use_mana_tide and menu.use_mana_tide:get_state()) or false
    ...
    -- Register Blood Fury (Orc offensive racial - spell power)
        ...
        local berserking_mw = {
            ...
            matches = function(ctx)
                ...
                if core.spell_book and core.spell_book.get_spell_cooldown then
                    local cd = core.spell_book.get_spell_cooldown(SHAMAN_SPELLS.BERSERKING)
                    if cd > 0 then return false end
                end
            ...
            execute = function(icon, ctx)
                if core.spell_queue and core.spell_queue.add then
                    core.spell_queue.add(SHAMAN_SPELLS.BERSERKING, "berserking", 80)
                    return true, "[MW] Berserking queued"
                elseif icon and icon.cast then
                    icon:cast(SHAMAN_SPELLS.BERSERKING)
    ...
    -- Register Mana Tide Totem middleware
    if use_mana_tide then
        local mana_tide_threshold = (menu.mana_tide_pct and menu.mana_tide_pct:get()) or 50
        local mana_tide_mw = {
            name = "Mana Tide Totem",
            priority = middleware.PRIORITY.RECOVERY_ITEMS,
            is_burst = false,
            is_defensive = true,
            is_gcd_gated = true,
            setting_key = "use_mana_tide",
            matches = function(ctx)
                if not ctx.in_combat then return false end
                if not ctx.me then return false end
                ...
                local mana_pct = ctx.me:get_power_percentage() or 100
                if mana_pct > mana_tide_threshold then return false end
                
                -- Check cooldown
                if core.spell_book and core.spell_book.get_spell_cooldown then
                    local cd = core.spell_book.get_spell_cooldown(SHAMAN_SPELLS.MANA_TIDE_TOTEM)
                    if cd > 0 then return false end
                end
            ...
            execute = function(icon, ctx)
                if core.spell_queue and core.spell_queue.add then
                    core.spell_queue.add(SHAMAN_SPELLS.MANA_TIDE_TOTEM, "mana_tide", 85)
                    return true, "[MW] Mana Tide queued"
                elseif icon and icon.cast then
                    icon:cast(SHAMAN_SPELLS.MANA_TIDE_TOTEM)
                    return true, "[MW] Mana Tide"
                end
                return false
            end,
        }
        middleware.register(mana_tide_mw)
    end
