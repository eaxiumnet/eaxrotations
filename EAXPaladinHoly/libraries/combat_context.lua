...
-- Combat context builder for EAXPaladinHoly
...
local function build(me)
    ...
    -- Mana (Paladin resource - critical for healers)
    ctx.mana = 0
    ctx.mana_pct = 0
        ...
        if ok_m then
            ...
            if me.get_max_power then
                ...
                if ok_max and max_m > 0 then
                    ctx.mana_pct = (m / max_m) * 100
                end
            end
        end
    end

    -- === THREAT DATA (for positioning awareness) ===
    ctx.has_aggro = false
    ctx.threat_status = 0
    ctx.threat_pct = 0
        ...
        if obj then
            ...
            if ok_valid and is_valid and ok_unit and is_unit and not (ok_dead and is_dead) then
                ...
                if ok_attack and can_attack then
                    ...
                    -- Squared distance check (5 yards = 25, 8 yards = 64, 10 yards = 100, 30 yards = 900, 40 yards = 1600)
                    local dist_sq = dist_squared(me, obj)
                    if dist_sq <= 25 then  -- 5 yards squared (melee range)
                        ctx.melee_enemies = ctx.melee_enemies + 1
                    elseif dist_sq <= 1600 then  -- 40 yards squared (healing range)
    ...
    -- === PARTY/RAID CONTEXT (Healing Priority) ===
    ctx.party_size = 0
    ctx.party_avg_hp = 100
    ctx.lowest_hp_member = nil
    ctx.lowest_hp_pct = 100
    ctx.tank_member = nil
    ctx.tank_hp_pct = 100
    ...
    for i = 1, 4 do
        ...
        if member then
            ...
            if ok_valid and is_valid then
                ...
                -- Get member health
                local ok_m_hp, m_hp = pcall(function() return member:get_health_percentage() end)
                if ok_m_hp then
                    total_hp = total_hp + m_hp
                    member_count = member_count + 1

                    -- Track lowest HP member
                    if m_hp < ctx.lowest_hp_pct then
                        ctx.lowest_hp_pct = m_hp
                        ctx.lowest_hp_member = member
                    end

                    -- Add to heal priority list if below 90%
                    if m_hp < 90 then
                        _heal_priority_list.n = _heal_priority_list.n + 1
                        _heal_priority_list[_heal_priority_list.n] = {
                            unit = member,
                            hp_pct = m_hp,
                            priority = 100 - m_hp  -- Lower HP = higher priority
                        }
                    end
                end
            end
    ...
    ctx.heal_priority_list = _heal_priority_list

    -- Calculate average party HP
    if member_count > 0 then
        ctx.party_avg_hp = total_hp / member_count
    end

    -- === HEALING PRIORITY ===
    ctx.healing_priority = 0  -- 0 = no healing needed, 100 = critical
    if ctx.lowest_hp_pct < 30 then
        ctx.healing_priority = 100  -- Critical
    elseif ctx.lowest_hp_pct < 50 then
        ctx.healing_priority = 75   -- High
    elseif ctx.lowest_hp_pct < 70 then
        ctx.healing_priority = 50   -- Medium
    elseif ctx.lowest_hp_pct < 90 then
        ctx.healing_priority = 25   -- Low
    end
