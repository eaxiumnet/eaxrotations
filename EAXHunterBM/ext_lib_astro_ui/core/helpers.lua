...
local function menu_slider_int(min_value, max_value, default_value, id)
    -- Try simple_ui first
    local sui = get_simple_ui()
    if sui and sui.menu and sui.menu.create_slider then
        return sui.menu.create_slider(min_value, max_value, default_value, {
            slider_type = "int",
            id = id
        })
    end
    -- Fall back to core.menu
...
local function menu_checkbox(default_value, id)
    -- Try simple_ui first
    local sui = get_simple_ui()
    if sui and sui.menu and sui.menu.create_checkbox then
        return sui.menu.create_checkbox(default_value, { id = id })
    end
    -- Fall back to core.menu
...
local function menu_combobox(default_value, id)
    -- Try simple_ui first
    local sui = get_simple_ui()
    if sui and sui.menu and sui.menu.create_combobox then
        return sui.menu.create_combobox(default_value, { id = id })
    end
    -- Fall back to core.menu
