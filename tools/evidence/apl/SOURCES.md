...
- **SoD era (2026-08-14): no wowsims APL fixtures by design.** wowsims has
  no Season-of-Discovery support (no rune meta), so `test_apl_conformance.lua`
  cannot pin SoD orders against upstream APLs. SoD rotations are designed
  from the SoD rune meta and validated by (a) the SoD spell-ID audit tier
  (`SOD_RUNE_IDS` + rune-id extraction in run_sylvanas_audit_tests.lua),
  (b) the SoD behavioral battery (never=0), and (c) the generated action-map
  evidence (`.omo/evidence`, task-1 map via
  EaxRotations/tools/generate_sod_task1_action_map.lua — gitignored scratch,
