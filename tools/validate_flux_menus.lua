...
for _, spec in ipairs(flux_specs) do
    ...
    -- Check 2: Old element definition pattern (missing elements array)
    -- WRONG: { element = menu.x, label = "X" } directly in the call
    -- CORRECT: { elements = { { element = menu.x, label = "X" } } }
    -- This is harder to detect with regex, but we can look for common mistakes
    
    -- Check 3: Missing elements array in slider_list calls
    for label, elements in content:gmatch("t:slider_list%(%{[^}]*label%s*=%s*([^,}]+)[^}]*elements%s*=%s*(%{") do
        -- Extract the elements array and verify it's not empty
        local elements_section = content:match("t:slider_list%(%{[^}]*elements%s*=%s*(%b{})")
        if elements_section and elements_section:match("elements%s*=%s*%{s*}") then
            table.insert(issues, spec .. ": slider_list has empty elements array")
        end
    end
    
    -- Check 4: Check for on_render callbacks
...
local sample_specs = {flux_specs[1], flux_specs[8], flux_specs[15], flux_specs[22]}
for _, spec in ipairs(sample_specs) do
    local menu_path = spec .. "/libraries/menu.lua"
    local cmd = "luac -p " .. menu_path .. " 2>&1"
    local handle = io.popen(cmd)
    local result = handle:read("*a")
    handle:close()
    
    if result and #result > 0 then
        print("✗ " .. spec .. "/libraries/menu.lua: " .. result)
    else
        print("✓ " .. spec .. "/libraries/menu.lua: syntax OK")
    end
end
