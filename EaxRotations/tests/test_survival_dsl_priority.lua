...
-- Load the survival spec
local surv = dofile("EaxRotations/classes/hunter/survival_sylvanas.lua")
local strategies = surv.strategies

-- ============================================================================
-- Priority order verification
-- ============================================================================
local dsl_names = { MendPet = true, HuntersMark = true, KillCommand = true,
                    AspectOfTheViper = true, FreezingTrap = true, FeignDeath = true }
local found = {}
...
assert_false(strategies[idx_kc].matches(make_ctx(), make_state({ in_combat = false })),
...
-- ============================================================================
-- AspectOfTheViper: not active, mana low
-- ============================================================================
local idx_viper = found.AspectOfTheViper
assert_true(strategies[idx_viper].matches(make_ctx(), make_state({ mana_pct = 5 })),
    "AspectOfTheViper matches when mana low and aspect missing")
assert_false(strategies[idx_viper].matches(make_ctx(), make_state({ has_aspect_viper = true, mana_pct = 5 })),
    "AspectOfTheViper skips when already active")
assert_false(strategies[idx_viper].matches(make_ctx(), make_state({ mana_pct = 10 })),
    "AspectOfTheViper skips when mana above threshold")
...
-- Summary
-- ============================================================================
print(string.format("test_survival_dsl_priority: %d passed, %d failed", _pass, _fail))
if _fail > 0 then
    os.exit(1)
end
print("PASS test_survival_dsl_priority")
