...
-- =========================================================================
local __eax_file = "tests/test_survival_concussive_misdirection.lua"
...
-- Documents which FrostByte features exist vs are missing in the rotation.
...
assert_true(strategy_names["RevivePet"], "RevivePet should be present")	-- Previously missing features (now implemented)
	assert_true(strategy_names["ConcussiveShot"], "ConcussiveShot should be present - FrostByte feature: kiting utility")
	assert_true(strategy_names["Misdirection"], "Misdirection should be present - FrostByte feature: threat management")		-- Previously missing FrostByte gaps (now implemented)
		assert_true(strategy_names["Volley"], "Volley should be present - FrostByte feature: AoE")
		assert_true(strategy_names["ScorpidSting"], "ScorpidSting should be present - FrostByte feature: debuff mode")
		assert_true(strategy_names["RaptorStrike"], "RaptorStrike should be present - FrostByte feature: melee weaving")
		assert_true(strategy_names["WingClip"], "WingClip should be present - FrostByte feature: slow")
		
		print("PASS test_survival_concussive_misdirection (gap audit: " .. #strategies .. " strategies present, 6 FrostByte gaps closed)")
