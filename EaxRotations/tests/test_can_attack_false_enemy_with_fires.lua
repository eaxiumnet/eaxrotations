-- Readability notes:
--   What: regression test for hostile validation when can_attack is stale/false.
--   When: selected target is hostile but one object method disagrees.
--   Why: a single negative API answer must not suppress stronger hostile signals.
...
assert(dispatcher.on_rotation_update() == true, "enemy_with should override stale can_attack=false")
assert(#casts > 0 and casts[1].unit == target, "cast should use selected hostile target")

print("PASS test_can_attack_false_enemy_with_fires")
