-- test_marksmanship_wotlk_dsl_priority.lua — WotLK marksmanship hunter DSL priority + match/no-match gate tests
-- Validates the 7 declarative strategies in the DSL_DEFS table.
    ...
    safe_state = function(tbl)
        local mt = {
            __index = function(_, key)
                local defaults = { hp=100, mana_pct=100, target_hp=100, enemy_count=0, mark_remains=0, serpent_remains=0 }
...
local ok, mod = pcall(require, "EaxRotations.classes.hunter.marksmanship_wotlk")
if not ok or not mod then
    error("Failed to load marksmanship_wotlk module: " .. tostring(ok))
end
...
-- ChimeraShot: unconditional (always matches)
tests.test_ChimeraShot_always_matches = test_match("ChimeraShot", {}, true)

-- AimedShot: unconditional (always matches)
tests.test_AimedShot_always_matches = test_match("AimedShot", {}, true)
