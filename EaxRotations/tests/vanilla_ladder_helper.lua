-- vanilla_ladder_helper.lua — shared learned-spell mock for Classic 1–60 ladder tests.
-- WHAT:  level-aware spell_ready/exists + class NS stubs; capture get_state via registry.
-- WHEN:  required by test_vanilla_spell_ladders.lua (and future class suites).
...
--- Sourced from classes/*/class_sylvanas.lua `levels` mins (Classic-relevant).
M.LEARN = {
    -- Hunter
    ...
    SteadyShot = 62, KillCommand = 66, -- TBC — must stay unlearned in Classic
    -- Warrior
    ...
    DefensiveStance = 10, BattleStance = 1, Devastate = 50, -- talent; rare Vanilla
    VictoryRush = 62, CommandingShout = 68, -- TBC
    -- Warlock
    ...
    Hellfire = 30, FelArmor = 62, UnstableAffliction = 50, -- UA talent / TBC
    ...
    ArcaneMissiles = 8, ArcaneBlast = 64, IceLance = 66, -- TBC
    -- Rogue
    ...
    Stormstrike = 40, -- listed in Classic tables; soft-gate if unlearned
    WaterShield = 62, EarthShield = 50, Bloodlust = 70, WrathOfAirTotem = 64,
    -- Priest
    Smite = 1, ShadowWordPain = 4, MindBlast = 10, MindFlay = 20,
    FlashHeal = 20, GreaterHeal = 40, Renew = 8, PowerWordShield = 6,
    VampiricTouch = 50, Shadowfiend = 66, ShadowWordDeath = 62, CircleOfHealing = 50,
    -- Paladin
    ...
    Mangle = 50, Lacerate = 66, Lifebloom = 64, -- TBC / late
}
...
--- Build spell table: every LEARN key + extras as named stub actions.
...
--- Create NS + package stubs; capture last registry registration.
        ...
        import_helpers = function(...)
            ...
            for i = 1, n do
                ...
                elseif key == "has_player_buff" then
                    ...
                    -- Prefer permissive defaults so combat fillers can match
                    out[i] = function() return true end
                end
            end
            return unpack(out)
...
function M.load_module(path, opts)
    ...
    if folder then
        opts = opts or {}
        opts.class_folder = folder
        NS, capture, level = M.setup({ level = opts.level or level, class_folder = folder })
    end
...
--- State proxy: missing *_ready → true (learned path); common combat fields filled.
...
--- Return true if any strategy in names matches under context/state.
...
--- Any strategy matches (fallback when name list misses aliases).
function M.any_strategy_matches(strategies, context, state, skip_names)
    ...
    for i = 1, #strategies do
        ...
        if s and s.matches and n and not skip[n] then
            -- Skip pure OOC/consumable noise for combat ladder
            if not n:match("Potion") and not n:match("Healthstone") and n ~= "Wand" then
                local ok, res = pcall(s.matches, context, state)
                if ok and res then return true, n end
            end
        end
    end
    -- Allow Wand as last resort for casters
    for i = 1, #strategies do
        local s = strategies[i]
        if s and s.name == "Wand" and s.matches then
            local ok, res = pcall(s.matches, context, state)
            if ok and res then return true, "Wand" end
        end
    end
    return false, nil
end

--- Build combat context for ladder tests.
function M.context(level, extra)
    local ctx = {
        ...
        is_group = false,
        is_solo = true,
        stance = 3, -- cat stance constant used by some modules
    }
    if extra then
