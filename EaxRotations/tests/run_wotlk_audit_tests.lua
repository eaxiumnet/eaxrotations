...
-- WHAT:  Scans WotLK rotation files for spell IDs absent from the lexxer.org WotLK index.
-- WHEN:  Run manually or in CI before releases.
-- WHY:   No WotLK DBC pipeline exists; this catches TBC/Vanilla IDs copied into WotLK files.
    ...
    for line in content:gmatch("[^\r\n]+") do
        ...
        if not is_comment_line(line) then
            ...
            for _, id in ipairs(ids) do
                if not valid_wotlk_ids[id] then
                    local kind = "INVALID"
                    if valid_tbc_ids[id] then
                        kind = "TBC_ID_IN_WOTLK"
                    elseif valid_vanilla_ids[id] then
                        kind = "VANILLA_ID_IN_WOTLK"
                    end
                    hits[#hits + 1] = {
                        line = line_no,
                        id = id,
...
print("=============================================================================")
print("  WOTLK SPELL ID AUDIT (lexxer.org existence check)")
print("  Source: lexxer.org/api/v1/spells/{id}?game=wotlk")
...
for _, class in ipairs({
    ...
    for _, suffix in ipairs({
        ...
        "arms", "fury", "protection", "leveling",
    }) do
        WOTLK_FILES[#WOTLK_FILES + 1] = "classes/" .. class .. "/" .. suffix .. "_wotlk.lua"
    end
end
...
print("  All wotlk files clean — every spell ID exists in lexxer.org WotLK data.")
os.exit(0)
