...
-- Throttle for object scanning to avoid per-frame lag in crowded areas
local _last_scan_time = 0
local _SCAN_INTERVAL = 0.25  -- seconds between get_all_objects() calls

-- Periodic cleanup of expired blacklist entries
local _last_blacklist_cleanup = 0
local _BLACKLIST_CLEANUP_INTERVAL = 60
...
function M.find_corpses(me, me_pos, range)
    ...
    -- Throttle: skip scan if interval hasn't elapsed
    if now - _last_scan_time < _SCAN_INTERVAL then
        return _nearby_corpses, _nearby_corpses.n
    end
    _last_scan_time = now

    -- Periodic cleanup of expired blacklist entries
    if now - _last_blacklist_cleanup >= _BLACKLIST_CLEANUP_INTERVAL then
        _last_blacklist_cleanup = now
        for guid, until_time in pairs(_corpse_blacklist) do
            if now >= until_time then
                _corpse_blacklist[guid] = nil
            end
        end
    end
