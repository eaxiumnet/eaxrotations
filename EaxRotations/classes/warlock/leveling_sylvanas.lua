...
local function healthstone_matches(context, state)
    ...
    -- Do not recreate if we already have one (prevents repeated CreateHealthstone spam)
    if NS.has_item then
        local hs_ids = { 22105, 22104, 22103, 19013, 19012, 19011, 19010, 19009, 19008, 19007, 19006, 19005, 19004, 5510, 5509, 5511, 5512 }
        for _, id in ipairs(hs_ids) do
            if NS.has_item(id) then return false end
        end
    end
    -- Need shard
    local has_shard = NS.has_item and NS.has_item(6265)
    if not has_shard and NS.core and NS.core.inventory and NS.core.inventory.get_item_count then
        local ok, cnt = pcall(NS.core.inventory.get_item_count, 6265)
        has_shard = ok and (cnt or 0) > 0
    end
    if not has_shard then return false end
    return state.healthstone_ready == true
