-- leveling_sylvanas -- warrior leveling_sylvanas rotation for TBC Anniversary (2.5.5).
...
function warrior_leveling.build_state(context)
    ...
    state.use_battle_shout = spec_kit.setting_bool(context, "use_battle_shout", true)
...
local battle_shout_matches = function(_, state)
    ...
    if not state.use_self_buffs or not state.use_battle_shout then return false end
...
local sunder_armor_matches = function(context, state)
    ...
    local me = L.get_player()
    if not me then return false end
    local ok_rage, rage = pcall(function() return me:get_power(NS.POWER_RAGE or 1) end)
