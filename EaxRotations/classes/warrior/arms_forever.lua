...
-- baseline cannot load, fail loudly — a silently missing "arms" playstyle is
-- worse than a hard error.
-- ---------------------------------------------------------------------------
local registry = NS.rotation_registry
if not registry or type(registry.register) ~= "function" then
    error("[FOREVER] warrior arms delta: rotation_registry unavailable", 0)
end
...
package.loaded["classes/warrior/arms_vanilla"] = nil
local baseline_ok, baseline_result = pcall(require, "classes/warrior/arms_vanilla")
registry.register = original_register
if not baseline_ok or type(baseline) ~= "table" or type(baseline.strategies) ~= "table" then
    error("[FOREVER] warrior arms delta: baseline load failed: " .. tostring(baseline_result), 0)
end
