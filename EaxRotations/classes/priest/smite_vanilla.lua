-- ============================================================================
-- Priest Smite DPS Rotation (Classic Vanilla)
-- Holy DPS with Shadow utility via SW:P
-- Holy Fire Weave optimization
-- ============================================================================
local _G = _G
...
-- Debuff / Buff IDs (Classic only — no TBC spell IDs >= 27000)
...
-- UnavailableClassic placeholders for TBC-only abilities
local UnavailableClassicPriestSwd = SPELLS.UnavailableClassicPriestSwd or nil  -- Shadow Word: Death (TBC)
local UnavailableClassicPriestSurge = SPELLS.UnavailableClassicPriestSurge or nil  -- Surge of Light (TBC talent)
...
local function build_smite_state(context)
    ...
    -- Mana conservation tiers: <30% downrank, <15% HF only, <5% wand only
    ...
    smite_state.in_weave_window = smite_state.swp_active
        and smite_state.swp_remaining > SMITE_CAST_BASE
        and smite_state.swp_remaining < HF_CAST_BASE
...
-- ============================================================================
-- STRATEGIES (Classic Vanilla — no TBC abilities)
-- ============================================================================
local strategies = {
    -- [0.1] Inner Fire maintenance
    {
        name = "InnerFire",
    ...
    -- [0.2] Solo/leveling/PvP self shield
    {
    ...
    -- [0.3] Renew for solo/PvP sustain
    {
        name = "SoloRenew",
        matches = function(context, state)
            if context.player_control_locked then return false end
    ...
    -- [1] Holy Fire (highest DPS priority)
    {
        name = "HolyFire",
        matches = function(context, state)
            if not context.in_combat then return false end
    ...
    -- [2] Shadow Word: Pain (maintain, mana-gated)
    {
        name = "ShadowWordPain",
        matches = function(context, state)
            if not context.in_combat then return false end
    ...
    -- [7] Mind Blast (optional, threat-gated, mana-gated)
    {
        name = "MindBlast",
        matches = function(context, state)
            if not context.in_combat then return false end
...
NS.rotation_registry:register("smite", strategies, {
    ...
    format_context_log = function(context, state)
        return format(
            "swp=%.1f hf=%s mb=%s dp=%.1f IF=%s ifRem=%.0f hp=%.0f mana=%.0f low=%s emerg=%s threat=%s",
...
NS.log("Smite priest rotation registered (Classic Vanilla)")
return strategies
