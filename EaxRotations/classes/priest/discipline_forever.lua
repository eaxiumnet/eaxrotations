...
-- baseline cannot load, fail loudly — a silently missing "discipline"
...
if not registry or type(registry.register) ~= "function" then
    error("[FOREVER] priest discipline delta: rotation_registry unavailable", 0)
end
...
package.loaded["classes/priest/discipline_vanilla"] = nil
local baseline_ok, baseline_result = pcall(require, "classes/priest/discipline_vanilla")
registry.register = original_register
if not baseline_ok or type(baseline) ~= "table" or type(baseline.strategies) ~= "table" then
    error("[FOREVER] priest discipline delta: baseline load failed: " .. tostring(baseline_result), 0)
end
