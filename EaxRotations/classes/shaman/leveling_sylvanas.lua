-- leveling_sylvanas -- shaman leveling_sylvanas rotation for TBC Anniversary (2.5.5).
...
local earth_shock_dps_matches = function(context, state)
    ...
    if state.default_shock == "earth" then return true end

    -- Fallback: if the preferred shock is not ready, use Earth Shock
    if state.default_shock == "flame" and not state.flame_shock_ready then return true end
    if state.default_shock == "frost" and not state.frost_shock_ready then return true end
