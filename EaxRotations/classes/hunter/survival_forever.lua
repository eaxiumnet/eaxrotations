...
-- baseline cannot load, fail loudly — a silently missing "survival"
...
if not registry or type(registry.register) ~= "function" then
    error("[FOREVER] hunter survival delta: rotation_registry unavailable", 0)
end
...
package.loaded["classes/hunter/survival_vanilla"] = nil
local baseline_ok, baseline_result = pcall(require, "classes/hunter/survival_vanilla")
registry.register = original_register
if not baseline_ok or type(baseline) ~= "table" or type(baseline.strategies) ~= "table" then
    error("[FOREVER] hunter survival delta: baseline load failed: " .. tostring(baseline_result), 0)
end
