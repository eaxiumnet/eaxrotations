-- Readability notes:
--   What: Hunter Marksmanship priority list.
...
local ACTIONS = {
    ...
    { name = "AimedShotPrepull", spell = SPELLS.AimedShot, ooc = true, not_moving = true, cooldown = 6, matches = function(context, action) return can_cast_before_auto(context, action, AIMED_SHOT_CAST_MS) end, after_cast = record_manual_shot },
...
for i = 1, #ACTIONS do
    ...
    strategies[#strategies + 1] = {
        ...
        execute = function(context) return execute_action(context, action, "[MARKSMANSHIP]") end,
    }
end

NS.rotation_registry:register("marksmanship", strategies, { get_state = function(context) return context end })
NS.log("Hunter marksmanship rotation registered")
return strategies
