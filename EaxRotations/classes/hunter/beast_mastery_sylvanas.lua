-- Hunter Beast Mastery priority list.
...
local function build_state(context)
    ...
    bm_state.has_pet = pet ~= nil
    bm_state.pet_alive = pet_alive == true
    bm_state.pet_dead = context.pet_dead == true or (pet ~= nil and not bm_state.pet_alive)
    bm_state.pet_hp_pct = bm_state.pet_alive and pet.get_health_percentage and pet:get_health_percentage() or 100
    bm_state.has_hunters_mark = target and NS.debuff_up(target, HUNTERS_MARK_DEBUFF) or false
    bm_state.has_serpent_sting = target and NS.debuff_up(target, SERPENT_STING_DEBUFF) or false
    bm_state.has_aspect_hawk = me and NS.buff_up(me, ASPECT_HAWK_BUFF) or false
    bm_state.has_aspect_viper = me and NS.buff_up(me, ASPECT_VIPER_BUFF) or false
    bm_state.mend_pet_ready = me and NS.spell_ready(SPELLS.MendPet, me, { skip_range = true }) or false
    bm_state.hunters_mark_ready = target and NS.spell_ready(SPELLS.HuntersMark, target) or false
    bm_state.bestial_wrath_ready = me and NS.spell_ready(SPELLS.BestialWrath, me, { skip_range = true, expected_cooldown = 120 }) or false
    bm_state.rapid_fire_ready = me and NS.spell_ready(SPELLS.RapidFire, me, { skip_range = true, expected_cooldown = 300 }) or false
    bm_state.kill_command_ready = target and NS.spell_ready(SPELLS.KillCommand, target, { expected_cooldown = 5 }) or false
    bm_state.multi_shot_ready = target and NS.spell_ready(SPELLS.MultiShot, target, { expected_cooldown = 10 }) or false
    bm_state.steady_shot_ready = target and NS.spell_ready(SPELLS.SteadyShot, target) or false
    bm_state.arcane_shot_ready = target and NS.spell_ready(SPELLS.ArcaneShot, target, { expected_cooldown = 6 }) or false
    bm_state.serpent_sting_ready = target and NS.spell_ready(SPELLS.SerpentSting, target) or false
    bm_state.call_pet_ready = me and NS.spell_ready(SPELLS.CallPet, me, { skip_range = true }) or false
    bm_state.revive_pet_ready = me and NS.spell_ready(SPELLS.RevivePet, me, { skip_range = true }) or false
    bm_state.feign_death_ready = me and NS.spell_ready(SPELLS.FeignDeath, me, { skip_range = true, expected_cooldown = 30 }) or false
    bm_state.freezing_trap_ready = me and NS.spell_ready(SPELLS.FreezingTrap, me, { skip_range = true, expected_cooldown = 30 }) or false	    bm_state.viper_sting_ready = target and NS.spell_ready(SPELLS.ViperSting, target, { expected_cooldown = 8 }) or false
	    bm_state.readiness_ready = me and NS.spell_ready(SPELLS.Readiness, me, { skip_range = true, expected_cooldown = 180 }) or false
	    bm_state.mana_pct = context.mana_pct or (me and NS.unit_mana_pct(me)) or 100
    bm_state.in_combat = context.in_combat or false
    bm_state.enemy_count = context.enemy_count or context.enemies_count or 1
    bm_state.pre_steady_leveling = ((context.player_level or 70) < 62) or (context.is_leveling == true and not bm_state.steady_shot_ready)
...
local BESTIAL_WRATH_ACTION = { name = "BestialWrath", spell = SPELLS.BestialWrath, target = "pet", combat = true, cooldown = 120, requires_target = false, setting = "use_cooldowns" }
...
local function freezing_trap_matches(context, s)
	...
	local function readiness_matches(context, s)
	    ...
	    -- Use after Bestial Wrath fade or if Bestial Wrath on CD to reset it
...
-- ============================================================================
local strategies = {
    { name = "MendPet", matches = mend_pet_matches, execute = function(context) return execute_action(context, MEND_PET_ACTION, "[BEAST_MASTERY]") end },
    { name = "CallPet", matches = call_pet_matches, execute = function(context) return execute_action(context, CALL_PET_ACTION, "[BEAST_MASTERY]") end },
    { name = "RevivePet", matches = revive_pet_matches, execute = function(context) return execute_action(context, REVIVE_PET_ACTION, "[BEAST_MASTERY]") end },
    { name = "AspectOfTheHawk", matches = aspect_hawk_matches, execute = function(context) return execute_action(context, ASPECT_HAWK_ACTION, "[BEAST_MASTERY]") end },
    { name = "AspectOfTheViper", matches = aspect_viper_matches, execute = function(context) return execute_action(context, ASPECT_VIPER_ACTION, "[BEAST_MASTERY]") end },
    { name = "FreezingTrap", matches = freezing_trap_matches, execute = function(context) return execute_action(context, FREEZING_TRAP_ACTION, "[BEAST_MASTERY]") end },
    { name = "HuntersMark", matches = hunters_mark_matches, execute = function(context) return execute_action(context, HUNTERS_MARK_ACTION, "[BEAST_MASTERY]") end },
    { name = "BestialWrath", matches = bestial_wrath_matches, execute = function(context) return execute_action(context, BESTIAL_WRATH_ACTION, "[BEAST_MASTERY]") end },
    { name = "RapidFire", matches = rapid_fire_matches, execute = function(context) return execute_action(context, RAPID_FIRE_ACTION, "[BEAST_MASTERY]") end },	    { name = "Readiness", matches = readiness_matches, execute = function(context) return execute_action(context, READINESS_ACTION, "[BEAST_MASTERY]") end },
	    { name = "KillCommand", matches = kill_command_matches, execute = function(context) return execute_action(context, KILL_COMMAND_ACTION, "[BEAST_MASTERY]") end },
	    { name = "FeignDeath", matches = feign_death_matches, execute = function(context) return execute_action(context, FEIGN_DEATH_ACTION, "[BEAST_MASTERY]") end },
    { name = "LevelingArcaneShot", matches = leveling_arcane_shot_matches, execute = function(context) if execute_action(context, ARCANE_SHOT_ACTION, "[BEAST_MASTERY]") then record_manual_shot() return true end return false end },
    { name = "LevelingSting", matches = leveling_sting_matches, execute = function(context) return execute_action(context, SERPENT_STING_ACTION, "[BEAST_MASTERY]") end },
    { name = "MultiShot", matches = multi_shot_matches, execute = function(context) if execute_action(context, MULTI_SHOT_ACTION, "[BEAST_MASTERY]") then record_manual_shot() return true end return false end },
    { name = "SteadyShot", matches = steady_shot_matches, execute = function(context) if execute_action(context, STEADY_SHOT_ACTION, "[BEAST_MASTERY]") then record_manual_shot() return true end return false end },
    { name = "ArcaneShot", matches = arcane_shot_matches, execute = function(context) if execute_action(context, ARCANE_SHOT_ACTION, "[BEAST_MASTERY]") then record_manual_shot() return true end return false end },
    { name = "ViperSting", matches = viper_sting_matches, execute = function(context) return execute_action(context, VIPER_STING_ACTION, "[BEAST_MASTERY]") end },
    { name = "SerpentSting", matches = serpent_sting_matches, execute = function(context) return execute_action(context, SERPENT_STING_ACTION, "[BEAST_MASTERY]") end },
}

NS.rotation_registry:register("beast_mastery", strategies, { get_state = build_state })
NS.log("Hunter beast_mastery rotation registered (Tier A)")
return strategies
