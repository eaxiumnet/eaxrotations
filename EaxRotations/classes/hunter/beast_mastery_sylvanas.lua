-- Readability notes:
--   What: Hunter Beast Mastery priority list.
...
local ACTIONS = {
    ...
    { name = "BestialWrath", spell = SPELLS.BestialWrath, target = "pet", combat = true, cooldown = 120, requires_target = false, setting = "use_cooldowns" },
...
for i = 1, #ACTIONS do
    ...
    strategies[#strategies + 1] = {
        ...
        execute = function(context) return execute_action(context, action, "[BEAST_MASTERY]") end,
    }
end

NS.rotation_registry:register("beast_mastery", strategies, { get_state = function(context) return context end })
NS.log("Hunter beast_mastery rotation registered")
return strategies
