...
function NS.action_matches(context, action)
    ...
    if action.ooc and context.in_combat then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: ooc_required") end
        return false
    end
    ...
    if action.max_hp and actor_hp > action.max_hp then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: max_hp=" .. tostring(actor_hp)) end
        return false
    end
    if action.min_hp and actor_hp < action.min_hp then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: min_hp=" .. tostring(actor_hp)) end
        return false
    end
    if action.target_max_hp and (context.target_hp or 100) > action.target_max_hp then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: target_max_hp=" .. tostring(context.target_hp)) end
        return false
    end
    ...
    if action.min_ttd then
        if action.require_ttd and not context.ttd_known then
            if debug then NS.log("[DEBUG] " .. name .. " blocked: ttd_unknown") end
            return false
        end
        if (context.ttd or 0) < action.min_ttd then
            if debug then NS.log("[DEBUG] " .. name .. " blocked: min_ttd=" .. tostring(context.ttd)) end
            return false
        end
    end
    if action.enemy_count and (context.enemy_count or 0) < action.enemy_count then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: enemy_count=" .. tostring(context.enemy_count)) end
        return false
    end
    if context.settings and context.settings.aoe_enabled == false and (action.enemy_count or action.is_aoe) then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: aoe_disabled") end
        return false
    end
    if action.max_enemy_count and (context.enemy_count or 0) > action.max_enemy_count then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: max_enemy_count=" .. tostring(context.enemy_count)) end
        return false
    end
    if action.min_mana and (context.mana_pct or 100) < action.min_mana then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: min_mana=" .. tostring(context.mana_pct)) end
        return false
    end
    if action.max_mana and (context.mana_pct or 100) > action.max_mana then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: max_mana=" .. tostring(context.mana_pct)) end
        return false
    end
    if action.target == "pet" and not NS.GetPet() then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: no_pet") end
        return false
    end
    if action.min_rage and (context.rage or 0) < action.min_rage then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: min_rage=" .. tostring(context.rage)) end
        return false
    end
    if action.min_energy and (context.energy or 0) < action.min_energy then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: min_energy=" .. tostring(context.energy)) end
        return false
    end
    if action.min_combo and (context.combo_points or 0) < action.min_combo then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: min_combo=" .. tostring(context.combo_points)) end
        return false
    end
    if action.not_moving and context.is_moving then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: moving") end
        return false
    end
    if action.moving and not context.is_moving then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: not_moving") end
        return false
    end
    if action.not_casting and (context.is_casting or context.is_channeling) then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: already_casting") end
        return false
    end
    if action.required_stance and context.stance ~= action.required_stance then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: wrong_stance") end
        return false
    end
    if action.required_form and not NS.has_form(action.required_form) then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: missing_form") end
        return false
    end
    if action.requires_buff and not NS.buff_up(NS.GetPlayer(), action.requires_buff) then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: missing_buff") end
        return false
    end
    if action.requires_behind and not NS.is_behind_target(context.target) then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: not_behind") end
        return false
    end
    if action.kind == "form" and NS.has_form(action.form) then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: already_in_form") end
        return false
    end
    if action.kind == "buff" and NS.buff_up(NS.GetPlayer(), action.buff or action.spell) then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: buff_already_up") end
        return false
    end
    if action.kind == "threat_drop" and not NS.should_drop_threat(context) then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: no_threat_drop") end
        return false
    end
    if action.requires_ready_spell then
        ...
        if not ready_target or not NS.spell_ready(action.requires_ready_spell, ready_target, { expected_cooldown = action.requires_ready_cooldown }) then
            if debug then NS.log("[DEBUG] " .. name .. " blocked: requires_ready_spell") end
            return false
        end
    end
    ...
    if action.target_not_player and target then
        ...
        if is_player and safe(is_player, target) == true then
            if debug then NS.log("[DEBUG] " .. name .. " blocked: target_player") end
            return false
        end
    end
    if action.creature_types and target then
        ...
        if not creature_type or not action.creature_types[creature_type] then
            if debug then NS.log("[DEBUG] " .. name .. " blocked: creature_type=" .. tostring(creature_type)) end
            return false
        end
    end
    if action.debuff and target then
        ...
        if debuff_mode == "help_stack" then
            ...
            if stacks >= max_stacks then
                if debug then NS.log("[DEBUG] " .. name .. " blocked: debuff_stack_cap=" .. tostring(stacks)) end
                return false
            end
        elseif debuff_mode == "maintain" then
            ...
            if stacks >= max_stacks then
                ...
                if remains > refresh then
                    if debug then NS.log("[DEBUG] " .. name .. " blocked: debuff_refresh=" .. tostring(remains)) end
                    return false
                end
            end
        else
    ...
    if action.requires_debuff and target and not NS.debuff_up(target, action.requires_debuff) then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: requires_debuff") end
        return false
    end
    if action.blocked_debuff and target and NS.debuff_up(target, action.blocked_debuff) then
        if debug then NS.log("[DEBUG] " .. name .. " blocked: blocked_debuff") end
        return false
    end
