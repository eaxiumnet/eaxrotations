...
Total: 475 rotation suites + 31 leveling suites registered (475/475 rotation passing at runtime — incl. the 3 `check_*` static-analysis audits; `test_sod_source_audit` self-provisions its `.omo/evidence` via the tracked generator).
---
...
`tools/run_all_checks.sh` regenerates and drift-checks it; otherwise CI consumes
the committed artifact. **Manual step:** after adding spell IDs/ladders to the
