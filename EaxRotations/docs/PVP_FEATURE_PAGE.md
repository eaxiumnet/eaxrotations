...
Total: 486 rotation suites + 31 leveling suites registered (486/486 rotation passing at runtime — incl. the 3 `check_*` static-analysis audits; `test_sod_source_audit` self-provisions its `.omo/evidence` via the tracked generator).
---

### Data provenance: buff/debuff verification pipeline

The spell-ID audit artifact `EaxRotations/tools/buff_debuff_full_verification.json`
is **committed and consumed by tests** (`test_id_audit_report.lua` in the rotation
suite). It is regenerated by `EaxRotations/tools/generate_buff_debuff_verification.py`
(LOCAL-ONLY: its `wowheadScrape/dbc_extract/wowsims.db` DBC input is gitignored,
so a fresh checkout cannot regenerate it). When the DB is present,
regenerate it manually and commit the result (the pre-commit gate drift-checks
the committed artifact); otherwise CI consumes it as-is. **Manual step:** after adding spell IDs/ladders to the
WotLK bridges or `*_wotlk.lua` files, run the generator, verify with
`lua EaxRotations/tests/test_id_audit_report.lua`, and commit the regenerated JSON.
...
EaxRotations v2.23.0 — CC-BY-4.0 License — Built for TBC Classic Anniversary
