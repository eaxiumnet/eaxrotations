...
| Decision rules exercised by the test rig | 3583 |
...
| Behavioral test battery | 589 rotation suites — every one must pass or the release gate fails (plus leveling and per-era gates) |
...
- Run the whole release gate yourself: `lua EaxRotations/tests/run_verify_all.lua` (589 rotation suites + leveling + five era batteries + this page’s drift check).
