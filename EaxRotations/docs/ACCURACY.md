...
| Behavioral test battery | 565 rotation suites — every one must pass or the release gate fails (plus leveling and per-era gates) |
...
- Run the whole release gate yourself: `lua EaxRotations/tests/run_verify_all.lua` (565 rotation suites + leveling + four era batteries + this page’s drift check).
