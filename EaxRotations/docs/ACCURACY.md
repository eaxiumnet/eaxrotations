...
| Decision rules exercised by the test rig | 3616 |
| Rules that could never fire in live play (dead code) | 0 — the gate fails if this is ever above 0 |
| Rules the rig never triggers, each with a filed written reason | 29 |
| Behavioral test battery | 602 rotation suites — every one must pass or the release gate fails (plus leveling and per-era gates) |
...
- Run the whole release gate yourself: `lua EaxRotations/tests/run_verify_all.lua` (602 rotation suites + leveling + five era batteries + this page’s drift check).
