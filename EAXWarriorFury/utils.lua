...
function utils.get_current_stance(me)
    ...
    -- 5. Usability probe: the stance you're IN is not usable (you can't cast it again)
    if core.spell_book.get_global_cooldown() <= 0 then
        local battle_usable = core.spell_book.is_usable_spell(spells.BUFF_BATTLE_STANCE[1])
        local berserker_usable = core.spell_book.is_usable_spell(spells.BUFF_BERSERKER_STANCE[1])
        local defensive_usable = core.spell_book.is_usable_spell(spells.BUFF_DEFENSIVE_STANCE[1])

        if not battle_usable and (berserker_usable or defensive_usable) then
            utils._tracked_stance = "battle"
            return "battle"
        end
        if not berserker_usable and (battle_usable or defensive_usable) then
            utils._tracked_stance = "berserker"
            return "berserker"
        end
        if not defensive_usable and (battle_usable or berserker_usable) then
            utils._tracked_stance = "defensive"
            return "defensive"
        end
    end
