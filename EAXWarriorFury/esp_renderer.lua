...
--   esp.init("fury")  -- Call once with unique spec identifier
--   core.register_on_render_callback(function()
--       esp.on_render(menu, state)
--   end)
--
-- And after each cast decision, call:
--   esp.on_cast(spell_id, spell_name, color_hint, target_name)
--   esp.notify(unique_id, message, color, duration_s)
--
-- v1.6.0 - Added spec isolation and target names
...
local function draw_target_esp(menu)
    ...
    if next_spell_name == "" then return end

    local age = core.time() - next_spell_set_at
    ...
    local label = "▶ " .. next_spell_name
    local col = to_api_color(next_spell_color or _color.gold(220))
