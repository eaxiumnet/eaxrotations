...
function Inventory:FindItemsToBuy()
	...
	for index=1,GetMerchantNumItems() do while(1) do 
		...
		if numAvail~=-1 and numAvail < item.amount then ZGV:Print(L['loot_autobuynostock']:format(merchItemName,item.amount)) return end -- not enough, aaaabort
...
function Inventory.OnEvent(self, event)
	...
	elseif event=="BAG_UPDATE" then
		if BankFrame and BankFrame:IsVisible() then
			if not ZGV:Throttler("Inventory_BAG_UPDATE",0) then
				Inventory:RecordBank()
			end
		end
...
-------------------------------------------------------------------------------
local bankslots = ZGV.IsRetail and {-1,6,7,8,9,10,11,12} or {-1,5,6,7,8,9,10}
local bagslots = ZGV.IsRetail and {0,1,2,3,4,5} or {0,1,2,3,4}
function Inventory:MoveItems(source,itemid,count)
	...
	local isReagent = select(17,ZGV:GetItemInfo(itemid))
