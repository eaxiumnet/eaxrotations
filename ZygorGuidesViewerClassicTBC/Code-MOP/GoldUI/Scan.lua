...
function Scan:ListenByKey(itemkey)
	...
	if name=="" then return false,"no name" end
	if (self.state~="SS_IDLE" and self.state~="SS_MESSAGE") then return false,"state "..self.state.." not SS_IDLE" end
...
function Scan:AnalyzeAuctions(cmd,arg) -- Ugly Self-coroutine.
	...
	if Scan.Proxy:IsFullScan() then
		...
		for i=#self.time,1,-1 do
			if self.time[i]<cutoff then
				table.remove(self.data,i)
				table.remove(self.time,i)
			end
		end
...
function Scan:DebugItem(id)
	...
	local scandata_id = ZGV.F.dig_in(ZGV.db.factionrealm,"gold_scan_data",1,id)
	if scandata_id then
		--DebugL("AH data:")
		local count=0
...
function Scan:ImportHourly()
	...
	local date = C_DateAndTime.GetCalendarTimeFromEpoch(ZGV.db.factionrealm.LastScan*1000000)
...
function Scan:SaveLastReplicateCount()
	...
	if num_replic>0 then self.db.factionrealm.gold_last_num_replicate = num_replic end
end
function Scan:GetLastReplicateCount()
	return self.db.factionrealm.gold_last_num_replicate
end
