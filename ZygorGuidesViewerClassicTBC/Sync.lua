...
function Sync:OnChatReceived(msg,channel,sender)
	...
	if not msg then Sync:Debug("No packet received") return end

	local msg_decoded = LibDeflate:DecodeForWoWAddonChannel(msg)
	if not msg_decoded then Sync:Debug("No packet decoded") return end

	local msg_unpacked = LibDeflate:DecompressDeflate(msg_decoded)
	if not msg_unpacked then Sync:Debug("No packet unpacked") return end
