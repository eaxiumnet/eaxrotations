...
function eax_utils.get_focus_target(menu_settings)
    if not menu_settings or not menu_settings.focus_priority then return nil end
    local focus_enabled = false
    if type(menu_settings.focus_priority.get_state) == "function" then focus_enabled = menu_settings.focus_priority:get_state() end
...
function eax_utils.get_self_heal_threshold(me, base_threshold, menu_settings)
    ...
    if menu_settings and menu_settings.combat_self_hp_boost then
        if type(menu_settings.combat_self_hp_boost.get) == "function" then boost = menu_settings.combat_self_hp_boost:get() / 100 end
    end
    if in_combat then return math.min(base_threshold + boost, 0.95) end
