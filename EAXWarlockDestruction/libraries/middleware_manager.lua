-- EAXWarlockDestruction Middleware Manager
-- Integrates libraries/middleware with Destruction Warlock-specific registrations
...
--- Initialize and register all Destruction Warlock-specific middleware
...
function middleware_manager.initialize(menu)
    ...
    local use_death_coil = (menu.use_death_coil and menu.use_death_coil:get_state()) or false
    local death_coil_threshold = (menu.death_coil_hp_pct and menu.death_coil_hp_pct:get()) or 30
    
    -- Register Healthstone middleware (off-GCD, defensive)
    if use_healthstone then
        middleware.register(middleware.healthstone(
            WARLOCK_SPELLS.HEALTHSTONE,
        ...
        local mana_mw = {
            ...
            matches = function(ctx)
                ...
                if core.spell_book and core.spell_book.get_item_cooldown then
                    local cd = core.spell_book.get_item_cooldown(WARLOCK_SPELLS.MANA_POTION)
    ...
    if use_berserking then
        middleware.register(middleware.offensive_racial(
            WARLOCK_SPELLS.BERSERKING,
            middleware.PRIORITY.OFFENSIVE_CDS - 5
        ))
    end
    
    -- Register Death Coil middleware (defensive CD)
    if use_death_coil then
        local death_coil_mw = {
            name = "Death Coil",
            ...
            matches = function(ctx)
                ...
                if core.spell_book and core.spell_book.get_spell_cooldown then
                    local cd = core.spell_book.get_spell_cooldown(WARLOCK_SPELLS.DEATH_COIL)
                    if cd > 0 then return false end
                end
        ...
        middleware.register(death_coil_mw)
    end
