-- EAX Warlock Demonology | libraries/menu.lua | Project Sylvanas
local unified = require("EAX_Unified/menu")
if not unified then error("[EAX Warlock Demonology] EAX_Unified/menu not found!") end

local ROTATION_KEY = "warlock_demonology"
local menu = {}
    ...
    categories = {
        {
            name = "Rotation",
            settings = {
                { key = "use_corruption",         type = "checkbox", label = "Corruption",         default = true },
                { key = "use_immolate",           type = "checkbox", label = "Immolate",           default = true,  tooltip = "Fire DoT — enables Conflagrate" },
                ...
                { key = "use_soul_fire",          type = "checkbox", label = "Soul Fire",          default = true,  tooltip = "Decimation proc — use in execute" },
                { key = "use_shadowburn",         type = "checkbox", label = "Shadowburn",         default = true,  tooltip = "Execute range instant" },
                { key = "mode",                   type = "combo",    label = "Priority Mode",      default = 1, options = {"Single Target", "AoE", "Metamorphosis"} },
            },
        },
        {
            name = "Pet",
            settings = {
                ...
                { key = "preferred_pet",       type = "combo",    label = "Preferred Pet",      default = 1, options = {"Felguard", "Felhunter", "Imp", "Succubus"} },
                { key = "use_soul_link",       type = "checkbox", label = "Soul Link",          default = true,  tooltip = "Share 20% damage with pet" },
                { key = "use_health_funnel",   type = "checkbox", label = "Health Funnel",      default = true,  tooltip = "Heal pet when low" },
                { key = "pet_heal_hp_pct",     type = "slider",   label = "Pet Heal HP %",      default = 40, min = 10, max = 70, suffix = "%" },
                { key = "health_funnel_hp_pct",type = "slider",   label = "Funnel Trigger HP %",default = 30, min = 5, max = 60, suffix = "%" },
            },
        },
        {
            name = "Sustain",
...
local callbacks = {
    on_enabled  = function() print("|cFF6666EE[EAX Demonology]|r Rotation ENABLED") end,
    on_disabled = function() print("|cFF6666EE[EAX Demonology]|r Rotation disabled") end,
}

unified.register_rotation("Warlock", "Demonology", MENU_DEF, callbacks)
...
menu.use_soul_link            = proxy("use_soul_link",           true)
menu.use_health_funnel        = proxy("use_health_funnel",       true)
menu.pet_heal_hp_pct          = proxy("pet_heal_hp_pct",         40)
menu.health_funnel_hp_pct     = proxy("health_funnel_hp_pct",    30)
...
local _enabled_keybind  = core.menu.keybind(7, false, "eax_warlock_demonology_enabled_kb")
local _toggle_key_elem  = core.menu.keybind(107, false, "eax_warlock_demonology_toggle_key")
...
function menu.on_menu_render()
    _menu_tree:render("EAX Demonology", function()
        _enabled_keybind:render("Enable EAX Demonology", "Toggle rotation on/off")
