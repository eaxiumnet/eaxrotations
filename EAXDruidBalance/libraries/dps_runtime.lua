...
local function load_health_prediction()
    ...
    local ok, module = pcall(require, "libraries/health_prediction")
    if ok then
...
local function unit_guid(unit)
    if type(unit) ~= "table" or type(unit.get_guid) ~= "function" then
        return nil
    end
...
function dps_runtime.build_snapshot(me, target, encounter_manager, ttd_tracker)
    ...
    if _last_snapshot ~= nil
        and _last_snapshot_tick == now_s
        and _last_snapshot_me == me
        and _last_snapshot_target == target
        and _last_snapshot_me_guid == me_guid
        and _last_snapshot_target_guid == target_guid then
