...
context.hp = (ok_hp and ok_max and hp and max_hp and max_hp > 0) and ((hp / max_hp) * 100) or 100
    local ok_max_hp2, max_hp2 = pcall(function() return me:get_max_health() end)
    context.max_hp = ok_max_hp2 and max_hp2 or 0
    local ok_cur_hp, cur_hp = pcall(function() return me:get_health() end)
    ...
    elseif me.get_power and me.get_max_power then
        ...
        local ok_power2, power2 = pcall(function() return me:get_power() end)
        if ok_max_power and ok_power2 and max_power and max_power > 0 then
            context.mana_pct = (power2 / max_power) * 100
        else
            context.mana_pct = 0
        end
    else
...
function _compat.cast_safe(spell_id, target)
    ...
    local ok_target, cast_target = pcall(function()
        if target then return target end
        if me and me.get_target then return me:get_target() end
        return nil
    end)
    if not ok_target then cast_target = nil end
