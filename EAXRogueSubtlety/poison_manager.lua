...
local function slot_has_temporary_enchant(me, slot_id)
    ...
    if type(me.get_equipped_items) == "function" then local ok, items = pcall(me.get_equipped_items, me); if ok and items then for _, value in pairs(items) do if value and value.slot_id == slot_id and value.object and item_has_temporary_enchant(value.object) then return true end end end end
    return false
end
