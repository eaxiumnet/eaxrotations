...
local function is_valid_object(obj)
    if not obj or type(obj) ~= "table" or type(obj.is_valid) ~= "function" then
        return false
    end

    local ok, valid = pcall(obj.is_valid, obj)
...
function rotation_context.get(cache, me, target, deps)
    if not cache or not is_valid_object(me) then
        return nil
    end
    ...
    if me.get_combo_points_target then
        ...
        if ok and is_valid_object(cp_target) and cp_target.get_guid then
    ...
    -- Build new context
    local ok, ctx = pcall(combat_context.build, me, target, cache.spec_meta, deps)
    if not ok or not ctx then
        cache.dirty = true
        return cache.ctx or combat_context.default_context(now_s)
    end
