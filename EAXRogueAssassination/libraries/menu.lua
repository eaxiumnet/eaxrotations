-- EAX Rogue Assassination | libraries/menu.lua | Project Sylvanas
local unified = require("EAX_Unified/menu")
if not unified then error("[EAX Rogue Assassination] EAX_Unified/menu not found!") end

local ROTATION_KEY = "rogue_assassination"
local menu = {}
    ...
    categories = {
        {
            name = "Rotation",
            settings = {
                { key = "use_mutilate",          type = "checkbox", label = "Mutilate",          default = true,  tooltip = "Primary combo builder" },
                { key = "use_backstab",          type = "checkbox", label = "Backstab",          default = true,  tooltip = "Behind-target builder" },
                { key = "use_sinister_strike",   type = "checkbox", label = "Sinister Strike",   default = true,  tooltip = "Fallback builder" },
                { key = "use_rupture",           type = "checkbox", label = "Rupture",           default = true,  tooltip = "Bleed finisher" },
                { key = "rupture_combo_points",  type = "slider",   label = "Rupture Min CP",    default = 5, min = 1, max = 5 },
                { key = "rupture_refresh_seconds",type = "slider",  label = "Rupture Refresh (s)",default = 3, min = 1, max = 6, tooltip = "Refresh when this many seconds remain" },
                { key = "use_eviscerate",        type = "checkbox", label = "Eviscerate",        default = true,  tooltip = "Direct damage finisher" },
                { key = "eviscerate_combo_points",type = "slider",  label = "Eviscerate Min CP", default = 5, min = 1, max = 5 },
                { key = "use_slice_and_dice",    type = "checkbox", label = "Slice and Dice",    default = true,  tooltip = "Attack speed buff — keep up" },
                { key = "snd_combo_points",      type = "slider",   label = "SnD Min CP",        default = 1, min = 1, max = 5 },
                { key = "use_expose_armor",      type = "checkbox", label = "Expose Armor",      default = false, tooltip = "Armor debuff finisher" },
        ...
        },
        {
            name = "Cooldowns",
            settings = {
                { key = "use_cold_blood",        type = "checkbox", label = "Cold Blood",        default = true,  tooltip = "100% crit guaranteed" },
                { key = "use_cheap_shot",        type = "checkbox", label = "Cheap Shot",        default = true,  tooltip = "Opener stun from stealth" },
                { key = "use_kidney_shot",       type = "checkbox", label = "Kidney Shot",       default = true,  tooltip = "Combo point stun" },
        ...
        },
        {
            ...
            settings = {
                ...
                { key = "use_feint",             type = "checkbox", label = "Feint",             default = true,  tooltip = "AoE threat reduction" },
                { key = "feint_aoe_threshold",   type = "slider",   label = "Feint AoE Threshold",default = 3, min = 2, max = 8 },
            },
        },
        {
            name = "System",
...
local callbacks = {
    on_enabled  = function() print("|cFFF44040[EAX Assassination]|r Rotation ENABLED") end,
    on_disabled = function() print("|cFFF44040[EAX Assassination]|r Rotation disabled") end,
}

unified.register_rotation("Rogue", "Assassination", MENU_DEF, callbacks)
...
menu.use_mutilate               = proxy("use_mutilate",              true)
...
local _enabled_keybind  = core.menu.keybind(7, false, "eax_rogue_assassination_enabled_kb")
local _toggle_key_elem  = core.menu.keybind(107, false, "eax_rogue_assassination_toggle_key")
...
function menu.on_menu_render()
    _menu_tree:render("EAX Assassination", function()
        _enabled_keybind:render("Enable EAX Assassination", "Toggle rotation on/off")
