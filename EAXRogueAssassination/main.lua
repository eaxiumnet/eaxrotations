...
core.register_on_render_callback(function()
    if menu and menu.on_render then
        ...
        if not ok then
            core.log_error(string.format("[EAX Assassination] Render error: %s", tostring(err)))
        end
    end
end)
...
core.register_on_render_menu_callback(function()
    if menu and menu.on_menu_render then
        ...
        if not ok then
            core.log_error(string.format("[EAX Assassination] Menu render error: %s", tostring(err)))
        end
    end
end)
