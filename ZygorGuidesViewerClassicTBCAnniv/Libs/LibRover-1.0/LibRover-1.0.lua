...
function Lib:BoatLockDisable(mode)
	...
	LibRover.BoatUnlockZone = nil
	LibRover.BoatLock = false
end

local lastzonedebug = ""

function Lib:BoatLockDisableByZone()
	if not LibRover.BoatUnlockZone then return end
	
	local minizone = (BZR[GetMinimapZoneText()] or GetMinimapZoneText() or "")

	if LibRover.BoatUnlockZone == (BZR[GetMinimapZoneText()] or GetMinimapZoneText() or "") then
		Lib:BoatLockDisable("zone")
...
function Lib:BoatLockEnable(mode)
	...
	if (BZR[GetMinimapZoneText()] or GetMinimapZoneText() or "") == "Rut'theran Village" then
		LibRover.BoatUnlockZone = "Mist's Edge"
	end
	
	if (BZR[GetMinimapZoneText()] or GetMinimapZoneText() or "") == "Mist's Edge" then
		LibRover.BoatUnlockZone = "Rut'theran Village"
	end
...
local temp_fields_i = {"cost","time","mycost","mytime","speed","status","parentlink","parent","prev","next","text","maplabel","toend","taxiFinal","taxiDestination","link","a_b","a_b__c_d","costdesc","wayp_override_icon","wayp_override_text","border_optimization","changed_modes","through_instance"}
...
function Lib:StepPath() -- THE WORKHORSE.
	...
	for ni=1,#current_n do local current_n_ni=current_n[ni] local neigh,neighlink=current_n_ni[1],current_n_ni[2]
		...
		then
			...
			-- === FAILED NODES 
				-- do not, I repeat DO NOT take shortcuts through instances
				local through_instance = current.through_instance or 0
				
				if Lib.IsRetail or Lib.IsClassicCATA then
					local mapinfo_src = ZGV.GetMapInfo(current.m)
					local mapinfo_dst = ZGV.GetMapInfo(neigh.m)
					if (mapinfo_src.mapType==4 and mapinfo_dst.mapType~=4) or (mapinfo_src.mapType==4 and mapinfo_dst.mapType~=4) then
						through_instance = through_instance + 1
					end
				else
					if Lib.data.DungeonMaps[current.m]~=Lib.data.DungeonMaps[neigh.m] then
						through_instance = through_instance + 1
					end
				end
												
				if through_instance > 1 then
			...
			if not neigh.cost or cost<neigh.cost then
				...
				neigh.through_instance = through_instance
				neigh.speed = speed
				updated=true
...
function Lib:OnUpdate(elapsed)
	...
	if ospdel>0.1 then
		...
		if issecretvalue and issecretvalue(unitspeed) then return end
				
		local isGliding = C_PlayerInfo and C_PlayerInfo.GetGlidingInfo and C_PlayerInfo.GetGlidingInfo()
		local inv_mov = (unitspeed==0 and not isGliding and self.actual_speed and self.actual_speed>0)
		if inv_mov then
	...
	elseif event=="ZONE_CHANGED" or event=="ZONE_CHANGED_NEW_AREA" or event=="ZONE_CHANGED_INDOORS" or event=="LOADING_SCREEN_DISABLED" then
		...
		if event=="ZONE_CHANGED" then
			if LibRover.BoatLock then
				LibRover:BoatLockDisableByZone()
			end		
		end
