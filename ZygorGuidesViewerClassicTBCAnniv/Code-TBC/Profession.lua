...
ZGVP.tradeskills = {
	...
	[755] = {name="Jewelcrafting",crafting=true, skill=762},
	[633] = {name="Lockpicking", skill=633},
}
...
ZGVP.skillLocale = {
	...
	[633]={deDE="Schlossknacken",	esES="Ganzúa",		frFR="Crochetage",	ptBR="Arrombamento",	ruRU="Вскрытие замков",	koKR="자물쇠 따기",zhCN="开锁",	zhTW="開鎖",	esMX="Ganzúa"},
} -- GETS TRIMMED.
...
function ZGV:CacheRecipesCraft()
	if ZGVP.CRQ_Timer then ZGV:CancelTimer(ZGVP.CRQ_Timer) end
	ZGVP.CRQ_Timer = ZGV:ScheduleTimer(function() 
		ZGV:CacheRecipesCraft_Queued()
	end, 2)
end

function ZGV:CacheRecipesCraft_Queued()
	local skill = GetCraftDisplaySkillLine()
	if skill=="UNKNOWN" then return end
	...
	local scanned=0
	for i = 1,500 do
		local tradeName, _, tradeType, numAvailable = GetCraftInfo(i)
		...
		if tradeName and tradeType~="header" and tradeType~="subheader" then
			...
			if link then
				...
				local spell = recipe.productid and tonumber(recipe.productid)
				if spell then 
					recipes[spell]=recipe
					recipe.spell = spell
					recipe.learned = true
					...
					recipe.numSkillUps = 1
					recipe.numAvailable = numAvailable
					--recipe.link = GetCraftRecipeLink(i)
					recipe.productid = tonumber(recipe.productid)
					recipe.nummade = {GetCraftNumMade(i)}
					recipe.reagents={}
					local numreagents = GetCraftNumReagents(i)
					for r=1,numreagents do
						local reagent = {}
						reagent.name,reagent.texture,reagent.num,reagent.have = GetCraftReagentInfo(i,r)

						if not reagent.id then ZGV:Debug("Missing reagent in recipe %d: %d %s", i,recipe.spell,recipe.link) end

						reagent.link = GetCraftReagentItemLink(i,r)
						if reagent.link then reagent.id = tonumber(reagent.link:match("|H%w+:(%d+)")) end
						tinsert(recipe.reagents,reagent)
						if not reagent.id then ZGV:Debug("Missing reagent %d in recipe %d: %d %s", r, i, recipe.spell,recipe.link) end
					end
...
tinsert(ZGV.startups,{"Professions setup",function(self)
	...
	self:AddEventHandler("CRAFT_UPDATE","CacheRecipesCraft")
