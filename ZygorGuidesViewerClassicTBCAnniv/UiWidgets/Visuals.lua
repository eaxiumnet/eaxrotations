...
Visuals.item = function(self,element,width,mode)
	...
	function object:onenterfunc()
		...
		if guide and not guide.missing then 
			if (not guide.description or guide.description=="") and not guide.headerdata.model then return end
			tooltip.Text:SetText(object.tooltip or guide.description or "")
			...
			if guide.headerdata.model then
				...
				if guide.description and guide.description~="" then
					...
					tooltip.GuideModel:SetWidth(100)
				else
					tooltip.Text:SetWidth(0)
					tooltip.GuideModel:SetWidth(250)
				end
...
Visuals.guideslist = function(self,element,width)
	...
	if not element.results then
		element.results = ZGV:FindFilteredGuides(element.filters,element.path)
		table.sort(element.results,function(a,b) return a[2]<b[2] end)
	end
	...
	if #element.results==0 and element.text then
		if guideslist_content then 
	...
	local counter = 0
	for i,v in ipairs(element.results) do
		counter = counter + 1
