...
GOALTYPES['get'] = { -- combining old 'get' and 'collect' now.
		...
		if self.targets then
			for _,target in ipairs(self.targets) do
				if tonumber(target[2]) then
					got = got + C_Item.GetItemCount(target[2],self.usebank)
				end
			end
		else
			if tonumber(self.targetid) then
				got = got + C_Item.GetItemCount(self.targetid,self.usebank)
			end
		end
	...
	events = {"PLAYER_INTERACTION_MANAGER_FRAME_SHOW"}, -- not BANKFRAME_OPENED, since at that time BankFrame is still not visible
	onenterevent = "PLAYER_INTERACTION_MANAGER_FRAME_SHOW",
	onevent = function(self, _, event, interactionType)
		if not ZGV.db.profile.autogossip then return end
		if not (self.count and self.got) then return end -- sanity check
		if self.noautogossip then return end

		if (self.force_nocomplete or not GOALTYPES['get'].iscomplete(self)) and ((event=="PLAYER_INTERACTION_MANAGER_FRAME_SHOW" and interactionType==Enum.PlayerInteractionType.Banker) or BankFrame:IsVisible()) then 
			local targetcount = (self.count==1 and self.plural and "all") or (self.count-self.got)
			ZGV.Inventory:MoveItems("bank",self.targetid,targetcount)
		end
	end,
...
GOALTYPES['trash'] = {
	...
	gettext = function(self)
		local got = C_Item.GetItemCount(self.targetid)
		if got>0 then
			return L["stepgoal_"..self.action.." #"]:format(COLOR_ITEM(ZGV:GetItemInfo(self.targetid)),COLOR_COUNT(got))
		else
			return L["stepgoal_"..self.action..""]:format(COLOR_ITEM(ZGV:GetItemInfo(self.targetid)),COLOR_COUNT(got))
		end
	end,
...
GOALTYPES['bank'] = {
	parse = GOALTYPES['trash'].parse,
	iscomplete = GOALTYPES['trash'].iscomplete,
	gettext = GOALTYPES['trash'].gettext,
	events = {"PLAYER_INTERACTION_MANAGER_FRAME_SHOW"}, -- not BANKFRAME_OPENED, since at that time BankFrame is still not visible
	onenterevent = "PLAYER_INTERACTION_MANAGER_FRAME_SHOW",
	onevent = function(self, _, event, interactionType)
		if not ZGV.db.profile.autogossip then return end
		...
		if (event=="PLAYER_INTERACTION_MANAGER_FRAME_SHOW" and interactionType==Enum.PlayerInteractionType.Banker) or BankFrame:IsVisible() then 
			ZGV.Inventory:MoveItems("bags",self.targetid,"all")
		end
	end,
}
...
GOALTYPES['collect'] = {
	...
	iscomplete = GOALTYPES['get'].iscomplete,
	events = GOALTYPES['get'].events,
	onenterevent = GOALTYPES['get'].onenterevent,
	onevent = GOALTYPES['get'].onevent,	
	-- gettext complex; still in Goal:GetText()
}
...
GOALTYPES['multiq'] = {
	-- param is a comma separated list of quest ids
	-- goes green if all quests are either completed, or not in log
	-- goes red if any quest is in log, but not completed
	parse = function(self,params,step)
		local mult = {split(",",params)}
		self.quests = {}

		for i,data in ipairs(mult) do
			local quest,questid = ParseID(data)
			table.insert(self.quests,questid)
		end
			
		if self.action=="text" then self.action="multiq" end -- override plaintext actions. A bit of a hack...
	end,
	iscomplete = function(self)
		...
		for _, questID in ipairs(self.quests) do
			local complete = ZGV.completedQuests[questID]
			local quest = ZGV.questsbyid[questID]
			local active = not complete and (quest and quest.inlog)
			
		
			if active then possible = true end -- quests in log will always make it possible
			if active then completed = false end -- but since they are in log, they are not complete
			if complete then anycompleted = true end -- if anything got completed, this goal is viable to go green
			if active or complete then anyvalid = true end -- any active or completed quest means this goal is valid

			if possible and not completed then break end
		end

		return anycompleted and completed, anyvalid and possible
	end
}
...
GOALTYPES['equipped'] = {
	...
	gettext = function(self) return L["stepgoal_equipped"]:format(COLOR_ITEM(self.target)) end,
}
...
GOALTYPES['unequipped'] = {
	...
	gettext = function(self) return L["stepgoal_unequipped"]:format(COLOR_ITEM(self.target)) end,
}
...
function Goal:UpdateStatus()
	...
	local visibilitychanged = (self.status~=newstatus) and (newstatus=="hidden" or self.status=="hidden")
	self.status = newstatus
	return self.status,visibilitychanged
end
...
function Goal:IsComplete()
 ...
 -- - if it completed, then report that; if it didn't, then stick with what the quest/achi/scen.obj said.
	...
	-- Many quests, handle it.
	if self.quests and self.action~="abandon" then
		local iscomplete,ispossible,numdone,numneeded,comment,isbad = GOALTYPES['multiq'].iscomplete(self)
		numdone,numneeded = norm_nums(numdone or iscomplete,numneeded)
		if not numdone and not numneeded then numdone,numneeded = iscomplete and 1 or 0,1 end
		if iscomplete or (not self.future and not ispossible) then 
			return iscomplete,ispossible,numdone,numneeded,comment,isbad
		else
			fallthrough_ispossible,fallthrough_numdone,fallthrough_numneeded = ispossible,numdone,numneeded
		end
...
function Goal:GetText(showcompleteness,brief,showtotals,nocolor)
	...
	elseif self.action=='get' or self.action=='craft' or self.action=='farm' then
		...
		if self.force_nocomplete and goalcountneeded==1 then goalcountneeded=0 end -- for |n lines, just show collected amount
