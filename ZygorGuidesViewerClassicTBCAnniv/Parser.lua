...
local ConditionEnv = {
	...
	itemcount = function(...)
		...
		for i = 1, count do
			...
			total = total + C_Item.GetItemCount(itemid)
		end
		return total
	end,
	...
	equipped = function(itemID)
		return ZGV.GOALTYPES["equipped"].iscomplete({targetid=itemID})
	end,
	discountgold = function(faction,amount)
		local rep = ZGV:GetReputation(faction)
		local standing = rep.standing
		
		if ZGV.IsClassic then
			-- classic, flat 10% discount at honored
			if standing>=6 then
				amount = math.ceil(amount - (amount * 0.1))
			end
		else
			-- rest, 5% for each above neutral
			if standing>4 then
				amount = math.ceil(amount - (amount * 0.05 * (standing-4)))
			end
		end
				...
				if step and not cmd_parsed then
					...
					--]]
						...
						elseif cmd=="noautoaccept" then
							...
							if params == "inparty" then goal.noautoacceptparty = true end
