...
local lastUnitSpeed = {}
function Pointer:SecureGetUnitSpeed(unit)
	local spdnow,spdrun,spdfly,spdswim = GetUnitSpeed(unit)
	if ZGV.IsSecret(spdnow) then
		return unpack(lastUnitSpeed)
	else
		lastUnitSpeed[1]=spdnow
		lastUnitSpeed[2]=spdrun
		lastUnitSpeed[3]=spdfly
		lastUnitSpeed[4]=spdswim
	end
...
function Pointer:OnEvent(event,...)
	...
	elseif event=="ZGV_GOAL_VISIBILITY_CHANGED" then
		ZGV:ShowWaypoints()
...
function Pointer:Startup()
	...
	ZGV:AddMessageHandler("ZGV_GOAL_VISIBILITY_CHANGED",self.OnEvent)
	...
	ZGV:ScheduleRepeatingTimer(function() Pointer:PointToNextTimer() end,0.2) -- moved from arrow onupdate
...
Pointer.Icons = {
	...
	mapmarker = { tex={file="mapicons",coords={0.0,0.5,0.0,0.5},r=254/255,g=97/255,b=0/255}, size=30, alpha=1.0, minisize=25, minimap_alpha=1, rotates=false, edgetex={file="mapicons",coords={0,0.5,0.5,1},r=1,g=1,b=1}, edgesize=20, onminimap="always" }, -- actually orange
...
function Pointer:GetMapMarker (m,x,y,data)
	...
	waypoint.frame_minimap:EnableMouse(false) -- not (waypoint.passive or waypoint.passivieminimap) -- 20260326 - minimap icons are passive now
	waypoint.frame_worldmap:EnableMouse(not waypoint.passive or waypoint.passivieworldmap)
...
function markerproto:SetIcon(icon) -- leave icon empty to just reset the current icon
	...
	local size = self.minisize or self.size or icon.minisize or 30
		...
		mini_ico:SetAlpha((icon.tex.a or 1) * (icon.minimap_alpha or ZGV.db.profile.minimap_marker_opacity or newalpha))
	--
...
function Pointer:SetupArrow()
	...
	if not self.ArrowFrame.ArrowIcon then
		self.ArrowFrame.ArrowIcon = CHAINC(CreateFrame("Button","ZygorGuidesViewerPointerArrow_Icon",self.ArrowFrameCtrl,"SecureActionButtonTemplate,SecureHandlerStateTemplate"))
			...
			.__END
		if ZGV.IsRetail or ZGV.IsClassicTBC or ZGV.IsClassicMOP then
...
function Pointer:SetFontSize(size)
	...
	self.ArrowFrame.title:SetFont(f,size,ZGV.db.profile.arrowoutline==true and "OUTLINE" or nil)
	self.ArrowFrame.desc:SetFont(f,size,ZGV.db.profile.arrowoutline==true and "OUTLINE" or nil)
	--[[
...
function Pointer.frame_worldmap_functions.OnEvent(self,event,...)
	...
	elseif event=="MODIFIER_STATE_CHANGED" then
		...
		if (key=="LCTRL" or key=="RCTRL") and GameTooltip:IsShown() then
			...
			if pin and pin.waypoint then
				Pointer.frame_minimap_functions.OnEnter(GameTooltip:GetOwner(),nil,GameTooltip)
			end
		end
	end
end
...
function Pointer.ArrowFrame_ShowSpellArrow(self,waypoint)
	...
	local found,name,_,texture,equiptype,atlas
	...
	if safe then
		atlas = node.atlas or (node.link and node.link.atlas)
		--if mode=="hearth" then
...
function Pointer:PointToNextTimer()
	if not self.ArrowFrame then return end
	...
	if (pathfollow=="smart" or pathfollow=="loose" or pathfollow=="smart2" or pathfollow=="smart3" or pathfollow=="smartbounce" or pathfollow=="bounce")
...
function Pointer:DoAudioCues(targetangle,playerangle,dist)
	...
	if Pointer:SecureGetUnitSpeed("player")==0 then laststoptime=t else lastmovetime=t end
...
function Pointer:CalculateETA(speed,dist,elapsed)
	...
	if #speeds>=speedtable_minlimit and avg>0 then
		...
		local spd,mntspd,flyspd,swimspd = Pointer:SecureGetUnitSpeed("player")
...
function Pointer.ArrowFrame_OnUpdate_Common(self,elapsed)
	...
	if not px then
		if Pointer:SecureGetUnitSpeed("player")>0 then
		...
		elseif waypoint.type=="route" then
			...
			then
				if GetTime()>noskip_time then
					...
					local path = Pointer.pointsets["path"]
					if (route and path) then
						ZGV:Debug("&waypoints Arrived at end of route, switching to waypath")
						ZGV:ShowWaypoints()
					end					
				end
			else
...
local function set_waypoints(points,worldsize,minisize,ptype,setname)
	...
	for k,point in ipairs(points) do 
		if point.x and not point.force_noway and not (point.IsVisible and not point:IsVisible()) then
			...
			data.in_set = setname
			data.source = point
			data.passivieminimap = point.passivieminimap
			...
			data.title = (
					...
					or (pathmode and (step and step.waypath and step.waypath.title and step.waypath.title))
					or (point.parentStep and point.parentStep:GetWayTitle())
					--or (step and step:GetWayTitle())
					or (point.map and point.x and ("%s%s %d,%d"):format(pathmode and "Path: " or "",ZGV.GetMapNameByID(point.map),point.x*100,point.y*100))
...
local function PathFoundHandler(state,path,ext,reason)
	...
	if state=="success" then
		...
		if not ZGV.LibTaxi:IsContinentKnown() and ZGV:GetPlayerPreciseLevel() >= 13 then
			local cfirst = path[1]
			...
			Pointer.ShowMissingTaxiInfo = (cfirst.m~=clast.m) and ((cdist or 999) > 500 or (ceta or 999) > 300)
		end
	...
	for _,goal in ipairs(ZGV.CurrentStep.goals) do
		if goal.action=="mapmarker" and (not goal.condition_visible or goal.condition_visible()) then
			if not goal.mapmarker then
				...
				point.icon = ZGV.Pointer.Icons[goal.mapicon or "mapmarker"] -- graydot, arrow,crosshair
				point.iconoverride = true
				point.map = goal.map
				point.f = goal.floor
				point.x = goal.x
				point.y = goal.y
				point.passivieminimap = true
...
tinsert(ZGV.startups,{"Pointer additional",function(self)
	Pointer:SetArrowSkin(ZGV.db.profile.arrowskin)
	Pointer:SetupArrow()
