...
FluxCompat.rotation_registry = {
    ...
    check_prerequisites = function(self, strategy, context)
        ...
        if strategy.spell then
            ...
            -- FIXED: is_ready() doesn't exist in Sylvanas, use cooldown_up() and is_castable_to_unit()
            if not strategy.spell:cooldown_up() then return false end
            local target = strategy.spell_target or context.target
            if target and not strategy.spell:is_castable_to_unit(target) then return false end
        end
        return true
    end,
...
-- /fluxlog slash command (WoW API - only register if available)
if _G.SlashCmdList then
    _G.SLASH_FLUXCOMPATLOG1 = "/fluxlog"
    _G.SLASH_FLUXCOMPATLOG2 = "/flog"
    _G.SlashCmdList["FLUXCOMPATLOG"] = function()
...
function FluxCompat.get_spell_mana_cost(spell)
    -- FIXED: spell:get_spell_power_cost() doesn't exist in Sylvanas
    -- Return 0 as safe default - mana checks should use is_usable() instead
    return 0
end
...
function FluxCompat.safe_self_cast(ability, label)
    ...
    -- FIXED: is_ready() doesn't exist, use cooldown_up() and is_usable()
    if not ability:cooldown_up() then return nil end
    if not ability:is_usable() then return nil end
...
function FluxCompat.try_cast_fmt(spell, target, prefix, name, info_fmt, ...)
    ...
    -- FIXED: is_ready() doesn't exist, use cooldown_up() and is_castable_to_unit()
    if not spell:cooldown_up() then return nil end
    if target and not spell:is_castable_to_unit(target) then return nil end
...
function FluxCompat.try_heal_cast(spell, target_unit, label)
    ...
    -- FIXED: is_ready() doesn't exist, use cooldown_up() and is_usable()
    if not spell:cooldown_up() then return nil end
    if not spell:is_usable() then return nil end
...
function FluxCompat.predict_effective_deficit(unitID, castTime)
    ...
    -- FIXED: unit:health_deficit() doesn't exist - calculate manually
    local deficit = unit:get_max_health() - unit:get_health()
    if deficit <= 0 then return 0 end
    
    local inc_heal = unit:get_incoming_heals(castTime) or 0
    -- FIXED: unit:get_heal_rate() doesn't exist - estimate from HoT data or return 0
    local hot_hps = 0  -- Simplified: HoT rate not directly available
    local hot_heal = hot_hps * castTime
    -- FIXED: unit:absorb() doesn't exist - use get_total_shield()
    local absorb = unit:get_total_shield() or 0
    -- FIXED: unit:get_incoming_damage_rate() doesn't exist - use get_incoming_damage()
    local inc_dmg_dps = (unit:get_incoming_damage(castTime) or 0) / castTime
...
function FluxCompat.safe_ability_cast(ability, target, label)
    ...
    -- FIXED: is_ready() doesn't exist, use cooldown_up() and is_castable_to_unit()
    if not ability:cooldown_up() then return nil end
    if target and not ability:is_castable_to_unit(target) then return nil end
...
function FluxCompat.create_combat_strategy(config)
    ...
    return {
        matches = function(context)
            ...
            return spell:cooldown_up() and (not target or spell:is_castable_to_unit(target))
        end,
...
function FluxCompat.register_trinket_middleware()
    ...
    FluxCompat.rotation_registry:register_middleware({
        ...
        matches = function(context)
            ...
            local s = context.settings
            -- FIXED: is_ready() doesn't exist on items, use cooldown_up() and is_usable()
            if s["trinkets.slot1_mode"] == "offensive" and Trinket1 and Trinket1:cooldown_up() and Trinket1:is_usable() then return true end
            if s["trinkets.slot2_mode"] == "offensive" and Trinket2 and Trinket2:cooldown_up() and Trinket2:is_usable() then return true end
            return false
        end,
        
        execute = function(icon, context)
            local s = context.settings
            if s["trinkets.slot1_mode"] == "offensive" and Trinket1 and Trinket1:cooldown_up() and Trinket1:is_usable() then
                return Trinket1:cast("player"), "[MW] Trinket 1 (Burst)"
            end
            if s["trinkets.slot2_mode"] == "offensive" and Trinket2 and Trinket2:cooldown_up() and Trinket2:is_usable() then
    ...
    FluxCompat.rotation_registry:register_middleware({
        ...
        matches = function(context)
            ...
            local s = context.settings
            -- FIXED: is_ready() doesn't exist on items, use cooldown_up() and is_usable()
            if s["trinkets.slot1_mode"] == "defensive" and Trinket1 and Trinket1:cooldown_up() and Trinket1:is_usable() then return true end
            if s["trinkets.slot2_mode"] == "defensive" and Trinket2 and Trinket2:cooldown_up() and Trinket2:is_usable() then return true end
            return false
        end,
        
        execute = function(icon, context)
            local s = context.settings
            if s["trinkets.slot1_mode"] == "defensive" and Trinket1 and Trinket1:cooldown_up() and Trinket1:is_usable() then
                return Trinket1:cast("player"), "[MW] Trinket 1 (Defensive)"
            end
            if s["trinkets.slot2_mode"] == "defensive" and Trinket2 and Trinket2:cooldown_up() and Trinket2:is_usable() then
...
function FluxCompat.update_frame_rate()
    ...
    -- Calculate FPS manually since GetFramerate() is not available in Sylvanas
    -- FPS = 1 / time_delta_between_frames
    local fps = 0
    if FluxCompat.frame_rate.last_update > 0 then
        local delta = now - FluxCompat.frame_rate.last_update
        if delta > 0 then
            fps = 1 / delta
        end
    end
    FluxCompat.frame_rate.current_fps = fps
