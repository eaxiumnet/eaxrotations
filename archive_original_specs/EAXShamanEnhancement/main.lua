...
local TOTEM_ROTATION = {
    { name = "windfury", id_field = "windfury_totem_id", toggle = menu.use_windfury_totem, label = "Windfury Totem", slot = 4, duration = 115 },
}
...
local function try_shamanistic_rage(me, ctx)
    if not (menu.use_cooldowns and menu.use_cooldowns:get_state()) or not runtime.shamanistic_rage_id then
        return false
    end
    ...
    if mana_pct <= (((menu.shamanistic_rage_mana and menu.shamanistic_rage_mana:get()) or 40) / 100) then
...
local function should_chain_lightning_weave()
    ...
    local now = _core_time()
    local clip_window = math.max((menu.swing_clip_ms and menu.swing_clip_ms:get()) or 200, profile.swing_clip_ms) / 1000
    return (now - runtime.last_stormstrike_at) >= clip_window
end

local function get_swing_clip_window_s()
    local profile = get_mode_profile()
    return math.max((menu.swing_clip_ms and menu.swing_clip_ms:get()) or 200, profile.swing_clip_ms) / 1000
end
...
local function try_chain_lightning_weave(me, target, ctx)
    if not (menu.use_chain_lightning_weave and menu.use_chain_lightning_weave:get_state()) or not runtime.chain_lightning_id or not target then
        return false
    end
    local can_cast = resource_gate.shaman.has_mana_pct(ctx, 0.15)
    if not can_cast then
...
local function try_shock(me, target, ctx)
    ...
    local shock_index = (menu.shock_mode and menu.shock_mode:get()) or 1
