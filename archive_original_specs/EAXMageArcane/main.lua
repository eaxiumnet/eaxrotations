...
-- FIXED: Corrected rotation priority for Arcane Blast stacking (3 maintain -> AM clear -> conserve)
...
local function try_mana_gem(me)
    ...
    -- FIXED: Add nil guard for menu reference
    if utils.get_mana_pct(me) > ((menu.mana_gem_pct and menu.mana_gem_pct:get()) or 20) then return false end
...
local function try_arcane_missiles(me, target)
    ...
    -- FIXED: Add nil guard for menu reference
    local low_mana = utils.get_mana_pct(me) <= (((menu.evocation_pct and menu.evocation_pct:get()) or 20) + 15)
...
local function try_fire_blast_move(me, target)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_fire_blast_move and menu.use_fire_blast_move:get_state()) then return false end
...
local function try_arcane_blast(me, target)
    ...
    if (menu.leveling_conserve_mana and menu.leveling_conserve_mana:get_state()) then
        ...
        arcane_blast_id = spell_downrank.select_dps_rank(spells.ARCANE_BLAST, target_level, player_level, mana_pct) or arcane_blast_id
    end
    if target_will_die_before_cast_finishes(me, target, arcane_blast_id, 0.35) then return false end
    if is_pending_cast(arcane_blast_id) or utils.is_spell_already_queued(arcane_blast_id) then return false end
    if not utils.can_cast_hostile(arcane_blast_id, me, target) then return false end
    local ab_stacks = utils.get_buff_stacks(me, spells.BUFF_ARCANE_BLAST)
    ...
    if utils.cast_target(arcane_blast_id, target, "Arcane Blast") then
        mark_pending_cast(arcane_blast_id, PENDING_CAST_TIMEOUT_S)
        note_cast()
                esp_renderer.on_cast(arcane_blast_id, "Arcane Blast", color.purple(220))
        return true
    end
...
-- FIXED: Removed duplicate try_remove_curse and try_arcane_explosion functions
    ...
    ooc_manager.on_update(me, menu, utils, {
        group_buffs = {
            { spell_id = runtime.ooc_arcane_intellect_id,
               ...
               -- FIXED: Add nil guard for menu reference
               toggle = (menu.ooc_group_buff and menu.ooc_group_buff:get_state()) or false },
        },
    })
