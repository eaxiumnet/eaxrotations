...
local function ensure_righteous_fury(me)
    -- FIXED: Proper nil guard
    if not (menu.use_righteous_fury and menu.use_righteous_fury:get_state()) or not runtime.righteous_fury_id then
        return false
    end
...
-- FIXED: Holy Shield maintenance 8 charges per README
local function ensure_holy_shield(me, target)
    -- FIXED: Proper nil guard
    if not (menu.use_holy_shield and menu.use_holy_shield:get_state()) or not runtime.holy_shield_id then
        return false
    end
...
local function ensure_aura_upkeep(me)
    -- FIXED: Proper nil guard
    if not (menu.use_aura and menu.use_aura:get_state()) then
        return false
    end
...
local function try_consecration(me, enemy_count)
    ...
    -- FIXED: Proper nil guard
    if not (menu.use_consecration and menu.use_consecration:get_state()) or not runtime.consecration_id then
        return false
    end

    -- FIXED: Proper nil guard
    local consecration_enemy_count = (menu.consecration_enemy_count and menu.consecration_enemy_count:get()) or 3
...
local function get_assigned_seal_profile(me, target)
	if not (menu.use_judgement and menu.use_judgement:get_state()) or not runtime.judgement_id then
		return nil, nil, nil
	end
...
-- FIXED: Avenger's Shield priority per README
local function try_avengers_shield(me, target, mode)
    -- FIXED: Proper nil guard
    if not (menu.use_avengers_shield and menu.use_avengers_shield:get_state()) or not runtime.avengers_shield_id then
        return false
    end
...
local function try_judgement(me, target)
	if not (menu.use_judgement and menu.use_judgement:get_state()) or not runtime.judgement_id or not target then
		return false
	end
...
local function try_lay_on_hands_emergency(me)
    ...
    -- FIXED: Proper nil guard
    local hp_threshold = ((menu.use_lay_on_hands_hp_pct and menu.use_lay_on_hands_hp_pct:get()) or 20) / 100
...
local function on_update()
    ...
    if interrupt_manager.should_interrupt(target) then
        -- FIXED: Proper nil guard
        if menu.use_interrupt and (menu.use_interrupt and menu.use_interrupt:get_state()) and interrupt_manager.try_interrupt(me, target, "paladin", utils) then
            return
        end
    end
    ...
    -- FIXED: Rotation priority per README - Holy Shield maint 8 charges -> Avenger's Shield -> Judgement
    ...
    local enemy_count = utils.count_enemies_within_radius(me, (menu.consecration_radius and menu.consecration_radius:get()) or 10)
...
try_ooc_group_blessings = function(me)
    -- FIXED: Proper nil guard
    if not (menu.ooc_group_buff and menu.ooc_group_buff:get_state()) then
        return false
    end
    if not me or not me:is_valid() or me:is_dead() then
        return false
    ...
    actions = {
        ...
        interrupt_control = {
            handler = function(_, action_deps)
                ...
                -- FIXED: Proper nil guard
                return (menu.use_interrupt and (menu.use_interrupt and menu.use_interrupt:get_state())) and interrupt_manager.try_interrupt(action_deps.me, interrupt_target, "paladin", utils)
            end,
        },
