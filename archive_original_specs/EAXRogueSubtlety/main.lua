...
local function try_premeditation(me, target)
    if not menu.use_premeditation or not menu.use_premeditation:get_state() then
        return false
    end
...
local function try_cheap_shot(me, target, ctx)
    if not (menu.use_cheap_shot and menu.use_cheap_shot:get_state()) or not should_use_cheap_shot() then
        return false
    end
...
local function try_ambush(me, target, ctx)
    if not menu.use_ambush or not menu.use_ambush:get_state() then
        return false
    end
    if current_mode() ~= "solo" then
        return false
...
local function try_shadowstep(me, target)
    if not menu.use_shadowstep or not menu.use_shadowstep:get_state() then
        return false
    end
...
local function try_preparation(me, target)
    if not menu.use_preparation or not menu.use_preparation:get_state() then return false end
...
local function try_slice_and_dice(me, target, ctx)
    ...
    local regular_min_combo_points = math.min((menu.finisher_combo_points and menu.finisher_combo_points:get()) or 5, 3)
...
local function try_rupture(me, target, ctx)
    ...
    local min_combo_points = (menu.finisher_combo_points and menu.finisher_combo_points:get()) or 5
...
local function try_eviscerate(me, target, ctx)
    ...
    if runtime.combo_points < ((menu.finisher_combo_points and menu.finisher_combo_points:get()) or 5) then
        return false
    end
...
local function try_backstab(me, target, ctx)
    if not menu.use_backstab or not menu.use_backstab:get_state() then
        return false
    end
...
local function try_hemorrhage(me, target, ctx)
    if not menu.use_hemorrhage or not menu.use_hemorrhage:get_state() then
        return false
    end
