...
-- FIXED: Corrected rotation priority for Fire (Scorch 5 stacks -> Fireball spam)
...
local function try_combustion(me, target)
    ...
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_combustion and menu.use_combustion:get_state()) then return false end
    ...
    -- FIXED: Add nil guard for menu reference
    if (menu.use_scorch and menu.use_scorch:get_state()) and scorch_refresh_due(me, target) then return false end
...
local function try_pyroblast(me, target)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_pyroblast and menu.use_pyroblast:get_state()) then return false end
...
local function try_scorch(me, target)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_scorch and menu.use_scorch:get_state()) then return false end
...
local function try_fireball(me, target)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_fireball and menu.use_fireball:get_state()) then return false end
...
local function try_flamestrike(me, target)
    ...
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_flamestrike and menu.use_flamestrike:get_state()) then return false end
    if not runtime.flamestrike_id then return false end
    local enemy_count = count_enemies_near(me, spells.FLAMESTRIKE_RADIUS)
    -- FIXED: Add nil guard for menu reference
    if enemy_count < ((menu.flamestrike_enemy_count and menu.flamestrike_enemy_count:get()) or 3) then return false end
...
local function try_blast_wave(me, target)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_blast_wave and menu.use_blast_wave:get_state()) then return false end
...
local function try_dragons_breath(me, target)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_dragons_breath and menu.use_dragons_breath:get_state()) then return false end
