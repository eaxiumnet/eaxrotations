...
function utils.can_cast_target(spell_id, me, target)
    if not spell_id then return false, "no_spell_id" end
    if not target or not target:is_valid() then return false, "invalid_target" end
    if not me or not me:is_valid() then return false, "invalid_caster" end
    
    local castable, reason = spell_helper:is_spell_castable(spell_id, me, target)
    if not castable then
        return false, reason or "not_castable"
    end
    ...
    if not core.spell_book.is_spell_in_range(spell_id, target) then
