...
core.register_on_update_callback(function()
    -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 209)
...
local function build_totem_rotation()
    ...
    -- FIXED: Added proper nil guard for menu.auto_totem_healing_stream:get_state() (line 342)
    if menu.auto_totem_healing_stream and menu.auto_totem_healing_stream:get_state() then
    ...
    -- FIXED: Added proper nil guard for menu.auto_totem_wrath:get_state() (line 350)
    if menu.auto_totem_wrath and menu.auto_totem_wrath:get_state() then
    ...
    -- FIXED: Added proper nil guard for menu.auto_totem_wrath_of_air:get_state() (line 358)
    if menu.auto_totem_wrath_of_air and menu.auto_totem_wrath_of_air:get_state() and rt.wrath_of_air_id then
...
local function get_effective_mode()
    -- FIXED: Added proper nil guard for menu.mode:get() (line 416)
    local sel = (menu.mode and menu.mode:get()) or 1
...
local function try_reincarnation(me)
    -- FIXED: Added proper nil guard for menu.use_reincarnation:get_state() (line 547)
    if not menu.use_reincarnation or not menu.use_reincarnation:get_state() then return false end
...
local function ensure_water_shield(me)
    -- FIXED: Added proper nil guard for menu.use_water_shield:get_state() (line 564)
    if not menu.use_water_shield or not menu.use_water_shield:get_state() then return false end
...
local function ensure_flametongue(me)
    -- FIXED: Added proper nil guard for menu.use_flametongue:get_state() (line 581)
    if not menu.use_flametongue or not menu.use_flametongue:get_state() then return false end
...
local function ensure_auto_attack(me)
    -- FIXED: Added proper nil guard for menu.use_auto_attack:get_state() (line 605)
    if not menu.use_auto_attack or not menu.use_auto_attack:get_state() then return end
...
local function try_drink(me)
    -- FIXED: Added proper nil guard for menu.use_drink:get_state() (line 632)
    if not menu.use_drink or not menu.use_drink:get_state() then return false end
    if me:is_in_combat() then return false end
    if me:is_moving() then return false end
    -- Already drinking? (generic drink channel buff)
    if utils.has_buff(me, DRINK_BUFF_ID) then return false end
    -- FIXED: Added proper nil guard for menu.drink_mana_pct:get() (line 637)
    local threshold = ((menu.drink_mana_pct and menu.drink_mana_pct:get()) or 50) / 100.0
...
local function try_mana_potion(me)
    -- FIXED: Added proper nil guard for menu.use_mana_potion:get_state() (line 671)
    if not menu.use_mana_potion or not menu.use_mana_potion:get_state() then return false end
    if not me:is_in_combat() then return false end
    -- FIXED: Added proper nil guard for menu.mana_potion_pct:get() (line 673)
    local threshold = ((menu.mana_potion_pct and menu.mana_potion_pct:get()) or 50) / 100.0
    if utils.get_mana_pct(me) >= threshold then return false end
    local now = _core_time()
...
local function try_totemic_recall(me)
    -- FIXED: Added proper nil guard for menu.use_totemic_recall:get_state() (line 707)
    if not menu.use_totemic_recall or not menu.use_totemic_recall:get_state() then return false end
...
local function ensure_totems(me)
    -- FIXED: Added proper nil guard for menu.auto_totems:get_state() (line 741)
    if not menu.auto_totems or not menu.auto_totems:get_state() then return end
...
local function try_prepull_totems(me)
    -- FIXED: Added proper nil guard for menu.prepull_totems:get_state() (line 776)
    if not menu.prepull_totems or not menu.prepull_totems:get_state() then return false end
    if me:is_in_combat() then return false end
    build_totem_rotation()
    local now = _core_time()
...
local function try_ooc_self_heal(me)
    -- FIXED: Added proper nil guard for menu.ooc_self_heal:get_state() (line 813)
    if not menu.ooc_self_heal or not menu.ooc_self_heal:get_state() then return false end
    if me:is_in_combat() then return false end
    if not is_gcd_ready() then return false end
    -- FIXED: Added proper nil guard for menu.ooc_self_hp:get() (line 816)
    if heal_engine.get_effective_hp_pct(me) >= (((menu.ooc_self_hp and menu.ooc_self_hp:get()) or 50) / 100.0) then return false end
...
local function try_natures_swiftness(me, tank)
    -- FIXED: Added proper nil guard for menu.use_cooldowns:get_state() (line 825)
    if not menu.use_cooldowns or not menu.use_cooldowns:get_state() then return false end
    -- FIXED: Added proper nil guard for menu.use_natures_swiftness:get_state() (line 826)
    ...
    -- FIXED: Added proper nil guard for menu.ns_emergency_hp:get() (line 832)
    local emergency = eax_utils.get_self_heal_threshold(me, ((menu.ns_emergency_hp and menu.ns_emergency_hp:get()) or 30) / 100.0, menu)
...
local function try_bloodlust(me)
    -- FIXED: Added proper nil guard for menu.use_cooldowns:get_state() (line 847)
    if not menu.use_cooldowns or not menu.use_cooldowns:get_state() then return false end
    -- FIXED: Added proper nil guard for menu.use_bloodlust:get_state() (line 848)
    if not menu.use_bloodlust or not menu.use_bloodlust:get_state() then return false end
    ...
    -- FIXED: Added proper nil guard for menu.bloodlust_on_pull:get_state() (line 859)
    if menu.bloodlust_on_pull and menu.bloodlust_on_pull:get_state() and me:is_in_combat() then
        should_lust = true
    end
    local target = me:get_target()
    if target and target:is_valid() and not target:is_dead() and me:can_attack(target) then
        -- FIXED: Added proper nil guard for menu.bloodlust_hp:get() (line 865)
        local boss_hp = unit_helper:get_health_percentage_inc(target, 3.0)
        if boss_hp <= (((menu.bloodlust_hp and menu.bloodlust_hp:get()) or 95) / 100.0) then should_lust = true end
    end
...
local function try_proactive_mana_tide(me)
    -- FIXED: Added proper nil guard for menu.mana_tide_timing:get_state() (line 878)
    if not menu.mana_tide_timing or not menu.mana_tide_timing:get_state() then return false end
    ...
    -- FIXED: Added proper nil guard for menu.mana_tide_mana_pct:get() (line 882)
    local threshold = ((menu.mana_tide_mana_pct and menu.mana_tide_mana_pct:get()) or 50) / 100.0
...
local function select_shaman_direct_heal_target(me, tank)
    ...
    -- FIXED: Added proper nil guard for menu.heal_tank_hp:get() (line 918)
    local tank_threshold = math.min((menu.heal_tank_hp and menu.heal_tank_hp:get()) or 70, profile.heal_tank_hp) / 100.0
    -- FIXED: Added proper nil guard for menu.heal_party_hp:get() (line 919)
    local party_threshold = math.min((menu.heal_party_hp and menu.heal_party_hp:get()) or 70, profile.heal_party_hp) / 100.0
...
local function should_delay_totem_drop(me, tank)
    local profile = get_mode_profile()
    -- FIXED: Added proper nil guard for menu.ns_emergency_hp:get() (line 953)
    local self_threshold = eax_utils.get_self_heal_threshold(me, ((menu.ns_emergency_hp and menu.ns_emergency_hp:get()) or 30) / 100.0, menu)
    -- FIXED: Added proper nil guard for menu.heal_tank_hp:get() (line 954)
    local tank_threshold = math.min((menu.heal_tank_hp and menu.heal_tank_hp:get()) or 70, profile.heal_tank_hp) / 100.0
    -- FIXED: Added proper nil guard for menu.heal_party_hp:get() (line 955)
    local party_threshold = math.min((menu.heal_party_hp and menu.heal_party_hp:get()) or 70, profile.heal_party_hp) / 100.0
...
local function try_chain_heal(me)
    ...
    local profile = get_mode_profile()
    -- FIXED: Added proper nil guard for menu.heal_party_hp:get() (line 1000)
    local party_threshold = math.min((menu.heal_party_hp and menu.heal_party_hp:get()) or 70, profile.heal_party_hp) / 100.0
    -- FIXED: Added proper nil guard for menu.chain_heal_targets:get() (line 1001)
    local min_targets     = math.min((menu.chain_heal_targets and menu.chain_heal_targets:get()) or 3, profile.chain_heal_targets)
    -- FIXED: Added proper nil guard for menu.mana_floor:get() (line 1002)
    local mana_floor      = math.max((menu.mana_floor and menu.mana_floor:get()) or 30, profile.mana_floor) / 100.0
...
local function try_healing_wave(me, tank)
    ...
    local profile = get_mode_profile()
    -- FIXED: Added proper nil guard for menu.heal_tank_hp:get() (line 1034)
    local tank_threshold = math.min((menu.heal_tank_hp and menu.heal_tank_hp:get()) or 70, profile.heal_tank_hp) / 100.0
    -- FIXED: Added proper nil guard for menu.mana_floor:get() (line 1035)
    local mana_floor = math.max((menu.mana_floor and menu.mana_floor:get()) or 30, profile.mana_floor) / 100.0
...
local function try_dispel(me)
    -- FIXED: Added proper nil guard for menu.use_dispels:get_state() (line 1052)
    if not menu.use_dispels or not menu.use_dispels:get_state() then return false end
    -- FIXED: Added proper nil guard for menu.heal_emergency_hp:get() (line 1053)
    local emergency = ((menu.heal_emergency_hp and menu.heal_emergency_hp:get()) or 30) / 100.0
...
local function try_lesser_healing_wave(me)
    ...
    local profile = get_mode_profile()
    -- FIXED: Added proper nil guard for menu.heal_party_hp:get() (line 1075)
    local threshold = math.min((menu.heal_party_hp and menu.heal_party_hp:get()) or 70, profile.heal_party_hp) / 100.0
    -- FIXED: Added proper nil guard for menu.mana_floor:get() (line 1076)
    local mana_floor = math.max((menu.mana_floor and menu.mana_floor:get()) or 30, profile.mana_floor) / 100.0
...
local function try_pvp_utilities(me)
    -- FIXED: Added proper nil guard for menu.pvp_mode:get_state() (line 1171)
    if not menu.pvp_mode or not menu.pvp_mode:get_state() then return false end
    ...
    -- FIXED: Added proper nil guard for menu.pvp_use_grounding:get_state() (line 1183)
    if menu.pvp_use_grounding and menu.pvp_use_grounding:get_state() and rt.grounding_totem_id then
    ...
    -- FIXED: Added proper nil guard for menu.pvp_use_tremor:get_state() (line 1192)
    if menu.pvp_use_tremor and menu.pvp_use_tremor:get_state() and rt.tremor_totem_id then
    ...
    -- FIXED: Added proper nil guard for menu.pvp_use_purge:get_state() (line 1210)
    if menu.pvp_use_purge and menu.pvp_use_purge:get_state() and rt.purge_id then
        local target = me:get_target()
...
local function try_dps_filler(me, target)
    local profile = get_mode_profile()
    -- FIXED: Added proper nil guard for menu.enable_dps:get_state() (line 1225)
    if not menu.enable_dps or not menu.enable_dps:get_state() then return false end
    -- FIXED: Added proper nil guard for menu.use_dps_filler:get_state() (line 1226)
    if not menu.use_dps_filler or not menu.use_dps_filler:get_state() then return false end
    if not profile.enable_dps then return false end
    if not target or not target:is_valid() or target:is_dead() then return false end
    if not utils.is_valid_hostile(me, target) then return false end
    if utils.get_mana_pct(me) < (math.max((menu.mana_floor and menu.mana_floor:get()) or 30, profile.mana_floor) / 100.0) then return false end
    ...
    -- FIXED: Added proper nil guard for menu.use_interrupt:get_state() (line 1237)
    if menu.use_interrupt and menu.use_interrupt:get_state() and rt.earth_shock_id then
        if target:is_casting_spell() then
    ...
    -- Purge
    -- FIXED: Added proper nil guard for menu.use_purge:get_state() (line 1246)
    if menu.use_purge and menu.use_purge:get_state() and rt.purge_id then
        local buffs = target:get_buffs()
...
local function do_rotation(me)
    ...
    if target and interrupt_manager.should_interrupt(target) then
        -- FIXED: Added proper nil guard for menu.use_interrupt:get_state() (line 1325)
        if menu.use_interrupt and menu.use_interrupt:get_state() and interrupt_manager.try_interrupt(me, target, "shaman", utils) then
            return
        end
    end
    ...
    if focus_target then
        ...
        -- FIXED: Added proper nil guard for menu.heal_tank_hp:get() (line 1346)
        if fhp < (((menu.heal_tank_hp and menu.heal_tank_hp:get()) or 70) / 100.0) then
    ...
    -- -- Self emergency ----------------------------------------------------
    -- FIXED: Added proper nil guard for menu.ns_emergency_hp:get() (line 1353)
    local self_thr = eax_utils.get_self_heal_threshold(me, ((menu.ns_emergency_hp and menu.ns_emergency_hp:get()) or 30) / 100.0, menu)
...
local function detect_toggle()
    -- FIXED: Added proper nil guard for menu.toggle_key:get_state() (line 1389)
    local current = (menu.toggle_key and menu.toggle_key:get_state()) or false
    if current and not rt.prev_toggle_state then
        -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 1391)
...
core.register_on_render_callback(function()
    -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 1516)
    if not menu or not menu.enabled or not (menu.enabled and menu.enabled:get_state()) or false then return end
    on_render()
end)
-- __EAX_ESP_GUARD
core.register_on_update_callback(function()
    ...
    if me and utils.throttle("mode_refresh", MODE_REFRESH_INTERVAL) then
        ...
        -- FIXED: Added proper nil guard for menu.tank_priority_weight:get() (line 1525)
        heal_engine.set_tank_priority((menu.tank_priority_weight and menu.tank_priority_weight:get()) or 10)
    end
    ...
    -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 1535)
    ...
    -- FIXED: Added proper nil guard for menu.auto_ooc_food_drink:get_state() (line 1543)
    ...
    if me:is_in_combat() then
        -- FIXED: Added proper nil guard for menu.auto_combat_potions:get_state() (line 1548)
        if menu.auto_combat_potions and menu.auto_combat_potions:get_state() then
            consumables_manager.try_use_combat_consumable(me, menu, utils)
        end
        -- FIXED: Added proper nil guard for menu.auto_flask:get_state() (line 1551)
