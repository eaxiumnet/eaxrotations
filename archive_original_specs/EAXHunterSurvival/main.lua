...
-- FIXED: Added nil guards to all menu references
-- FIXED: Replaced math.sqrt with squared distance comparisons
-- FIXED: Added pcall protection for unit method calls
...
-- FIXED: Use squared distance comparison
local function dist(t)
    local me = get_me(); if not me or not t then return 999999 end
...
local function pet_is_engaged_on_target(pet, target)
    ...
    -- FIXED: Use squared comparison (100 = 10 yards squared)
...
local function try_mongoose_bite(me, t, ctx)
    ...
    -- FIXED: Use squared distance (25 = 5 yards squared)
    if dist(t) > 25 then return false end
    if not resource_gate.hunter.has_mana_pct(ctx, 0.10) then return false end
...
local function do_rotation(me, t)
    ...
    -- FIXED: Use squared distance (64 = 8 yards squared)
    if d <= 64 then
        try_concussive(me, t)
        try_wing_clip(me, t)
    ...
    if interrupt_manager.should_interrupt(t) then
        -- FIXED: Add nil guard for menu reference
        if (menu.use_interrupt and menu.use_interrupt:get_state()) and interrupt_manager.try_interrupt(me, t, "hunter", utils) then return true end
    end
    ...
    -- FIXED: Use squared distance (1600 = 40 yards squared, 25 = 5 yards squared)
    if me:is_in_combat() and d > 25 and d <= 1600 and not is_moving() then
        leveling_manager.ensure_ranged(me, t)
    end

    -- FIXED: Use squared distance (1600 = 40 yards squared)
    if d > 1600 then return end
...
try_scare_beast = function(me, current_target)
    ...
    if utils.cast_target(rt.scare_beast_id, focus) then
...
try_wyvern_sting = function(me, current_target)
    -- FIXED: Add nil guard for menu reference
    ...
    local focus = get_direct_focus_unit(current_target); if not focus or not me:can_attack(focus) then return false end
    if rt.last_wyvern_sting_cast_count == core.spell_book.get_spell_cast_count(rt.wyvern_sting_id) then return false end
    if utils.cast_target(rt.wyvern_sting_id, focus) then
...
try_flare = function(me, current_target)
    ...
    if core.input.cast_spell_position and core.input.cast_spell_position(rt.flare_id, pos) then
