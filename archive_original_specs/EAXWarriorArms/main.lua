...
local function try_overpower(me, target, ctx)
    ...
    local can_use_smart = should_use_overpower_smart(me, target, ctx, ctx.self.rage or 0)
...
local function try_recklessness(me)
    ...
    -- Clear pending flag if we actually entered Berserker Stance
    if in_berserker then
        runtime.pending_recklessness_berserker = false
    end
    ...
    -- If we're waiting for Berserker Stance to apply, don't spam
    if not in_berserker and runtime.pending_recklessness_berserker then
        return false
    end
