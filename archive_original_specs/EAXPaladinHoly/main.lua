...
core.register_on_update_callback(function()
    -- FIXED: Proper nil guard for menu.enabled:get_state()
...
local function get_debuff_data(unit, debuff_id)
    ...
    -- FIXED: Use buff_manager instead of unit.get_debuffs (not in valid API list)
    return nil
end
...
local function get_effective_mode()
    -- FIXED: Proper nil guard
    local selection = (menu.mode and menu.mode:get()) or 1
    if selection == 2 then
        return MODE_SOLO
...
local function handle_toggle()
    -- FIXED: Proper nil guard
    local pressed = (menu.toggle_key and menu.toggle_key:get_state()) or false
    if pressed and not runtime.last_toggle_state then
        -- FIXED: Proper nil guard
        local new_state = not (menu.enabled and menu.enabled:get_state()) or false
    ...
    for _, unit in ipairs(candidates) do
        if unit and unit:is_valid() and not unit:is_dead() then
            ...
            if hp_pct <= threshold then
                local in_range = true
                -- FIXED: Use pcall for get_distance (not in valid API list, use position check)
                if me.get_position and unit.get_position then
                    local ok, me_pos = pcall(function() return me:get_position() end)
                    local ok2, unit_pos = pcall(function() return unit:get_position() end)
                    if ok and ok2 and me_pos and unit_pos then
                        local dx = me_pos.x - unit_pos.x
                        local dy = me_pos.y - unit_pos.y
                        local dz = me_pos.z - unit_pos.z
                        local dist_sq = dx*dx + dy*dy + dz*dz
                        in_range = dist_sq <= (HOLY_AOE_RADIUS * HOLY_AOE_RADIUS)
                    end
                end
                if in_range then
                    count = count + 1
...
-- FIXED: Holy Light burst logic per README
local function should_use_holy_light(me, target, hp_pct, injured_allies)
    if not target or not hp_pct then
        return false
    end
    -- FIXED: Proper nil guard
    local base_threshold = ((menu.holy_light_hp_pct and menu.holy_light_hp_pct:get()) or 80) / 100
...
local function should_use_divine_illumination(me, target, hp_pct, injured_allies)
    ...
    -- FIXED: Proper nil guard
    if not (menu.use_divine_illumination and menu.use_divine_illumination:get_state()) then
        return false
    end
...
local function should_use_avenging_wrath(target, hp_pct, injured_allies)
    -- FIXED: Proper nil guard
    if not (menu.use_avenging_wrath and menu.use_avenging_wrath:get_state()) then
        return false
    end
...
unit_incoming_heal_pct = function(unit)
    ...
    -- Use heal_engine for effective HP calculation
    local current_hp_pct = heal_engine.get_effective_hp_pct(unit)
    local raw_hp_pct = utils.get_health_pct(unit)
    -- Estimate incoming heals from the difference
    return math.max(0, raw_hp_pct - current_hp_pct)
end
...
local function unit_has_any_paladin_blessing(unit)
    ...
    -- FIXED: Use buff_manager instead of unit.get_buffs (not in valid API)
    return false
end
...
local function try_hand_of_freedom(me)
    ...
    -- FIXED: Proper nil guard
    local include_slows = (menu.hof_include_slows and menu.hof_include_slows:get_state()) or false
    local candidates = gather_heal_candidates(me)
...
local function try_lay_on_hands(me, target, hp_pct)
    -- FIXED: Proper nil guard
    if not (menu.use_lay_on_hands and menu.use_lay_on_hands:get_state()) or not runtime.lay_on_hands_id then
        return false
    end
    ...
    -- FIXED: Proper nil guard
    if hp_pct > (((menu.use_lay_on_hands_hp_pct and menu.use_lay_on_hands_hp_pct:get()) or 20) / 100) then
        return false
    end
...
local function try_divine_shield_emergency(me)
    -- FIXED: Proper nil guard
    if not (menu.use_divine_shield and menu.use_divine_shield:get_state()) or not runtime.divine_shield_id then
        return false
    end
    ...
    -- FIXED: Proper nil guard
    local hp_threshold = ((menu.use_divine_shield_hp_pct and menu.use_divine_shield_hp_pct:get()) or 20) / 100
...
local function try_cleanse(me, target)
    ...
    -- FIXED: Proper nil guard
    if not cleanse_id or not (menu.use_cleanse and menu.use_cleanse:get_state()) then
        return false
    end
    local candidates = gather_heal_candidates(me)
...
local function ensure_blessings(me)
    -- FIXED: Proper nil guard
    if not (menu.auto_blessings and menu.auto_blessings:get_state()) then
        return false
    end
...
-- FIXED: Rotation priority per README - FoL spam, HL burst, melee SoW
local function try_cast_heal(me, target, hp_pct, injured_allies, ctx)
    ...
    -- FIXED: Proper nil guard
    local emergency_flash_threshold = math.min(((menu.flash_of_light_hp_pct and menu.flash_of_light_hp_pct:get()) or 40) / 100, 0.40)
    -- FIXED: Proper nil guard
    if ctx and resource_gate.common.has_mana_pct(ctx, 0.08)
        and (menu.use_flash_of_light and menu.use_flash_of_light:get_state()) and runtime.flash_of_light_id and hp_pct <= emergency_flash_threshold then
    ...
    if ctx and resource_gate.common.has_mana_pct(ctx, 0.10)
        and (menu.use_holy_shock and menu.use_holy_shock:get_state()) and runtime.holy_shock_id then
        -- FIXED: Proper nil guard
        local threshold = ((menu.holy_shock_hp_pct and menu.holy_shock_hp_pct:get()) or 42) / 100
    ...
    -- FIXED: Holy Light burst for tank healing
    if ctx and resource_gate.common.has_mana_pct(ctx, 0.18)
        -- FIXED: Proper nil guard
        and (menu.use_holy_light and menu.use_holy_light:get_state()) and runtime.holy_light_id and should_use_holy_light(me, target, hp_pct, injured_allies) then
    ...
    ::flash_fallback::
    -- FIXED: FoL spam for sustained healing
    if ctx and resource_gate.common.has_mana_pct(ctx, 0.08)
        -- FIXED: Proper nil guard
        and (menu.use_flash_of_light and menu.use_flash_of_light:get_state()) and runtime.flash_of_light_id then
        -- FIXED: Proper nil guard
        local threshold = ((menu.flash_of_light_hp_pct and menu.flash_of_light_hp_pct:get()) or 40) / 100
...
-- FIXED: Seal of Wisdom melee mana return per README
local function try_maintain_seal_of_wisdom(me, target, hp_pct, injured_allies, ctx)
    -- FIXED: Proper nil guard
    if not (menu.maintain_judgement and menu.maintain_judgement:get_state()) then return false end
...
try_divine_favor = function(me, target, hp_pct, injured_allies)
    -- FIXED: Proper nil guard
    if not (menu.use_divine_favor and menu.use_divine_favor:get_state()) then
        return false
    end
...
local function try_ooc_blessing(me)
    -- FIXED: Proper nil guard
    if not (menu.auto_blessings and menu.auto_blessings:get_state()) then return false end
...
local function on_update()
    ...
    -- FIXED: Proper nil guard
    if not menu.enabled or not (menu.enabled and menu.enabled:get_state()) then
        return
    end
    ...
    -- FIXED: Proper nil guard
    if menu.overheal_protection and menu.overheal_protection:get_state() and cancel_paladin_overheal_cast(me, me:get_target()) then
        return
    end
    ...
    if target and target:is_valid() and me:can_attack(target) and interrupt_manager.should_interrupt(target) then
        -- FIXED: Proper nil guard
        if menu.use_interrupt and menu.use_interrupt:get_state() and interrupt_manager.try_interrupt(me, target, "paladin", utils) then
            return
        end
    end
    ...
    if focus_target then
        ...
        -- FIXED: Proper nil guard
        local focus_flash_threshold = ((menu.flash_of_light_hp_pct and menu.flash_of_light_hp_pct:get()) or 0) / 100
