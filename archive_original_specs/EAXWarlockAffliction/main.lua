...
core.register_on_update_callback(function()
    -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 197)
...
-- FIXED: Removed duplicate target_has_utility_curse function (was defined twice at lines 293 and 543)
-- Keeping only one definition at line 293
...
local function get_effective_mode()
    -- FIXED: Added proper nil guard for menu.mode:get() (line 368)
...
local function handle_toggle()
    -- FIXED: Added proper nil guard for menu.toggle_key:get_state() (line 388)
    local current = (menu.toggle_key and menu.toggle_key:get_state()) or false
    if current and not runtime.prev_toggle_state then
        -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 390)
...
local function try_curse_of_elements(me, target)
    -- FIXED: Added proper nil guard for menu.use_curse_of_elements:get_state() (line 445)
...
local function try_death_coil(me, target)
    -- FIXED: Added proper nil guard for menu.use_death_coil:get_state() (line 460)
...
local function try_seed_of_corruption(me, target, enemy_count)
    ...
    -- FIXED: Added proper nil guard for menu.use_seed_of_corruption:get_state() (line 496)
...
local function try_refresh_dots(me, target)
    -- FIXED: Added proper nil guard for menu.use_unstable_affliction:get_state() (line 511)
    if menu.use_unstable_affliction and menu.use_unstable_affliction:get_state()
    ...
    -- FIXED: Added proper nil guard for menu.use_corruption:get_state() (line 518)
    if menu.use_corruption and menu.use_corruption:get_state()
    ...
    -- FIXED: Added proper nil guard for menu.use_siphon_life:get_state() (line 526)
    if menu.use_siphon_life and menu.use_siphon_life:get_state() then
        ...
        if ua_stable and corr_stable and dot_manager.can_refresh_dot(target, spells.SIPHON_LIFE, runtime.siphon_life_id, utils.get_debuff_remaining_ms) then
            ...
            -- FIXED: Added proper nil guard for menu.use_isb_gated_siphon:get_state() (line 531)
            local isb_gated = menu.use_isb_gated_siphon and menu.use_isb_gated_siphon:get_state()
...
local function should_hold_filler_for_dot_refresh(target)
    ...
    -- FIXED: Added proper nil guard for menu.use_unstable_affliction:get_state() (line 576)
    if menu.use_unstable_affliction and menu.use_unstable_affliction:get_state() and runtime.unstable_affliction_id
        and utils.get_debuff_remaining_ms(target, spells.UNSTABLE_AFFLICTION) <= refresh_window_ms then
        return true
    end
    -- FIXED: Added proper nil guard for menu.use_corruption:get_state() (line 580)
    if menu.use_corruption and menu.use_corruption:get_state() and runtime.corruption_id
        and utils.get_debuff_remaining_ms(target, spells.CORRUPTION) <= refresh_window_ms then
        return true
    end
    -- FIXED: Added proper nil guard for menu.use_siphon_life:get_state() (line 584)
    if menu.use_siphon_life and menu.use_siphon_life:get_state() and runtime.siphon_life_id
...
local function get_selected_curse(mode, target)
    ...
    -- FIXED: Added proper nil guard for menu.cod_fallback_ttd:get_value() (line 598)
    local threshold = (menu.cod_fallback_ttd and menu.cod_fallback_ttd:get_value()) or 60
    ...
    -- FIXED: Added proper nil guard for menu.use_cod_to_coa_fallback:get_state() (line 602)
    if menu.use_cod_to_coa_fallback and menu.use_cod_to_coa_fallback:get_state() and runtime.curse_doom_id and type(ttd_s) == "number" and ttd_s < threshold then
        -- Target will die before CoD tick - fall back to CoA below
    -- FIXED: Added proper nil guard for menu.prefer_doom:get_state() (line 604)
    elseif menu.prefer_doom and menu.prefer_doom:get_state() and runtime.curse_doom_id and type(ttd_s) == "number" and ttd_s >= threshold then
...
local function try_apply_curse(me, target, mode)
    -- FIXED: Added proper nil guard for menu.use_curse:get_state() (line 620)
    if not menu.use_curse or not menu.use_curse:get_state() then return false end
    ...
    -- FIXED: Added proper nil guard for menu.use_amplify_curse:get_state() (line 635)
    if menu.use_amplify_curse and menu.use_amplify_curse:get_state()
    ...
    then
        ...
        -- FIXED: Added proper nil guard for menu.amplify_before_cod:get_state() (line 642)
        -- FIXED: Added proper nil guard for menu.amplify_before_coa:get_state() (line 643)
        if (is_cod and menu.amplify_before_cod and menu.amplify_before_cod:get_state())
            or (is_coa and menu.amplify_before_coa and menu.amplify_before_coa:get_state())
        then
...
local function try_execute(me, target)
    ...
    -- FIXED: Added proper nil guard for menu.use_drain_soul:get_state() (line 660)
...
local function try_filler(me, target)
    ...
    -- FIXED: Added proper nil guard for menu.use_shadow_bolt:get_state() (line 688)
    if menu.use_shadow_bolt and menu.use_shadow_bolt:get_state() then
        -- Leveling: use appropriate spell rank
        local shadow_bolt_id = runtime.shadow_bolt_id
        -- FIXED: Added proper nil guard for menu.leveling_conserve_mana:get_state() (line 691)
...
local function try_life_tap(me)
    -- FIXED: Added proper nil guard for menu.use_life_tap:get_state() (line 717)
...
local function try_howl_of_terror(me)
    -- FIXED: Added proper nil guard for menu.use_howl_of_terror:get_state() (line 745)
...
local function desired_pet_name(mode)
    -- FIXED: Added proper nil guard for menu.preferred_pet:get() (line 815)
...
local function try_soul_shard_farm(me, target, drain_soul_id)
    -- FIXED: Added proper nil guard for menu.auto_shard_farm:get_state() (line 891)
    ...
    -- FIXED: Added proper nil guard for menu.min_shards:get() (line 897)
...
local function do_rotation(me, target)
    ...
    -- FIXED: Added proper nil guard for menu.use_interrupt:get_state() (line 918)
...
local function try_create_healthstone(me)
    -- FIXED: Added proper nil guard for menu.use_create_healthstone:get_state() (line 1009)
...
local function try_use_healthstone(me)
    -- FIXED: Added proper nil guard for menu.use_create_healthstone:get_state() (line 1034)
    if not menu.use_create_healthstone or not menu.use_create_healthstone:get_state() then return false end
...
local function try_soulstone_dead_ally(me)
    -- FIXED: Added proper nil guard for menu.use_soulstone:get_state() (line 1053)
...
local function try_self_soulstone(me)
    -- FIXED: Added proper nil guard for menu.use_soulstone:get_state() (line 1080)
    if not menu.use_soulstone or not menu.use_soulstone:get_state() then return false end
    if not runtime.soulstone_id then return false end
    if me:is_in_combat() then return false end
    if me:is_moving() then return false end
    ...
    actions = {
        ...
        interrupt_control = {
            handler = function(_, action_deps)
                ...
                -- FIXED: Added proper nil guard for menu.use_interrupt:get_state() (line 1118)
...
core.register_on_render_callback(function()
    -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 1147)
    if not menu or not menu.enabled or not (menu.enabled and menu.enabled:get_state()) or false then return end
    on_render()
end)
-- __EAX_ESP_GUARD
core.register_on_update_callback(function()
    ...
    handle_toggle()
    -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 1159)
    ...
    -- FIXED: Added proper nil guard for menu.auto_ooc_food_drink:get_state() (line 1168)
    ...
    if me:is_in_combat() then
        -- FIXED: Added proper nil guard for menu.auto_combat_potions:get_state() (line 1179)
        if menu.auto_combat_potions and menu.auto_combat_potions:get_state() then
            consumables_manager.try_use_combat_consumable(me, menu, utils)
        end
        -- FIXED: Added proper nil guard for menu.auto_flask:get_state() (line 1182)
