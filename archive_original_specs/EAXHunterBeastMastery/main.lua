...
-- FIXED: Corrected rotation priority - Kill Command is now correctly prioritized FIRST
-- FIXED: Fixed indentation in try_revive function
-- FIXED: All menu references have nil guards
-- FIXED: Squared distance comparisons used throughout (no sqrt for comparisons)
...
-- CC awareness: check if target can be CC'd (Freezing Trap, Scare Beast)
...
-- FIXED: Use proper API pattern for pet abilities
local function try_pet_ability(spell_id, target)
    ...
    -- FIXED: Use izi_sdk pattern if available, fallback to core API
    local ok_izi, izi = pcall(require, "common/izi_sdk")
    if ok_izi and izi then
        local spell = izi.spell(spell_id)
        if spell and spell.cast_safe then
            return spell:cast_safe(target)
        end
    end
...
-- FIXED: Use squared distance comparison (no sqrt)
local function dist(target)
    ...
    -- FIXED: Return squared distance instead of sqrt
    return (dx*dx + dy*dy + dz*dz)
end

-- FIXED: Compare squared distances
local function dist_yards(target)
    local d2 = dist(target)
    return math.sqrt(d2)  -- Only convert to yards when needed for display
end
...
local function warces_calc_window(me)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_warces_mode and menu.use_warces_mode:get_state()) then
        return nil, 0, 0, 0, false
    end
    local swing_remaining = swing_timer.get_time_to_swing(me)
    -- FIXED: Add nil guard for menu.warces_latency
...
local FLARE_SAFE_CAST_RANGE_SQ = FLARE_SAFE_CAST_RANGE * FLARE_SAFE_CAST_RANGE  -- FIXED: Pre-calculate squared
...
local function clamp_flare_cast_pos(me, pos)
    ...
    -- FIXED: Use squared comparison
    if not dist2 or dist2 <= FLARE_SAFE_CAST_RANGE_SQ then return pos end
    local dist_val = math.sqrt(dist2)
    if not dist_val or dist_val <= 0 then return pos end
    local scale = FLARE_SAFE_CAST_RANGE / dist_val
...
local function render_stealth_overlay(me)
    -- FIXED: Add nil guard for menu reference
    if not (menu.stealth_overlay_enabled and menu.stealth_overlay_enabled:get_state()) then return end
    rt.stealth_tracks = rt.stealth_tracks or {}
    local now = _core_time()
...
local function try_auto_stealth_flare(me)
    -- FIXED: Add nil guards for menu references
    local wants_flare = (menu.auto_stealth_flare and menu.auto_stealth_flare:get_state()) or false
    local wants_warning = (menu.stealth_warning and menu.stealth_warning:get_state()) or false
...
local function can_use_travel_aspect(me)
    ...
    -- FIXED: Add nil guard for menu reference
    if not (menu.auto_travel_aspect and menu.auto_travel_aspect:get_state()) then return false end
...
local function try_travel_aspect(me)
    ...
    -- FIXED: Add nil guard for menu reference
    local use_pack = (menu.use_pack_as_travel_aspect and menu.use_pack_as_travel_aspect:get_state()) or false
...
local function pet_is_engaged_on_target(pet, target)
    ...
    -- FIXED: Use squared comparison (36 = 6 yards squared)
...
pet_is_committed_on_target = function(pet, target, bm_state, now)
    ...
    if not ok or not target_guid or bm_state.last_target_guid ~= target_guid then
        return false
    end

    local fight_started_at = bm_state.fight_started_at or 0
    if fight_started_at <= 0 then return false end
    return (now - fight_started_at) >= KILL_COMMAND_SETTLE_S
end
...
local function try_aspect(me)
    ...
    -- Check if any enemy is attacking us in melee range (throttled)
    local now = _core_time()
    local target = me:get_target()
        ...
        for _, o in ipairs(core.object_manager.get_all_objects()) do
            if o and o:is_valid() and o:is_unit() and not o:is_dead() and me:can_attack(o) then
                ...
                if ok and ot and utils.same_unit(ot, me) then
                    ...
                    if p1 and p2 then
                        local dx,dy,dz = p1.x-p2.x, p1.y-p2.y, p1.z-p2.z
                        -- FIXED: Use squared comparison (64 = 8 yards squared)
...
local function try_revive(me)
    ...
    if p and p:is_dead() then
        ...
        if rt.revive_pet_id and utils.can_cast_self(rt.revive_pet_id, me) then
            ...
            -- FIXED: Corrected indentation (was outside if block)
...
local function try_mend(me)
    ...
    if pp and mp then
        ...
        -- FIXED: Use squared comparison (196 = 14 yards squared)
...
local function try_scorpid_sting(me, t)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_scorpid_sting and menu.use_scorpid_sting:get_state()) then return false end
...
local function try_viper_sting(me, t)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_viper_sting and menu.use_viper_sting:get_state()) then return false end
    ...
    -- FIXED: Add nil guards for all menu references
    if class_id == 5 and not ((menu.viper_sting_priest and menu.viper_sting_priest:get_state()) or false) then return false end
    if class_id == 2 and not ((menu.viper_sting_paladin and menu.viper_sting_paladin:get_state()) or false) then return false end
    if class_id == 7 and not ((menu.viper_sting_shaman and menu.viper_sting_shaman:get_state()) or false) then return false end
    if class_id == 8 and not ((menu.viper_sting_mage and menu.viper_sting_mage:get_state()) or false) then return false end
    if class_id == 9 and not ((menu.viper_sting_warlock and menu.viper_sting_warlock:get_state()) or false) then return false end
    if class_id == 6 and not ((menu.viper_sting_druid and menu.viper_sting_druid:get_state()) or false) then return false end
    if class_id == 3 and not ((menu.viper_sting_hunter and menu.viper_sting_hunter:get_state()) or false) then return false end
...
local function try_misdirection(me, t)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_misdirection and menu.use_misdirection:get_state()) then return false end
...
local function try_intimidation(me, t)
    if enc and enc.hold_cooldowns then return false end
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_intimidation and menu.use_intimidation:get_state()) then return false end
...
local function try_kill_command(me, t, ctx, bm_state, now)
    -- FIXED: Add nil guard for menu reference
...
local function try_warces_shots(me, t, ctx)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_warces_mode and menu.use_warces_mode:get_state()) then return false end
    if not rt.steady_shot_id then return false end
    ...
    -- FIXED: Add nil guards for menu references
    local use_arcane = (menu.use_arcane_shot and menu.use_arcane_shot:get_state()) or false
    local use_multi = (menu.use_multi_shot and menu.use_multi_shot:get_state()) or false
...
local function try_raptor_strike(me, t)
    ...
    -- FIXED: Use squared distance (25 = 5 yards squared)
    if not rt.raptor_strike_id or dist(t) > 25 then return false end
...
local function try_wing_clip(me, t)
    ...
    -- FIXED: Use squared distance (25 = 5 yards squared)
    if not rt.wing_clip_id or dist(t) > 25 then return false end
    ...
    -- FIXED: Add pcall protection for unit method calls
...
local function try_concussive(me, t)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_concussive and menu.use_concussive:get_state()) then return false end
...
local function try_disengage(me)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_disengage and menu.use_disengage:get_state()) then return false end
...
local function try_trap(me, t)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_traps and menu.use_traps:get_state()) then return false end
    -- FIXED: Use squared distance (36 = 6 yards squared)
    if dist(t) > 36 then return false end
    -- FIXED: Add nil guard for menu reference
...
-- -- Main rotation -------------------------------------------------------------
-- FIXED: Corrected rotation priority to match README:
-- 1. Kill Command (when pet engaged)
-- 2. Bestial Wrath (on cooldown)
-- 3. Intimidation (pet stun)
-- 4. Mend Pet (if pet HP <50%)
...
local function do_rotation(me, t)
    ...
    if (menu.use_interrupt and menu.use_interrupt:get_state()) and interrupt_manager.should_interrupt(t) and interrupt_manager.try_interrupt(me, t, "hunter", utils) then
        return
    end
    ...
    -- FIXED: Corrected rotation priority - Kill Command MUST be first (Priority 1)
    -- BM Hunter rotation priority: Kill Command → Bestial Wrath → Intimidation → Serpent/arcane weaving → Steady Shot
    ...
    if pet_manager.pet_alive(pet) then
        ...
        if try_kill_command(me, t, ctx, bm_state, now) then
            invalidate_ctx()
    ...
    -- Priority 3: Intimidation (pet stun)
    if try_intimidation(me, t) then return end

    -- Priority 4: Mend Pet (keep pet alive)
    ...
    -- FIXED: Use squared distance (1225 = 35 yards squared)
    if me:is_in_combat() and d > 25 and d <= 1225 and not is_moving() then
    ...
    -- Priority 5: Standard shot rotation (Aimed → Arcane/Steady weave)
...
local function handle_toggle()
    -- FIXED: Add nil guard for menu reference
    local cur = (menu.toggle_key and menu.toggle_key:get_state()) or false
    if cur and not rt.prev_toggle_state then
        -- FIXED: Add nil guard for menu.enabled
        if menu and menu.enabled then
            menu.enabled:set(not menu.enabled:get_state())
        end
    end
    rt.prev_toggle_state = cur
end
...
local function on_update()
    ...
    if me:is_in_combat() then
        ...
        if (menu.auto_combat_potions and menu.auto_combat_potions:get_state()) then
            if consumables_manager.try_use_combat_consumable(me, menu, utils) then return end
        end
        if (menu.auto_flask and menu.auto_flask:get_state()) then
            if consumables_manager.try_maintain_flask(me, menu, utils) then return end
        end
    end
    ...
    if pvp_instance or pvp_manager.is_world_pvp(me) then
        ...
        if #enemy_players > 0 then
            ...
            elseif pvp_instance == "battleground" then
                ...
                if priority then t = priority end
            end
        end
    end
    ...
    -- PvP cooldowns: trinket, deterrence, disengage
    if pvp_instance or pvp_manager.is_world_pvp(me) then
        ...
        pvp_manager.try_hunter_pvp_cooldowns(me, t)
    end
end
    ...
    actions = {
        ...
        interrupt_control = {
            handler = function(_, action_deps)
                ...
                return (menu.use_interrupt and menu.use_interrupt:get_state()) and interrupt_manager.try_interrupt(action_deps.me, interrupt_target, "hunter", utils)
            end,
        },
...
try_scare_beast = function(me, current_target)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_scare_beast and menu.use_scare_beast:get_state()) then return false end
...
try_flare = function(me, current_target)
    -- FIXED: Add nil guard for menu reference
    if not (menu.use_flare and menu.use_flare:get_state()) then return false end
...
sync_pet_autocast = function(me)
    ...
    local mode = active_mode()
    local now = _core_time()
    if rt.pet_autocast_configured and rt.pet_autocast_guid == guid and rt.pet_autocast_mode == mode and (now - (rt.pet_autocast_last_scan or 0)) < 1.5 then return end
