...
local function try_blade_flurry(me, target)
    ...
    if not menu.use_blade_flurry or not menu.use_blade_flurry:get_state() then
        return false
    end
...
local function try_adrenaline_rush(me)
    if enc and enc.hold_cooldowns then return false end
    if not menu.use_adrenaline_rush or not menu.use_adrenaline_rush:get_state() then
        return false
    end
...
local function should_pool_for_finisher(me, target)
    ...
    if cp < ((menu.finish_combo_points and menu.finish_combo_points:get() or 5) - 1) then
        return false
    end
    ...
    local rupture_due = menu.use_rupture:get_state()
        and cp >= (menu.finish_combo_points and menu.finish_combo_points:get() or 5)
...
local function try_slice_and_dice(me, target, ctx)
    ...
    local enemy_count = encounter_manager.enemy_count_in_range(me, 8)
    local regular_min_combo_points = math.min((menu.finish_combo_points and menu.finish_combo_points:get() or 5), 3)
...
local function try_rupture(me, target, ctx)
    ...
    local min_combo_points = (menu.finish_combo_points and menu.finish_combo_points:get()) or 5
...
local function try_sinister_strike(me, target, ctx)
    if not menu.use_sinister_strike or not menu.use_sinister_strike:get_state() then
        return false
    end
...
local function try_evasion(me)
    ...
    if hp_pct > (((menu.evasion_hp_pct and menu.evasion_hp_pct:get()) or 0.30) / 100) then return false end
