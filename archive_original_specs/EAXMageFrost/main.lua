...
local function visual_build_tracked_auras(me, target)
    ...
    -- FIXED: Use is_dead() instead of is_dead_or_ghost() (API compliance)
...
-- FIXED: Winter's Chill priority logic - maintain 5x stacks per README
local function should_refresh_winters_chill(target)
    -- FIXED: Proper nil guard for menu.use_winters_chill
    if not (menu.use_winters_chill and menu.use_winters_chill:get_state()) then return false end
    if not target or not target:is_valid() or target:is_dead() then return false end
    local stacks, remaining = get_debuff_stacks_and_remaining(target, spells.DEBUFF_WINTERS_CHILL)
    if stacks <= 0 then return true end  -- Need to start stacking
    if stacks < 5 then return true end   -- Need to reach 5 stacks
    -- FIXED: Proper nil guard for menu.winters_chill_refresh
    local refresh_s = (menu.winters_chill_refresh and menu.winters_chill_refresh:get()) or 3
...
local function update_set_bonus()
    ...
    if runtime.set_multiplier > 1.0 then
        -- FIXED: Proper nil guard for menu debug
...
local function try_water_elemental(me, target)
    -- FIXED: Proper nil guard
    if not (menu.use_water_elemental and menu.use_water_elemental:get_state()) then return false end
...
local function try_ice_lance(me, target)
    -- FIXED: Proper nil guard
    if not (menu.use_ice_lance and menu.use_ice_lance:get_state()) then return false end
...
try_frostbolt = function(me, target)
    -- FIXED: Proper nil guard
    if not (menu.use_frostbolt and menu.use_frostbolt:get_state()) then return false end
    if not runtime.frostbolt_id then return false end
    if not is_valid_hostile_target(me, target) then return false end
    if me:is_moving() then return false end
    ...
    -- FIXED: Proper nil guard for menu.leveling_conserve_mana
...
local function try_ice_barrier(me)
    -- FIXED: Proper nil guard
    if not (menu.use_ice_barrier and menu.use_ice_barrier:get_state()) then return false end
...
local function try_cone_of_cold_frost(me, target)
    -- FIXED: Proper nil guard
    if not (menu.use_cone_of_cold and menu.use_cone_of_cold:get_state()) then return false end
...
local function try_arcane_explosion(me, target)
    -- FIXED: Proper nil guard
    if not (menu.use_arcane_explosion and menu.use_arcane_explosion:get_state()) then return false end
...
-- FIXED: Rotation priority per README - Winter's Chill 5x -> Frostbolt spam
local function do_rotation(me, target)
    ...
    -- Interrupt
    -- FIXED: Proper nil guard for menu.use_interrupt
    ...
    -- FIXED: Winter's Chill priority - maintain 5x stacks before Frostbolt spam
    ...
    -- FIXED: Frostbolt spam after Winter's Chill maintained
...
local function handle_toggle()
    -- FIXED: Proper nil guard for menu.toggle_key
    local current = (menu.toggle_key and menu.toggle_key:get_state()) or false
    if current and not runtime.prev_toggle_state then
        -- FIXED: Proper nil guard for menu.enabled:get_state()
        menu.enabled:set(not ((menu.enabled and menu.enabled:get_state()) or false))
        -- FIXED: Proper nil guard
        if menu.debug and menu.debug:get_state() then
            utils.log_debug(menu, "Toggle -> " .. tostring((menu.enabled and menu.enabled:get_state()) or false))
        end
    end
...
local function try_ice_block(me)
    ...
    -- FIXED: Proper nil guard for menu.ice_block_hp_pct
    if hp_pct > (((menu.ice_block_hp_pct and menu.ice_block_hp_pct:get()) or 30) / 100) then return false end
    ...
    actions = {
        ...
        interrupt_control = {
            handler = function(_, action_deps)
                ...
                -- FIXED: Proper nil guard
                return (menu.use_interrupt and menu.use_interrupt:get_state()) and interrupt_manager.try_interrupt(action_deps.me, interrupt_target, "mage", utils)
            end,
        },
...
core.register_on_update_callback(function()
    ...
    -- FIXED: Proper nil guard
    if not menu.enabled or not (menu.enabled and menu.enabled:get_state()) then return end
