...
--- Extends can_cast_target with a hostility check and self-cast guard.
...
function utils.can_cast_hostile(spell_id, me, target)
    ...
    if not target:is_hostile() then return false end
...
function utils.cast_self(spell_id, me, message)
    return utils.cast_self_fast(spell_id, me, message)
end
...
function utils.log_debug(menu, message)
    if menu and menu.debug and menu.debug:get_state() then
        core.log("[Eax Mage Frost] " .. message)
    end
end
