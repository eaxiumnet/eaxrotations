...
local function handle_toggle()
    -- FIXED: Added nil guard for menu.toggle_key (line 411)
    local current = (menu and menu.toggle_key and menu.toggle_key:get_state()) or false
    if current and not runtime.prev_toggle_state then
        -- FIXED: Added nil guard for menu.enabled (line 413)
...
local function try_kick(me, target)
    if not menu.use_kick or not menu.use_kick:get_state() then
        return false
    end
...
local function try_cold_blood(me, target)
    ...
    if not menu.use_cold_blood or not menu.use_cold_blood:get_state() then
        return false
    end
    if mode == "solo" then
        return false
...
local function try_slice_and_dice(me, target, ctx)
    if not menu.use_slice_and_dice or not menu.use_slice_and_dice:get_state() then
        return false
    end
    ...
    local regular_min_combo_points = math.min((menu.rupture_combo_points and menu.rupture_combo_points:get()) or 4, 4)
...
local function try_envenom(me, target, ctx)
    if not menu.use_envenom or not menu.use_envenom:get_state() then
        return false
    end
    ...
    local required_combo_points = (menu.envenom_combo_points and menu.envenom_combo_points:get()) or 4
    -- FIXED: Removed execute phase 3 CP logic - Envenom should always require 4+ CP (line 690)
    -- FIXED: Allow Envenom at 4+ CP regardless of set bonus (line 697)
    ...
    if poison_stacks < ((menu.poison_stack_threshold and menu.poison_stack_threshold:get()) or 5) then
        return false
    end
    -- FIXED: Allow Envenom at 4+ CP when target won't live long enough for 5 CP (line 711)
    if ttd_s >= 20 and runtime.combo_points < ASSA_FINISHER_COMBO_POINTS then
        return false
    end
    local ok_energy, energy = pcall(function() return me:get_power(3) end)
...
local function try_rupture(me, target, ctx)
    if not menu.use_rupture or not menu.use_rupture:get_state() then
        return false
    end
    ...
    local rupture_min_combo_points = math.max((menu.rupture_combo_points and menu.rupture_combo_points:get()) or 4, 4)
    ...
    if ctx then
        local can_cast, reason = resource_gate.rogue.can_finisher(ctx, 25, runtime.combo_points, (menu.rupture_combo_points and menu.rupture_combo_points:get()) or 4)
        if not can_cast then
            return false
        end
    end
...
local function try_eviscerate(me, target, ctx)
    if not menu.use_eviscerate or not menu.use_eviscerate:get_state() then
        return false
    end
    ...
    if ttd_s >= 14 and poison_stacks >= ((menu.poison_stack_threshold and menu.poison_stack_threshold:get()) or 5) then
        return false
    end
    local ok_energy, energy = pcall(function() return me:get_power(3) end)
    if (target:is_casting_spell() or target:is_channelling_spell()) and ok_energy and tonumber(energy) and tonumber(energy) <= (KICK_ENERGY_RESERVE + 10) then
...
local function try_mutilate(me, target, ctx)
    if not menu.use_mutilate or not menu.use_mutilate:get_state() then
        return false
    end
    ...
    local poison_stacks = utils.get_debuff_stacks(target, spells.DEBUFF_DEADLY_POISON) or 0
    if runtime.set_multiplier > 1.0 and runtime.combo_points >= 4 and poison_stacks >= ((menu.poison_stack_threshold and menu.poison_stack_threshold:get()) or 5) then
        return false
    end
...
local function do_rotation(me, target)
    ...
    if target and interrupt_manager.should_interrupt(target) and not enc.hold_cooldowns then
        if (menu.use_interrupt and menu.use_interrupt:get_state()) and interrupt_manager.try_interrupt(me, target, "rogue", utils) then
            return true
        end
    end
...
local function try_evasion(me)
    if not menu.use_evasion or not menu.use_evasion:get_state() then return false end
    if not runtime.evasion_id then
        runtime.evasion_id = utils.resolve_spell_id(spells.EVASION)
    end
    if not runtime.evasion_id then return false end
    local hp_pct = me:get_health_percentage() / 100
    if hp_pct > (((menu.evasion_hp_pct and menu.evasion_hp_pct:get()) or 35) / 100) then return false end
    ...
    actions = {
        ...
        interrupt_control = {
            handler = function(_, action_deps)
                ...
                return (menu.use_interrupt and menu.use_interrupt:get_state()) and interrupt_manager.try_interrupt(action_deps.me, interrupt_target, "rogue", utils)
            end,
        },
...
core.register_on_update_callback(function()
    ...
    handle_toggle()
    -- FIXED: Added nil guard for menu.enabled (line 1272)
    if not menu or not menu.enabled or not menu.enabled:get_state() then
        return
    end
    ...
    -- FIXED: Added nil guard for menu.auto_ooc_food_drink (line 1290)
    if menu and menu.auto_ooc_food_drink and menu.auto_ooc_food_drink:get_state() then
        consumables_manager.try_use_ooc_food_drink(me, menu, utils)
    end

    if me:is_in_combat() then
        -- FIXED: Added nil guard for menu.auto_combat_potions (line 1295)
        if menu and menu.auto_combat_potions and menu.auto_combat_potions:get_state() then
            consumables_manager.try_use_combat_consumable(me, menu, utils)
        end
        -- FIXED: Added nil guard for menu.auto_flask (line 1298)
        if menu and menu.auto_flask and menu.auto_flask:get_state() then
    ...
    if my_hp < self_threshold then
        -- FIXED: Corrected function call from try_evasion to try_evasion(me) (line 1336)
        if try_evasion(me) then end
    end
end)
