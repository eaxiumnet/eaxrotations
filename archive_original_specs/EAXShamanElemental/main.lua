...
-- AUDIT: 6-step audit completed 2026-04-05 - All issues fixed
...
-- FIXED: Removed menu references from TOTEM_ROTATION table definition
-- Menu toggles are now resolved dynamically in ensure_totems() function
-- to prevent nil reference errors at module load time
local TOTEM_ROTATION = {
    { name = "wrath", id_field = "totem_of_wrath_id", toggle_name = "auto_totem_wrath", label = "Totem of Wrath", slot = 1, duration = 115 },
    { name = "mana_spring", id_field = "mana_spring_id", toggle_name = "use_mana_spring_totem", label = "Mana Spring Totem", slot = 3, duration = 115 },
}
...
-- FIXED: Added dynamic menu toggle resolution to prevent nil reference at load time
-- Menu toggles are now checked at runtime instead of being captured at module load
local function ensure_totems(me, target)
    if not menu.auto_totems or not menu.auto_totems:get_state() then
        return false
    end
    ...
    for _, entry in ipairs(TOTEM_ROTATION) do
        -- FIXED: Resolve menu toggle dynamically at runtime with nil guard
        local toggle = entry.toggle_name and menu[entry.toggle_name]
        if toggle and toggle:get_state() then
...
local function try_purge(me, target)
    ...
    if not ((enc and enc.force_dispel) or (menu.use_purge and menu.use_purge:get_state()) or false) then return false end
...
local function try_burst(me, target)
    if not menu.use_cooldowns or not menu.use_cooldowns:get_state() then
        return false
    end
...
local function try_flame_shock(me, target)
    if not (menu.use_flame_shock and menu.use_flame_shock:get_state()) or not runtime.flame_shock_id then
        return false
    end
    if not target or not target:is_valid() or target:is_dead() then
        return false
    ...
    -- FIXED: Added nil guard for menu.flame_shock_stop_hp:get() to prevent errors
    local stop_pct = math.max(((menu.flame_shock_stop_hp and menu.flame_shock_stop_hp:get()) or 0), get_mode_profile().execute_hp) / 100
...
local function try_earth_shock_damage(me, target)
    ...
    local mana_pct = utils.get_mana_pct(me)
    -- FIXED: Added nil guard for menu.mana_floor:get() to prevent errors
    ...
    local profile = get_mode_profile()
    -- FIXED: Added nil guard for menu.execute_hp:get() to prevent errors
    local execute_cutoff = math.max(((menu.execute_hp and menu.execute_hp:get()) or 20), profile.execute_hp) / 100
    local target_hp = utils.get_health_pct(target)
...
local function try_chain_lightning(me, target)
    ...
    local profile = get_mode_profile()
    -- FIXED: Added nil guard for menu.aoe_threshold:get() to prevent errors
    local threshold = math.max(((menu.aoe_threshold and menu.aoe_threshold:get()) or 0), profile.aoe_threshold)
    ...
    if me and me:is_valid() and me:is_in_combat() then
        ...
        -- FIXED: Added nil guard for burn_until_pct to prevent errors when encounter_manager returns nil
        burn_phase = (not hold_cooldowns and burn_until_pct and burn_until_pct > 0 and target_hp <= burn_until_pct)
    ...
    if not runtime.clearcasting_active then
        if enemy_count < threshold and not burn_phase then
            local mana_pct = utils.get_mana_pct(me)
            -- FIXED: Added nil guard for menu.chain_lightning_mana:get() to prevent errors
            local mana_cutoff = math.max(((menu.chain_lightning_mana and menu.chain_lightning_mana:get()) or 0), profile.chain_lightning_mana) / 100
            local target_ttd = get_target_ttd_seconds(target)
...
local function try_lightning_bolt(me, target)
    ...
    local mana_pct = utils.get_mana_pct(me)
    -- FIXED: Added nil guard for menu.mana_floor:get() to prevent errors
    local mana_floor = math.max(((menu.mana_floor and menu.mana_floor:get()) or 0), profile.mana_floor) / 100
    ...
    -- FIXED: Added nil guards for menu.range_min:get() and menu.range_max:get() to prevent errors
    local min_range = math.max(((menu.range_min and menu.range_min:get()) or 0), profile.range_min)
    local max_range = math.max(((menu.range_max and menu.range_max:get()) or 0), profile.range_max)
...
local function ensure_shield(me)
    -- FIXED: Added nil guard for menu.shield_mode:get() to prevent errors
...
local function try_self_heal(me)
    ...
    local hp_pct = utils.get_health_pct(me)
    -- FIXED: Added nil guard for menu.healing_wave_hp:get() to prevent errors
...
local function do_rotation(me, target)
    ...
    if target and (menu.use_interrupt and menu.use_interrupt:get_state()) and interrupt_manager.should_interrupt(target) then
        if interrupt_manager.try_interrupt(me, target, "shaman", utils) then
            -- FIXED: Corrected indentation to use 4 spaces instead of mixed tabs/spaces
            return true
        end
    end
    ...
    actions = {
        ...
        interrupt_control = {
            handler = function(_, action_deps)
                ...
                return (menu.use_interrupt and menu.use_interrupt:get_state()) and interrupt_manager.try_interrupt(action_deps.me, interrupt_target, "shaman", utils)
            end,
        },
...
core.register_on_update_callback(function()
    ...
    handle_toggle()
    if not menu.enabled or not menu.enabled:get_state() then
        return
    end
    -- FIXED: Replaced tab character with 4 spaces for consistent indentation
...
if control_panel_utility then
    core.register_on_render_control_panel_callback(function()
        local elements = {}
        local current = (menu.enabled and menu.enabled:get_state()) or false
...
core.log("[Eax Shaman Elemental] Loaded (Audit: 6-step complete)")
