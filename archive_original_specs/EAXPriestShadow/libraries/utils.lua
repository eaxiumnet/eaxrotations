...
function utils.get_mana_pct(unit)
    ...
    -- Use power type 0 (mana) - get_max_power and get_power are the correct APIs
...
--- Off-GCD self-cast (doesn't trigger GCD, casts immediately)
--- Used for spells like Inner Focus that are instant and off-GCD
---@param spell_id number|nil
---@param me game_object
---@return boolean
...
--- Get list of nearby hostile units for multi-dotting (AoE)
--- Scans all objects and returns hostiles within range
---@param me game_object
---@param max_range number|nil (default 40)
---@param max_count number|nil (default 10)
---@return table
function utils.get_nearby_hostiles(me, max_range, max_count)
    if not me or not me:is_valid() then return {} end
    max_range = max_range or 40
    max_count = max_count or 10
    
    local hostiles = {}
    local pos_me = nil
    local ok, value = pcall(function() return me:get_position() end)
    if ok then pos_me = value end
    
    local objects = core.object_manager.get_all_objects()
    for i = 1, #objects do
        if #hostiles >= max_count then break end
        local obj = objects[i]
        if obj and obj:is_valid() and obj:is_unit() and not obj:is_dead() then
            if me:can_attack(obj) then
                if pos_me then
                    local ok_u, pos_u = pcall(function() return obj:get_position() end)
                    if ok_u and pos_u then
                        local dx = pos_me.x - pos_u.x
                        local dy = pos_me.y - pos_u.y
                        local dz = pos_me.z - pos_u.z
                        if (dx*dx + dy*dy + dz*dz) <= (max_range * max_range) then
