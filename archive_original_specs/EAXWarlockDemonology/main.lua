...
core.register_on_update_callback(function()
    -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 200)
...
local function get_effective_mode()
    -- FIXED: Added proper nil guard for menu.mode:get() (line 407)
...
local function handle_toggle()
    -- FIXED: Added proper nil guard for menu.toggle_key:get_state() (line 427)
    local current = (menu.toggle_key and menu.toggle_key:get_state()) or false
    if current and not runtime.prev_toggle_state then
        -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 429)
...
local function try_fel_armor(me)
    -- FIXED: Added proper nil guard for menu.use_fel_armor:get_state() (line 476)
    if not (menu.use_fel_armor and menu.use_fel_armor:get_state()) or not runtime.fel_armor_id then
        return false
    end
...
local function try_health_funnel(me)
    -- FIXED: Added proper nil guard for menu.use_health_funnel:get_state() (line 486)
    if not (menu.use_health_funnel and menu.use_health_funnel:get_state()) or not runtime.health_funnel_id then
        return false
    end
...
local function try_fel_domination(me)
    -- FIXED: Added proper nil guard for menu.use_fel_domination:get_state() (line 533)
    if not (menu.use_fel_domination and menu.use_fel_domination:get_state()) or not runtime.fel_domination_id then
        return false
    end
...
local function ensure_soul_link(me)
    -- FIXED: Added proper nil guard for menu.maintain_soul_link:get_state() (line 546)
    if not (menu.maintain_soul_link and menu.maintain_soul_link:get_state()) or not runtime.soul_link_id then
        return false
    end
...
local function desired_pet_name(mode)
    -- FIXED: Added proper nil guard for menu.preferred_pet:get() (line 623)
...
local function try_summon_correct_pet(me, mode)
    ...
    -- FIXED: Added proper nil guard for menu.pet_check_interval:get() (line 636)
    local interval = (menu.pet_check_interval and menu.pet_check_interval:get()) or 30
...
local function try_banish(me, target)
    -- FIXED: Added proper nil guard for menu.use_banish:get_state() (line 696)
...
local function try_shadowfury(me, target)
    -- FIXED: Added proper nil guard for menu.use_shadowfury:get_state() (line 718)
    if not (menu.use_shadowfury and menu.use_shadowfury:get_state()) or not runtime.shadowfury_id or not target then
        return false
    end
...
local function try_apply_curse(me, target, mode)
    -- FIXED: Added proper nil guard for menu.use_curse:get_state() (line 774)
...
local function try_refresh_dots(me, target)
    -- FIXED: Added proper nil guard for menu.use_immolate:get_state() (line 804)
    if menu.use_immolate and menu.use_immolate:get_state() and runtime.immolate_id
    ...
    -- FIXED: Added proper nil guard for menu.use_corruption:get_state() (line 811)
    ...
    -- FIXED: Added proper nil guard for menu.use_unstable_affliction:get_state() (line 819)
...
local function try_shadow_burn(me, target)
    -- FIXED: Added proper nil guard for menu.use_shadow_burn:get_state() (line 830)
    if not (menu.use_shadow_burn and menu.use_shadow_burn:get_state()) or not runtime.shadow_burn_id then
        return false
    end
...
local function try_drain_soul(me, target)
    -- FIXED: Added proper nil guard for menu.use_drain_soul:get_state() (line 848)
    if not (menu.use_drain_soul and menu.use_drain_soul:get_state()) or not runtime.drain_soul_id then
        return false
    end
...
local function try_soul_fire(me, target)
    -- FIXED: Added proper nil guard for menu.use_soul_fire:get_state() (line 863)
    if not (menu.use_soul_fire and menu.use_soul_fire:get_state()) or not runtime.soul_fire_id then
        return false
    end
    if utils.get_health_pct(target) <= DRAIN_SOUL_HP_PCT then
        return false
...
local function try_shadow_bolt(me, target)
    -- FIXED: Added proper nil guard for menu.use_shadow_bolt:get_state() (line 878)
    if not (menu.use_shadow_bolt and menu.use_shadow_bolt:get_state()) or not runtime.shadow_bolt_id then
        return false
    end
    -- Leveling: use appropriate spell rank
    local shadow_bolt_id = runtime.shadow_bolt_id
    -- FIXED: Added proper nil guard for menu.leveling_conserve_mana:get_state() (line 883)
...
local function try_life_tap(me, mode)
    -- FIXED: Added proper nil guard for menu.use_life_tap:get_state() (line 911)
    ...
    -- FIXED: Added proper nil guard for menu.life_tap_threshold:get() (line 920)
...
local function try_drain_life_defensive(me, target)
    -- FIXED: Added proper nil guard for menu.use_drain_life_def:get_state() (line 935)
    ...
    -- FIXED: Added proper nil guard for menu.use_drain_life_def_hp_pct:get() (line 938)
...
local function try_create_healthstone(me)
    -- FIXED: Added proper nil guard for menu.use_create_healthstone:get_state() (line 957)
...
local function try_use_healthstone(me)
    -- FIXED: Added proper nil guard for menu.use_create_healthstone:get_state() (line 981)
    if not menu.use_create_healthstone or not menu.use_create_healthstone:get_state() then return false end
...
local function try_soulstone_dead_ally(me)
    -- FIXED: Added proper nil guard for menu.use_soulstone:get_state() (line 1000)
...
local function try_self_soulstone(me)
    -- FIXED: Added proper nil guard for menu.use_soulstone:get_state() (line 1025)
    if not menu.use_soulstone or not menu.use_soulstone:get_state() then return false end
    if not runtime.soulstone_id then return false end
    if me:is_in_combat() then return false end
    if me:is_moving() then return false end
...
local function do_rotation(me, target)
    ...
    -- FIXED: Added proper nil guard for menu.use_interrupt:get_state() (line 1048)
    ...
    actions = {
        ...
        interrupt_control = {
            handler = function(_, action_deps)
                ...
                -- FIXED: Added proper nil guard for menu.use_interrupt:get_state() (line 1140)
...
core.register_on_render_callback(function()
    -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 1168)
    if not menu or not menu.enabled or not (menu.enabled and menu.enabled:get_state()) or false then return end
    on_render()
end)
...
core.register_on_update_callback(function()
    ...
    handle_toggle()
    -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 1180)
    ...
    -- FIXED: Added proper nil guard for menu.auto_ooc_food_drink:get_state() (line 1190)
    ...
    if me:is_in_combat() then
        -- FIXED: Added proper nil guard for menu.auto_combat_potions:get_state() (line 1201)
        if menu.auto_combat_potions and menu.auto_combat_potions:get_state() then
            consumables_manager.try_use_combat_consumable(me, menu, utils)
        end
        -- FIXED: Added proper nil guard for menu.auto_flask:get_state() (line 1204)
