...
local function try_moonkin_form(me)
    if not menu.force_moonkin or not (menu.force_moonkin and menu.force_moonkin:get_state()) or false then return false end
...
local function try_faerie_fire(me, target, ctx)
    if not menu.use_faerie_fire or not (menu.use_faerie_fire and menu.use_faerie_fire:get_state()) or false then return false end
...
local function try_moonfire(me, target, ctx)
    if not menu.use_moonfire or not (menu.use_moonfire and menu.use_moonfire:get_state()) or false then return false end
...
local function try_insect_swarm(me, target, ctx)
    if not menu.use_insect_swarm or not (menu.use_insect_swarm and menu.use_insect_swarm:get_state()) or false then return false end
...
local function try_force_of_nature(me, target, mana_pct)
    ...
    if not menu.use_force_of_nature or not (menu.use_force_of_nature and menu.use_force_of_nature:get_state()) or false then return false end
...
local function try_starfire(me, target, ctx, mana_tier)
    ...
    if menu.leveling_conserve_mana and (menu.leveling_conserve_mana and menu.leveling_conserve_mana:get_state()) or false then
    ...
    -- Fix: Check if spell is actually on cooldown before attempting cast
    -- smart_cast_manager clears pending when cd <= 0, which can happen during GCD
    local cd_remaining = _get_spell_cd(starfire_id)
    if cd_remaining and cd_remaining > 0 then return false end
    if not utils.can_cast_hostile(starfire_id, me, target) then return false end
    if utils.cast_target(starfire_id, me, target) then
...
local function try_wrath(me, target, ctx)
    ...
    -- Fix: Check if spell is actually on cooldown before attempting cast
    -- smart_cast_manager clears pending when cd <= 0, which can happen during GCD
    local cd_remaining = _get_spell_cd(runtime.wrath_id)
    if cd_remaining and cd_remaining > 0 then return false end
...
local function try_hurricane(me, enemy_count, mana_pct)
    ...
    if not menu.use_hurricane or not (menu.use_hurricane and menu.use_hurricane:get_state()) or false then return false end
...
local function try_root_escape_balance(me)
    if not menu.use_root_escape or not (menu.use_root_escape and menu.use_root_escape:get_state()) or false then return false end
...
local function do_rotation(me, target, menu, utils)
    ...
    local deps = { 
        ...
        encounter_manager = encounter_manager,
        -- Minimal health_prediction for DPS specs (prevents fail_safe mode)
        health_prediction = {
            get_incoming_damage = function(_, unit, duration) return 0, true end,
            get_role_id = function(_) return "dps", true end,
            is_tank = function(_) return false, true end,
        },
    }
    ...
    -- Load settings with safe nil guards
    local settings = {
        maintain_faerie_fire = (menu.use_faerie_fire and menu.use_faerie_fire:get_state()) and 1 or 4,
        use_force_of_nature = (menu.use_force_of_nature and menu.use_force_of_nature:get_state()) or false,
        force_of_nature_min_ttd = (menu.force_of_nature_min_ttd and menu.force_of_nature_min_ttd:get()) or 10,
        balance_use_innervate = (menu.use_innervate and menu.use_innervate:get_state()) or false,
        balance_innervate_mana = (menu.innervate_mana_pct and menu.innervate_mana_pct:get()) or 30,
        hurricane_min_targets = (menu.hurricane_min_targets and menu.hurricane_min_targets:get()) or 4,
        clearcast_starfire = true,  -- default optimization
        maintain_insect_swarm = (menu.use_insect_swarm and menu.use_insect_swarm:get_state()) or false,
        maintain_moonfire = (menu.use_moonfire and menu.use_moonfire:get_state()) or false,
        ng_wrath_priority = (menu.bal_ng_wrath and menu.bal_ng_wrath:get_state()) or false,
    }

    -- Get TTD for cooldown decisions
    local ttd_s = nil
    if ttd_tracker and ttd_tracker.get then
        local ok, value = pcall(function() return ttd_tracker.get(target) end)
        if ok then ttd_s = tonumber(value) end
    ...
    -- Get enemy count for AoE
    local enemy_count = 0
    if ctx and ctx.enemy_count then
        enemy_count = ctx.enemy_count
    else
        enemy_count = utils.enemy_count_in_radius(me, 30)
    end

    -- Check for Clearcasting (Omen of Clarity) - also used as NG proxy
    local has_clearcasting = ctx and ctx.has_clearcasting or utils.has_buff(me, spells.BUFF_CLEARCASTING)

    -- ============================================
    -- Priority 1: Faerie Fire (armor debuff)
    -- ============================================
    if settings.maintain_faerie_fire ~= 4 then
        local ff_remains = utils.get_debuff_remaining_ms(target, spells.DEBUFF_FAERIE_FIRE) / 1000
        if ff_remains <= 3 then
            if try_faerie_fire(me, target, ctx) then return true end
        end
    end

    -- ============================================
    -- Priority 2: Force of Nature (burst CD)
    -- ============================================
    if settings.use_force_of_nature and ttd_s and ttd_s > settings.force_of_nature_min_ttd then
        if try_force_of_nature(me, target, mana_pct) then return true end
    end

    -- ============================================
    -- Priority 3: Innervate (mana recovery)
    -- ============================================
    if settings.balance_use_innervate and mana_pct * 100 <= settings.balance_innervate_mana then
        if not utils.has_buff(me, spells.BUFF_INNERVATE) then
            if try_innervate(me, mana_pct) then return true end
        end
    end

    -- ============================================
    -- Priority 4: AoE Hurricane (with Barkskin)
    -- ============================================
    if enemy_count >= settings.hurricane_min_targets then
        -- Try Barkskin first to protect the channel
        if runtime.barkskin_id and not utils.has_buff(me, spells.BUFF_BARKSKIN) then
            if utils.can_cast_self(runtime.barkskin_id, me) then
                if utils.cast_self(runtime.barkskin_id, me) then
                    mark_pending_cast(runtime.barkskin_id, PENDING_CAST_TIMEOUT_S)
                    utils.log_debug(menu, "Barkskin (protecting Hurricane)")
                    note_cast()
                    return true
                end
            end
    ...
    -- ============================================
    -- Priority 5: Pull Opener (out of combat)
    -- ============================================
    if not me:is_in_combat() then
        if not me:is_moving() then
            -- Try Starfire first
            if runtime.starfire_id and utils.can_cast_hostile(runtime.starfire_id, me, target) then
                if utils.cast_target(runtime.starfire_id, me, target) then
                    mark_pending_cast(runtime.starfire_id, PENDING_CAST_TIMEOUT_S, { action_key = "starfire", category = "long" })
                    utils.log_debug(menu, "Starfire (pull opener)")
                    note_cast()
                    return true
                end
            end
            -- Fallback to Wrath
            if runtime.wrath_id and utils.can_cast_hostile(runtime.wrath_id, me, target) then
                if utils.cast_target(runtime.wrath_id, me, target) then
                    mark_pending_cast(runtime.wrath_id, PENDING_CAST_TIMEOUT_S, { action_key = "wrath", category = "filler" })
                    utils.log_debug(menu, "Wrath (pull opener)")
                    note_cast()
                    return true
                end
            end
        end
        -- Moonfire if moving or other spells not available
        local mf_remains = utils.get_debuff_remaining_ms(target, spells.DEBUFF_MOONFIRE)
        if mf_remains <= 0 then
            if try_moonfire(me, target, ctx) then return true end
        end
    end

    -- ============================================
    -- Priority 6: Mana Tier System
    -- ============================================
    local tier1_threshold = ((menu.bal_tier1_mana and menu.bal_tier1_mana:get()) or 20) / 100
    local tier2_threshold = ((menu.bal_tier2_mana and menu.bal_tier2_mana:get()) or 10) / 100
    local mana_tier = (mana_pct < tier2_threshold) and 3 or (mana_pct < tier1_threshold) and 2 or 1

    -- ============================================
    -- Priority 7: Clearcasting Starfire (free cast)
    -- ============================================
    if has_clearcasting and settings.clearcast_starfire then
        if runtime.starfire_id and utils.can_cast_hostile(runtime.starfire_id, me, target) then
            if utils.cast_target(runtime.starfire_id, me, target) then
                mark_pending_cast(runtime.starfire_id, PENDING_CAST_TIMEOUT_S, { action_key = "starfire", category = "long" })
    ...
    -- ============================================
    -- Priority 8: DoT Maintenance
    -- ============================================
    local dot_refresh = ((menu.bal_dot_refresh and menu.bal_dot_refresh:get()) or 0)
    ...
    -- Insect Swarm: Tier 1 or 2 only
    if settings.maintain_insect_swarm and mana_tier <= 2 then
        local is_remains = utils.get_debuff_remaining_ms(dot_target, spells.DEBUFF_INSECT_SWARM) / 1000
        if is_remains <= dot_refresh then
            if try_insect_swarm(me, dot_target, ctx) then return true end
        end
    end

    -- Moonfire: Tier 1 only (high mana cost)
    if settings.maintain_moonfire and mana_tier == 1 then
        local mf_remains = utils.get_debuff_remaining_ms(dot_target, spells.DEBUFF_MOONFIRE) / 1000
        if mf_remains <= dot_refresh then
            if try_moonfire(me, dot_target, ctx) then return true end
        end
    end

    -- ============================================
    -- Priority 9: Nature's Grace Wrath Priority
    -- ============================================
    -- Check for Nature's Grace buff (proc from crit Starfire)
    local has_natures_grace = runtime.natures_grace_id and utils.has_buff(me, spells.BUFF_NATURES_GRACE)
    if has_natures_grace and settings.ng_wrath_priority then
        if runtime.wrath_id and utils.can_cast_hostile(runtime.wrath_id, me, target) and not me:is_moving() then
            if utils.cast_target(runtime.wrath_id, me, target) then
                mark_pending_cast(runtime.wrath_id, PENDING_CAST_TIMEOUT_S, { action_key = "wrath", category = "filler" })
                utils.log_debug(menu, "Wrath (Nature's Grace proc)")
                note_cast()
                esp_renderer.on_cast(runtime.wrath_id, "Wrath", color.blue(220))
                return true
            end
    ...
    -- ============================================
    -- Priority 10 & 11: Primary/Fallback Nukes
    -- ============================================
    -- Starfire (primary nuke)
    if try_starfire(me, dot_target, ctx, mana_tier == 3 and "emergency" or mana_tier == 2 and "conserve" or "full") then
        return true
    end
    ...
    actions = {
        ...
        interrupt_control = {
            handler = function(_, action_deps)
                ...
                return (menu.use_interrupt and (menu.use_interrupt and menu.use_interrupt:get_state()) or false) and interrupt_manager.try_interrupt(action_deps.me, interrupt_target, "druid", utils)
            end,
        },
...
core.register_on_update_callback(function()
    ...
    -- Interrupt
    if target and target:is_valid() and me:can_attack(target) and (menu.use_interrupt and menu.use_interrupt:get_state()) or false and interrupt_manager.should_interrupt(target) then
    ...
    -- PvP cooldowns: trinket, barkskin, nature's swiftness
    local pvp_instance = pvp_manager.is_in_pvp_instance()
    if pvp_instance or pvp_manager.is_world_pvp(me) then
        ...
        pvp_manager.try_druid_pvp_cooldowns(me, target)
    end
