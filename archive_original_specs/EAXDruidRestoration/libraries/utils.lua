...
function utils.can_cast_friendly(spell_id, me, target)
    ...
    if me:can_attack(target) then return false end
    return utils.can_cast_target(spell_id, me, target)
end
...
function utils.cast_friendly(spell_id, me, target)
    if not spell_id or not me or not target or not target:is_valid() then return false end
    if utils.same_unit(me, target) then
...
function utils.get_effective_hp_pct(unit)
    ...
    local max_hp = unit:get_max_health()
    if max_hp <= 0 then return 1.0 end
    local current_hp = unit:get_health()
    local ok, incoming = pcall(function() return unit:get_incoming_heals() end)
    local incoming_heals = (ok and tonumber(incoming)) or 0
    local effective = math.min(current_hp + incoming_heals, max_hp)
