...
-- ============================================================================
-- FLUX PORT: New advanced healing modules
-- ============================================================================
local lifebloom_bloom_manager = require("libraries/lifebloom_bloom_manager")
local tree_reshift_manager = require("libraries/tree_reshift_manager")
local rejuv_spread_manager = require("libraries/rejuv_spread_manager")
local emergency_handlers = require("libraries/emergency_handlers")
...
core.register_on_update_callback(function()
    -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 218)
...
local function get_effective_mode()
    -- FIXED: Added proper nil guard for menu.mode:get() (line 392)
...
local function handle_toggle()
    -- FIXED: Added proper nil guard for menu.toggle_key:get_state() (line 400)
    local current = (menu.toggle_key and menu.toggle_key:get_state()) or false
    if current and not runtime.prev_toggle_state then
        -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 402)
...
local function should_activate_healing_rotation(me, units)
    -- OOC mana conservation: don't start healing rotation if healer is low on mana
    local my_mana_pct = utils.get_mana_pct and utils.get_mana_pct(me) or 1.0
    if not me:is_in_combat() and my_mana_pct < 0.50 then
        return false  -- OOM protection: don't heal others when OOC and mana < 50%
    end
    
    -- OOC self-preservation: don't heal others if healer is injured
    local my_hp = heal_engine.get_effective_hp_pct(me)
    if not me:is_in_combat() and my_hp < 0.80 then
        return false  -- Self-preservation: heal self first when OOC and HP < 80%
    end
    
    local any_injured = false
    for i = 1, #units do
        local unit = units[i]
        if unit and unit:is_valid() and not unit:is_dead() then
            ...
            -- FIXED: Changed from 0.95 (95%) to 0.80 (80%) to prevent OOC spam
            -- Also check OOC threshold is lower to avoid healing at full health
            local hp_threshold = me:is_in_combat() and 0.95 or 0.80
            if heal_engine.get_effective_hp_pct(unit) < hp_threshold then
                any_injured = true
            end
        end
    end
    
    -- FIXED: If no one is actually injured (all >= 95% HP), don't activate
    return any_injured
end
...
local function try_mark_of_the_wild(me)
    -- FIXED: Added proper nil guard for menu.use_mark_of_the_wild:get_state() (line 591)
    if not menu.use_mark_of_the_wild or not (menu.use_mark_of_the_wild and menu.use_mark_of_the_wild:get_state()) or false then return false end
    ...
    -- FIXED: Added proper nil guard for menu.buff_friendlies:get_state() (line 596)
    local buff_friendlies = menu.buff_friendlies and (menu.buff_friendlies and menu.buff_friendlies:get_state()) or false
...
local function try_innervate(me, mana_pct)
    -- FIXED: Added proper nil guard for menu.use_innervate:get_state() (line 634)
    if not menu.use_innervate or not (menu.use_innervate and menu.use_innervate:get_state()) or false then return false end
    if not runtime.innervate_id then return false end
    if is_pending_cast(runtime.innervate_id) then return false end
    -- FIXED: Added proper nil guard for menu.innervate_mana_pct:get() (line 637)
    local threshold = ((menu.innervate_mana_pct and menu.innervate_mana_pct:get()) or 30) / 100
    -- FIXED: Added proper nil guard for menu.innervate_target:get() (line 638)
...
local function try_tranquility(me, injured_count, lowest_hp_pct, mode)
    -- FIXED: Added proper nil guard for menu.use_tranquility:get_state() (line 705)
    if not menu.use_tranquility or not (menu.use_tranquility and menu.use_tranquility:get_state()) or false then return false end
    if not runtime.tranquility_id then return false end
    if mode == "solo" then return false end
    -- FIXED: Added proper nil guard for menu.tranquility_injured_count:get() (line 708)
...
local function try_lifebloom(me, tank, tank_hp_pct)
    ...
    -- FIXED: Added proper nil guard for menu.use_lifebloom:get_state() (line 734)
    if not menu.use_lifebloom or not (menu.use_lifebloom and menu.use_lifebloom:get_state()) or false then
    ...
    -- FIXED: Added proper nil guard for menu.lifebloom_stacks:get() (line 751)
    ...
    -- FIXED: Added proper nil guard for menu.lifebloom_refresh_seconds:get() (line 752)
    ...
    -- FLUX PORT: Check bloom manager for optimal refresh vs bloom timing
    if stack_count >= 3 and remaining_ms > 0 then
        -- FIXED: Added proper nil guard for menu.lifebloom_allow_bloom:get() (line 759)
        local allow_bloom = menu.lifebloom_allow_bloom and menu.lifebloom_allow_bloom:get() or false
        if allow_bloom then
            -- FIXED: Added proper nil guard for menu.lifebloom_bloom_threshold:get() (line 761)
            local should_bloom, reason = lifebloom_bloom_manager.should_allow_bloom(
                tank, 
                (menu.lifebloom_bloom_threshold and menu.lifebloom_bloom_threshold:get()) or 1.5,
                true  -- is_tank
            )
            if should_bloom then
                log_debug_throttled("lifebloom_bloom", "Lifebloom: allowing bloom for mana return (" .. reason .. ")", 2.0)
                return false  -- Skip refresh, let it bloom
            end
        end
    end
...
local function try_swiftmend(me, target, target_hp_pct)
    -- FLUX PORT: Delegate to granular emergency handler
    if not emergency_handlers.should_swiftmend(me, target, target_hp_pct, menu, runtime, spells, utils) then
        return false
    end
    return emergency_handlers.execute_swiftmend(me, target, runtime, utils, color)
end

local function try_natures_swiftness_healing_touch(me, target, target_hp_pct)
    -- FLUX PORT: Delegate to granular emergency handler
    if not emergency_handlers.should_ns_healing_touch(me, target, target_hp_pct, menu, runtime) then
        return false
    end
    return emergency_handlers.execute_ns_healing_touch(me, target, runtime, utils, tree_reshift_manager, color)
end

-- FLUX PORT: New emergency handler for Nature's Swiftness + Regrowth (stays in Tree)
local function try_natures_swiftness_regrowth(me, target, target_hp_pct)
    if not emergency_handlers.should_ns_regrowth(me, target, target_hp_pct, menu, runtime) then
        return false
    end
    return emergency_handlers.execute_ns_regrowth(me, target, runtime, utils, color)
end

-- FLUX PORT: Emergency Barkskin (self-defense)
local function try_emergency_barkskin(me)
    return emergency_handlers.should_barkskin(me, menu, runtime) and
           emergency_handlers.execute_barkskin(me, runtime, utils)
end
...
local function try_rejuvenation(me, target, target_hp_pct)
    -- FIXED: Added proper nil guard for menu.use_rejuvenation:get_state() (line 861)
    if not menu.use_rejuvenation or not (menu.use_rejuvenation and menu.use_rejuvenation:get_state()) or false then return false end
    ...
    -- FIXED: Added proper nil guard for menu.rejuvenation_refresh_seconds:get() (line 873)
...
local function try_tree_of_life(me, units, mode, injured_count)
    -- FIXED: Added proper nil guard for menu.use_tree_of_life:get_state() (line 888)
    if not menu.use_tree_of_life or not (menu.use_tree_of_life and menu.use_tree_of_life:get_state()) or false then return false end
...
local function try_spread_rejuvenation(me, units, mana_pct)
    -- FIXED: Added proper nil guard for menu.use_rejuvenation:get_state() (line 950)
    if not menu.use_rejuvenation or not (menu.use_rejuvenation and menu.use_rejuvenation:get_state()) or false then return false end
    if not runtime.rejuvenation_id then return false end
    -- FIXED: Added proper nil guard for menu.mana_saver:get_state() (line 952)
    if menu.mana_saver and (menu.mana_saver and menu.mana_saver:get_state()) or false and mana_pct < 0.55 then return false end
    if is_pending_cast(runtime.rejuvenation_id) then return false end
    -- FIXED: Added proper nil guard for menu.rejuvenation_refresh_seconds:get() (line 954)
    local refresh_ms = ((menu.rejuvenation_refresh_seconds and menu.rejuvenation_refresh_seconds:get()) or 3) * 1000
...
local function try_regrowth(me, target, target_hp_pct, mana_pct)
    -- FIXED: Added proper nil guard for menu.use_regrowth:get_state() (line 993)
    if not menu.use_regrowth or not (menu.use_regrowth and menu.use_regrowth:get_state()) or false then return false end
    if not runtime.regrowth_id then return false end
    if target_hp_pct >= 0.80 then return false end
    -- FIXED: Added proper nil guard for menu.mana_saver:get_state() (line 996)
    if menu.mana_saver and (menu.mana_saver and menu.mana_saver:get_state()) or false and mana_pct < 0.45 and target_hp_pct > 0.80 then return false end
    -- FIXED: Added proper nil guard for menu.regrowth_refresh_seconds:get() (line 997)
...
local function try_healing_touch(me, target, target_hp_pct, mana_pct)
    -- FIXED: Added proper nil guard for menu.use_healing_touch:get_state() (line 1020)
    if not menu.use_healing_touch or not (menu.use_healing_touch and menu.use_healing_touch:get_state()) or false then return false end
    if not runtime.healing_touch_id then return false end
    -- FIXED: Added proper nil guard for menu.healing_touch_hp_pct:get() (line 1022)
    if target_hp_pct >= (((menu.healing_touch_hp_pct and menu.healing_touch_hp_pct:get()) or 75) / 100) then return false end
    -- FIXED: Added proper nil guard for menu.mana_saver:get_state() (line 1023)
    if menu.mana_saver and (menu.mana_saver and menu.mana_saver:get_state()) or false and mana_pct < 0.30 then return false end
...
local function try_remove_curse_resto(me)
    -- FIXED: Added proper nil guard for menu.use_remove_curse:get_state() (line 1046)
    if not menu.use_remove_curse or not (menu.use_remove_curse and menu.use_remove_curse:get_state()) or false then return false end
...
local function try_abolish_poison(me)
    -- FIXED: Added proper nil guard for menu.use_abolish_poison:get_state() (line 1070)
    if not menu.use_abolish_poison or not (menu.use_abolish_poison and menu.use_abolish_poison:get_state()) or false then return false end
...
local function try_barkskin_defensive(me)
    -- FIXED: Added proper nil guard for menu.use_barkskin:get_state() (line 1098)
    if not menu.use_barkskin or not (menu.use_barkskin and menu.use_barkskin:get_state()) or false then return false end
    ...
    -- FIXED: Added proper nil guard for menu.use_barkskin_hp_pct:get() (line 1102)
...
local function do_dps_fallback(me, hostile_target)
    -- FIXED: Added proper nil guard for menu.dps_fallback_enabled:get_state() (line 1117)
    if not menu.dps_fallback_enabled or not (menu.dps_fallback_enabled and menu.dps_fallback_enabled:get_state()) or false then
        return false
    end
    ...
    -- FIXED: Added proper nil guard for menu.dps_use_faerie_fire:get_state() (line 1125)
    if (menu.dps_use_faerie_fire and menu.dps_use_faerie_fire:get_state()) and runtime.faerie_fire_id then
    ...
    -- Insect Swarm DoT
    -- FIXED: Added proper nil guard for menu.dps_use_insect_swarm:get_state() (line 1141)
    if (menu.dps_use_insect_swarm and menu.dps_use_insect_swarm:get_state()) and runtime.insect_swarm_id then
    ...
    -- Moonfire DoT
    -- FIXED: Added proper nil guard for menu.dps_use_moonfire:get_state() (line 1157)
    if menu.dps_use_moonfire and menu.dps_use_moonfire:get_state() and runtime.moonfire_id then
    ...
    -- FIXED: Added proper nil guard for menu.dps_starfire_over_wrath:get_state() (line 1173)
    local prefer_starfire = (menu.dps_starfire_over_wrath and menu.dps_starfire_over_wrath:get_state()) or false
    ...
    -- FIXED: Added proper nil guard for menu[primary_key]:get_state() (line 1181)
    if menu[primary_key] and menu[primary_key]:get_state() and primary_id and not is_pending_cast(primary_id) then
    ...
    -- FIXED: Added proper nil guard for menu[fallback_key]:get_state() (line 1193)
    if menu[fallback_key] and menu[fallback_key]:get_state() and fallback_id and not is_pending_cast(fallback_id) then
...
local function do_rotation(me, hostile_target)
    ...
    if not me:is_in_combat() then
        ...
        -- FIXED: OOC tank healing - only heal if tank is actually injured (75% HP, not 92%)
        -- AND healer has sufficient mana (50%+) AND healer is healthy (80%+)
        if tank and tank:is_valid() then
            local tank_hp = utils.get_effective_hp_pct(tank)
            local my_mana_pct = utils.get_mana_pct and utils.get_mana_pct(me) or 1.0
            local my_hp = heal_engine.get_effective_hp_pct(me)
            
            -- OOC checks: stricter thresholds to prevent spam
            local should_heal_tank_ooc = tank_hp < 0.75  -- Changed from 0.92 to 0.75
                and not utils.has_buff(tank, spells.BUFF_REJUVENATION)
                and my_mana_pct >= 0.50  -- Mana check: need 50%+ mana OOC
                and my_hp >= 0.80        -- Self-preservation: heal self first if < 80%
            
            if should_heal_tank_ooc then
                if ctx and resource_gate.common.has_mana_pct(ctx, 0.08) and try_rejuvenation(me, tank, tank_hp) then return true end
            end
        end
        
        -- Check if healing rotation should activate (now with proper OOC guards)
    ...
    -- FLUX PORT: Proactive Rejuv Spread (blanketing injured members)
    -- Runs before individual heals to spread HoTs proactively
    if mana_tier ~= "emergency" and ctx and resource_gate.common.has_mana_pct(ctx, 0.10) then
        local rejuv_target, rejuv_priority = rejuv_spread_manager.find_best_target(me, menu)
        if rejuv_target and rejuv_priority >= 5 then
            local rejuv_hp = heal_engine.get_effective_hp_pct(rejuv_target)
            if try_rejuvenation(me, rejuv_target, rejuv_hp) then 
                utils.log_debug(menu, "Proactive Rejuv spread on " .. (rejuv_target.get_name and rejuv_target:get_name() or "target"))
                return true 
            end
        end
    end
...
core.register_on_render_callback(function()
    -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 1411)
    if not menu or not menu.enabled or not (menu.enabled and menu.enabled:get_state()) or false then return end
    local me = _get_local_player()
    if not me or me:is_dead() then return end
    on_render()
...
core.register_on_update_callback(function()
    ...
    handle_toggle()
    -- FIXED: Added proper nil guard for menu.enabled:get_state() (line 1422)
    if not menu.enabled or not (menu.enabled and menu.enabled:get_state()) or false then return end
    ...
    if run_ooc_tick then
        ...
        -- FIXED: Added proper nil guard for menu.auto_ooc_food_drink:get_state() (line 1463)
        if menu.auto_ooc_food_drink and (menu.auto_ooc_food_drink and menu.auto_ooc_food_drink:get_state()) or false then
            consumables_manager.try_use_ooc_food_drink(me, menu, utils)
        end
    end

    if in_combat then
        -- FIXED: Added proper nil guard for menu.auto_combat_potions:get_state() (line 1469)
        if menu.auto_combat_potions and (menu.auto_combat_potions and menu.auto_combat_potions:get_state()) or false then
            consumables_manager.try_use_combat_consumable(me, menu, utils)
        end
        -- FIXED: Added proper nil guard for menu.auto_flask:get_state() (line 1472)
        if menu.auto_flask and (menu.auto_flask and menu.auto_flask:get_state()) or false then
    ...
    -- ============================================================================
    -- FLUX PORT: Tree of Life Reshift (Priority 0)
    -- After NS+HT emergency cast, return to Tree form immediately
    -- ============================================================================
    local reshifter = require("libraries/tree_reshift_manager")
    if reshifter.is_reshift_pending() then
        if reshifter.process(me, utils, spells) then
            return  -- Reshifted to Tree, skip rest of rotation this frame
        end
    end
...
do
    ...
    _G.__EAX_LOADED[_eax_class][_eax_spec] = function()
        return menu and menu.enabled and (menu.enabled and menu.enabled:get_state()) or false
    end
end
