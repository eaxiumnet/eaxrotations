...
function utils.get_debuff_stack_count(unit, id_table)
    if not unit or not id_table or not unit:is_valid() then return 0 end
    local data = buff_manager:get_debuff_data(unit, id_table)
    if data and data.is_active then
        return data.stacks or data.count or 1
    end
    return 0
end

function utils.would_new_gcd_cross_swing_window(me, window_ms)
    if not me then return false end
    local gcd_ms = 1500
    local ok, swing_remaining = pcall(function() return me:get_swing_remaining() end)
    if not ok or not swing_remaining then swing_remaining = 0 end
    return (swing_remaining > 0 and swing_remaining < window_ms + gcd_ms)
end

function utils.is_next_swing_within_ms(me, ms)
    if not me then return false end
    local ok, swing_remaining = pcall(function() return me:get_swing_remaining() end)
    if not ok or not swing_remaining then swing_remaining = 0 end
    return swing_remaining > 0 and swing_remaining <= ms
end
