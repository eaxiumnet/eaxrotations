...
local function handle_toggle()
    -- FIXED: Proper nil guard
    local pressed = (menu.toggle_key and (menu.toggle_key and menu.toggle_key:get_state()) or false) or false
    if pressed and not runtime.last_toggle_state then
        -- FIXED: Proper nil guard
        local new_state = not ((menu.enabled and (menu.enabled and menu.enabled:get_state()) or false) or false)
...
local function twists_allowed_in_mode(mode)
    ...
    elseif mode == "dungeon" then
        -- FIXED: Proper nil guard
        return (menu.allow_twist_dungeon and (menu.allow_twist_dungeon and menu.allow_twist_dungeon:get_state()) or false) or false
    elseif mode == "raid" then
        -- FIXED: Proper nil guard
        return (menu.allow_twist_raid and (menu.allow_twist_raid and menu.allow_twist_raid:get_state()) or false) or false
    end
    return true
end
...
local function can_consider_seal_twist(me, target)
    -- FIXED: Proper nil guard
    if not (menu.use_seal_twist and menu.use_seal_twist:get_state()) then
        return nil, nil
    end
...
-- FIXED: Crusader Strike on CD 6s priority per README
local function should_hold_for_crusader_strike(me, target)
    -- FIXED: Proper nil guard
    if not (menu.use_crusader_strike and menu.use_crusader_strike:get_state()) then
        return false
    end
    ...
    -- FIXED: Proper nil guard
    return cooldown_ms <= (((menu.seal_twist_window and menu.seal_twist_window:get()) or 0) + SEAL_TWIST_INPUT_DELAY_MS)
end
...
local function should_hold_for_seal_twist(me, target)
    ...
    return utils.would_new_gcd_cross_swing_window(
        me,
        (menu.seal_twist_window and menu.seal_twist_window:get()) or 0,
...
local function should_start_seal_twist(me, target)
    ...
    -- FIXED: Proper nil guard
    if not utils.is_next_swing_within_ms(me, (menu.seal_twist_window and menu.seal_twist_window:get()) or 0, SEAL_TWIST_INPUT_DELAY_MS) then
        return false
    end
    -- FIXED: Proper nil guard
    local required_cooldown = ((menu.seal_twist_cooldown and menu.seal_twist_cooldown:get()) or 0) / 1000
...
local function should_reseal_baseline_now(me, target)
    ...
    -- FIXED: Proper nil guard
    return utils.is_next_swing_within_ms(me, (menu.seal_twist_window and menu.seal_twist_window:get()) or 0, SEAL_TWIST_INPUT_DELAY_MS)
end

local function selected_judgement_key(me)
    -- FIXED: Proper nil guard
    local choice = (menu.judgement_choice and menu.judgement_choice:get()) or 1
    if choice == 2 then
        return "crusader"
    end
    if choice == 3 then
...
local function can_consider_pre_pull_judgement(me, target)
    -- FIXED: Proper nil guard
    if not (menu.use_judgement and menu.use_judgement:get_state()) then
        return nil
    end
...
local function try_hand_of_freedom(me)
    ...
    -- FIXED: Proper nil guard
    local include_slows = (menu.hof_include_slows and (menu.hof_include_slows and menu.hof_include_slows:get_state()) or false) or false
...
local function try_divine_shield_emergency(me)
    ...
    -- FIXED: Proper nil guard
    local hp_threshold = ((menu.use_divine_shield_hp_pct and menu.use_divine_shield_hp_pct:get()) or 35) / 100
...
-- FIXED: Rotation priority per README - CS on CD 6s -> Judgement -> SoC/Blood maintain
