...
local function notify_cast(key, message, color_val, duration_s)
    _ = key, duration_s
    if menu.show_notifications and menu.show_notifications:get_state() then
...
local runtime = {
    ...
    last_stance_switch_at = 0,  -- Throttle stance switches
    -- Throttle timestamps for abilities (prevent spam when buff detection fails)
    last_hamstring_cast_at = 0,     -- 3.0s throttle
    last_battle_shout_cast_at = 0,   -- 10.0s throttle
    last_thunder_clap_cast_at = 0,   -- 5.0s throttle
}
...
local function get_mode_policy()
    ...
    if mode == "solo" then
        return {
            ...
            allow_hamstring = (menu.use_hamstring and menu.use_hamstring:get_state()) or false,
            allow_intercept = (menu.use_intercept and menu.use_intercept:get_state()) or false,
    ...
    if mode == "raid" then
        return {
            name = mode,
            auto_peel = (menu.auto_peel and menu.auto_peel:get_state()) or false,
            recovery_delta = 25,
    ...
    return {
        ...
        allow_hamstring = false,
        allow_intercept = false,
        allow_peel_intercept = (menu.use_peel_intercept and menu.use_peel_intercept:get_state()) or false,
        allow_risky_burst = false,
        allow_safe_burst = true,
    }
end
...
local function can_intercept_target(me, target)
    ...
    if distance < ((menu.intercept_min_range and menu.intercept_min_range:get()) or 15) then return false end
...
local function log_recovery_blocked(key, message)
    if menu.debug and (menu.debug and menu.debug:get_state()) or false and utils.throttle("simpleprot:debug:" .. key, 1.0) then
...
local function maybe_log_core_starvation(me, target, target_valid, rage, mode_policy)
    if not menu.debug or not (menu.debug and menu.debug:get_state()) or false then return end
...
local function log_lane_reason(key, message)
    if not menu.debug or not (menu.debug and menu.debug:get_state()) or false then
        return
    end
...
function runtime.update_charge_return_requests(me, target)
    ...
    if runtime.charge_queue_requested_at <= 0 and charge_just_fired and current_stance == "battle" then
        if menu.auto_defensive_after_charge and (menu.auto_defensive_after_charge and menu.auto_defensive_after_charge:get_state()) or false then
...
-- Remove the local function - will add to runtime table instead
...
local function execute_stance_swap(me, stance_name, stance_id, action_label)
    -- Verify stance is actually castable before attempting
    local ok, is_usable = pcall(function()
        return core.spell_book.is_usable_spell(stance_id)
    end)
    if not ok or not is_usable then return false end
    -- Throttle stance switches to prevent spam when buff detection fails
    local now = _core_time()
    if (now - runtime.last_stance_switch_at) < 3.0 then return false end
    if not utils.cast_self(stance_id, me) then return false end
    mark_pending_cast(stance_id, STANCE_PENDING_CAST_TIMEOUT_S)
    runtime.last_stance_switch_at = now
...
local STANCE_ACTION_VALIDATORS = {
    ...
    intercept = function(action, me)
        ...
        if utils.is_melee_target(me, target) or distance < ((menu.intercept_min_range and menu.intercept_min_range:get()) or 15) or distance > max_distance then
...
local function process_pending_stance_action(me)
    ...
    -- Leveling fallback: Sunder Armor as rage dump when Shield Slam/Devastate not learned
    if runtime.sunder_armor_id and rage_check >= 15 then
        if utils.can_cast_hostile(runtime.sunder_armor_id, me, target) then
            if utils.cast_target(runtime.sunder_armor_id, target) then
                utils.log_debug(menu, "ST: Sunder Armor (leveling filler)")
                note_cast()
                note_core_action()
                invalidate_ctx()
                return true
...
local function try_return_home_stance(me)
    ...
    -- Throttle stance switches
    local now = _core_time()
    if (now - runtime.last_stance_switch_at) < 3.0 then
        return false
    end
...
function runtime.try_return_after_charge(me)
    ...
    if not menu.auto_defensive_after_charge or not (menu.auto_defensive_after_charge and menu.auto_defensive_after_charge:get_state()) or false then
        runtime.charge_pending_return = false
        return false
    end
...
-- Ensure we're in Defensive Stance for Protection rotation (SS/Rev/Dev require it)
function runtime.try_ensure_defensive_stance(me)
    if utils.get_current_stance(me) == "defensive" then return false end
    local home_id = get_home_stance_id()
    if not home_id then return false end
    local now = _core_time()
    if (now - runtime.last_stance_switch_at) < 1.5 then return false end
    local ok, usable = pcall(function() return core.spell_book.is_usable_spell(home_id) end)
    if not ok or not usable then return false end
    if utils.can_cast_self(home_id, me) and utils.cast_self(home_id, me) then
        runtime.last_stance_switch_at = now
        utils.set_tracked_stance("defensive")
        utils.log_debug(menu, "Stance -> Defensive")
        note_cast()
        return true
    end
    return false
...
local function try_shout(me)
    local use_commanding = (menu.use_commanding_shout and menu.use_commanding_shout:get_state()) or false and runtime.commanding_shout_id
    local use_battle = (menu.use_battle_shout and menu.use_battle_shout:get_state()) or false and runtime.battle_shout_id
    ...
    -- CRITICAL: Check we have enough rage (Battle Shout costs 10 rage)
...
local function try_bloodrage(me, rage, action_target)
    ...
    if not me:is_in_combat() then
        if not (menu.use_prepull_bloodrage and menu.use_prepull_bloodrage:get_state()) or not is_valid_hostile_target(me, action_target) then return false end
...
local function try_thunder_clap(me, target, rage)
    if not (menu.use_thunder_clap and menu.use_thunder_clap:get_state()) or not runtime.thunder_clap_id then return false end
    if not target or not utils.is_melee_target(me, target) then return false end
...
local function do_emergency_defensive_lane(me, target, hp_pct)
    ...
    if menu.use_berserker_rage and menu.use_berserker_rage:get_state()
    ...
    if menu.use_spell_reflection and menu.use_spell_reflection:get_state()
        ...
        and get_active_cast_progress_pct(target) >= ((menu.spell_reflection_progress_pct and menu.spell_reflection_progress_pct:get()) or 100)
...
local function do_mitigation_lane(me, target, rage, mode_policy, hp_pct)
    if menu.use_shield_block and menu.use_shield_block:get_state()
...
local CONSUMABLE_DEFS = {
    {
        ...
        check_fn = function(me, rage)
            return rage < ((menu.rage_potion_rage_threshold and menu.rage_potion_rage_threshold:get()) or 30)
        end,
    ...
    },
    {
        ...
        check_fn = function(me)
            return utils.get_health_pct(me) < (((menu.healthstone_hp_pct and menu.healthstone_hp_pct:get()) or 35) / 100)
        end,
    ...
    },
    {
        ...
        check_fn = function(me)
            return utils.get_health_pct(me) < (((menu.health_potion_hp_pct and menu.health_potion_hp_pct:get()) or 35) / 100)
        end,
...
try_stoneform = function(me)
    ...
    if not me:is_in_combat() then return false end
    if utils.get_health_pct(me) >= (((menu.stoneform_hp_pct and menu.stoneform_hp_pct:get()) or 30) / 100) then return false end
...
try_ironshield_potion = function(me, target, mode_policy, hp_pct)
    if not menu.use_ironshield_potion or not menu.use_ironshield_potion:get_state() then return false end
    if not me:is_in_combat() then return false end
...
local function attempt_burst_action(me, action, rage, mode_policy)
...
local function do_burst_lane(me, target, rage, mode_policy)
    ...
    for i = 1, #BURST_ACTIONS do
        if attempt_burst_action(me, BURST_ACTIONS[i], rage, mode_policy) then return true end
    end
...
local function try_sunder_armor(me, target, target_hp_pct)
    ...
    if not target or not utils.is_melee_target(me, target) then return false end
    if menu.skip_sunder_with_expose and menu.skip_sunder_with_expose:get_state()
        and utils.get_debuff_remaining_ms(target, spells.DEBUFF_EXPOSE_ARMOR) > 0
    then
        return false
    end

    if menu.use_devastate and menu.use_devastate:get_state() and runtime.devastate_id then
        return false
    end
    ...
    if stack_count >= ((menu.sunder_max_stacks and menu.sunder_max_stacks:get()) or 5) then
        return false
    end
...
local function do_single_target_core_lane(me, target, ctx, rage, target_hp_pct)
    -- Safety: use direct rage if ctx is malformed
    local rage_check = rage
    if ctx and ctx.self and ctx.self.rage then
        rage_check = ctx.self.rage
    end
    ...
    if menu.use_shield_slam and menu.use_shield_slam:get_state()
    ...
    if runtime.devastate_id and rage_check >= 15 then
    ...
    if menu.use_devastate and menu.use_devastate:get_state()
    ...
    then
        ...
        if not expose_active and not (sunder_stacks >= ((menu.sunder_max_stacks and menu.sunder_max_stacks:get()) or 5) and sunder_remaining > 8000) then
            if utils.cast_target(runtime.devastate_id, target) then
                utils.log_debug(menu, "ST: Devastate")
                esp_renderer.on_cast(nil, "Devastate", color.yellow(220))
                note_cast()
    ...
    -- Emergency rage dump: Sunder Armor regardless of stance (for leveling/Berserker fallback)
    if runtime.sunder_armor_id and rage_check >= 15 then
        -- Skip can_cast_hostile check which fails in Berserker stance
        if utils.cast_target(runtime.sunder_armor_id, target) then
            utils.log_debug(menu, "ST: Sunder Armor (emergency dump)")
            note_cast()
            note_core_action()
            invalidate_ctx()
            return true
    ...
    if menu.use_execute and menu.use_execute:get_state()
        and target_hp_pct < EXECUTE_HP_THRESHOLD
        and runtime.execute_id
        and rage_check >= ((menu.execute_min_rage and menu.execute_min_rage:get()) or 15)
...
local function do_aoe_core_lane(me, target, ctx, rage)
    ...
    -- Leveling fallback: Sunder Armor as rage dump when Shield Slam/Devastate not learned
    if runtime.sunder_armor_id and rage_check >= 15 then
        if utils.can_cast_hostile(runtime.sunder_armor_id, me, primary_target) then
            if utils.cast_target(runtime.sunder_armor_id, primary_target) then
                utils.log_debug(menu, "AoE: Sunder Armor (leveling filler)")
                note_cast()
                note_core_action()
                invalidate_ctx()
                return true
...
local function try_shield_bash(me, target, context_label)
    if not (menu.use_shield_bash and menu.use_shield_bash:get_state()) or not runtime.shield_bash_id then return false end
    if not is_valid_hostile_target(me, target) then return false end
...
local function try_taunt(me, target, context_label)
    if not (menu.use_taunt and menu.use_taunt:get_state()) or not runtime.taunt_id then return false end
    if not is_valid_hostile_target(me, target) then return false end
...
local function try_concussion_blow(me, target, context_label)
    if not (menu.use_concussion_blow and menu.use_concussion_blow:get_state()) or not runtime.concussion_blow_id then return false end
    if not is_valid_hostile_target(me, target) then return false end
    if not utils.is_melee_target(me, target) then return false end
...
local function try_mocking_blow(me, target, context_label)
    if not (menu.use_mocking_blow and menu.use_mocking_blow:get_state()) or not runtime.mocking_blow_id then return false end
    if not is_valid_hostile_target(me, target) then return false end
    if not utils.is_melee_target(me, target) then return false end
...
local function try_mocking_blow_dance(me, target, rage, context_label)
    if not (menu.use_mocking_blow_dance and menu.use_mocking_blow_dance:get_state()) or not runtime.mocking_blow_id then return false end
    if not is_valid_hostile_target(me, target) then return false end
    if not me:is_in_combat() then return false end
...
local function try_challenging_shout(me, active_candidates, off_me_count)
    if not (menu.use_challenging_shout and menu.use_challenging_shout:get_state()) or not runtime.challenging_shout_id then return false end
...
local function try_intimidating_shout_keybind(me, target)
    ...
    if not menu.use_intimidating_shout or not menu.use_intimidating_shout:get_state() then return false end
...
try_rend = function(me, target, rage, target_hp_pct)
    if not (menu.use_rend and menu.use_rend:get_state()) or not runtime.rend_id then return false end
    if target_hp_pct < EXECUTE_HP_THRESHOLD then return false end
    if not is_valid_hostile_target(me, target) then return false end
    if not utils.is_melee_target(me, target) then return false end
...
try_hamstring_filler = function(me, target, rage, target_hp_pct)
    if not (menu.use_hamstring and (menu.use_hamstring and (menu.use_hamstring and menu.use_hamstring:get_state()) or false) or false) or not runtime.hamstring_id then return false end
    if target_hp_pct < EXECUTE_HP_THRESHOLD then return false end
    if not is_valid_hostile_target(me, target) then return false end
    if not utils.is_melee_target(me, target) then return false end
...
function try_charge(me, target, context_label)
    if not (menu.use_charge and menu.use_charge:get_state()) or not runtime.charge_id then return false end
...
function try_intervene(me, mode_policy)
    if not (menu.use_intervene and menu.use_intervene:get_state()) or not runtime.intervene_id then return false end
...
function maybe_cancel_queued_attack(target, rage)
    ...
    if not should_cancel and is_valid_hostile_target(_get_local_player(), target)
        ...
        and rage < (((menu.execute_min_rage and menu.execute_min_rage:get()) or 15) + HEROIC_STRIKE_COST)
    then
        should_cancel = true
    end
...
function try_cancel_external_blocking_buffs(me)
    ...
    local rage = utils.get_rage(me)
    if menu.cancel_pws and menu.cancel_pws:get_state() and rage < 25 and utils.has_buff(me, { 6788, 17139, 17140 }) then
        return cancel_matching_buff(me, { 6788, 17139, 17140 })
    end

    if menu.cancel_bop and menu.cancel_bop:get_state()
...
local function do_queue_lane(me, target, ctx, rage, is_aoe)
    ...
    -- Flux-style: only queue when swing window is open
    if not in_swing_window then
        return false
    end

    if is_aoe then
        if not (menu.use_cleave and menu.use_cleave:get_state()) or not runtime.cleave_id then return false end
        if rage < ((menu.cleave_rage and menu.cleave_rage:get()) or 40) or rage <= (reserve_rage + 10) then return false end

        if utils.can_cast_melee(runtime.cleave_id, me) and utils.cast_target_fast(runtime.cleave_id, target) then
            utils.log_debug(menu, "Queue: Cleave")
    ...
    -- Flux-style: allow queue if rage >= min (10) and meets threshold
    if rage < 10 then return false end
    if rage < ((menu.heroic_strike_rage and menu.heroic_strike_rage:get()) or 60) or rage <= (reserve_rage + 10) then return false end
...
local function on_spell_cast(data)
    ...
    if data.spell_id == runtime.charge_id then
        local auto_defensive = menu.auto_defensive_after_charge and (menu.auto_defensive_after_charge and menu.auto_defensive_after_charge:get_state()) or false or false
        runtime.charge_pending_return = auto_defensive
        runtime.suppress_home_return_after_charge = not auto_defensive
    end
...
local function on_legit_spell_cast(data)
    ...
    runtime.charge_queue_requested_at = _core_time()
    runtime.charge_pending_return = (menu.auto_defensive_after_charge and menu.auto_defensive_after_charge:get_state()) or false
    runtime.suppress_home_return_after_charge = not (menu.auto_defensive_after_charge and menu.auto_defensive_after_charge:get_state()) or false
end
...
local function on_update()
    ...
    if menu.debug and (menu.debug and menu.debug:get_state()) or false then
    ...
    if primary_target and primary_target:is_valid() and me:can_attack(primary_target) and interrupt_manager.should_interrupt(primary_target) then
        if menu.use_interrupt and menu.use_interrupt:get_state() and interrupt_manager.try_interrupt(me, primary_target, "warrior", utils) then
            return
        end
    end
    ...
    if not recovery.target
        ...
        and (menu.auto_peel and menu.auto_peel:get_state()) or false
        and is_aoe
    then
    ...
    if mode_policy.allow_intercept and (menu.use_intercept and menu.use_intercept:get_state()) or false then
    ...
    -- Try to ensure Defensive Stance before core abilities
    -- In Berserker/Battle stance, core abilities won't cast - attempt switch but don't block
    if utils.get_current_stance(me) ~= "defensive" then
        local home_id = get_home_stance_id()
        if home_id and utils.can_cast_self(home_id, me) then
            if utils.cast_self(home_id, me) then
                runtime.last_stance_switch_at = _core_time()
                utils.set_tracked_stance("defensive")
                utils.log_debug(menu, "Stance -> Defensive")
                note_cast()
                -- Stance changed, abilities may now be castable - continue to core lane
            end
        end
    end
    ...
    -- Final rage dump: Sunder Armor works in ANY stance (Berserker/Battle/Defensive)
    -- This is a fallback when all else fails to prevent rage capping
    if runtime.sunder_armor_id and rage >= 15 then
        if utils.cast_target(runtime.sunder_armor_id, action_target) then
            utils.log_debug(menu, "Rage Dump: Sunder Armor (any stance)")
            note_cast()
            invalidate_ctx()
            return
        end
...
function has_breakable_cc_nearby_flux(me, radius)
    ...
    -- Guard against missing object_manager API
    if not core.object_manager or not core.object_manager.get_objects_in_radius then
        return false
    end
    ...
    local ok, objects = pcall(function()
        return core.object_manager.get_objects_in_radius(me_pos, radius)
    end)
    ...
    for i = 1, #objects do
        ...
        if obj and obj:is_valid() and obj:is_unit() and not obj:is_dead() then
            for _, debuff_id in ipairs(cc_debuffs) do
                if debuff_id then
                    local ok2, has_debuff = pcall(function() return utils.has_debuff(obj, debuff_id) end)
                    if ok2 and has_debuff then return true end
                end
            end
        end
    end
