-- +--------------------------------------------------------------------------+
-- |  Eax's Warrior Protection - Menu v2.0 - menu.lua                         |
-- |                                                                          |
...
function menu.render()
    ...
    root_tree:render("Eax's Warrior Protection", function()
        ...
        rotation_tree:render("Rotation", function()
            ...
            menu.cleave_rage:render("Cleave Rage", "Min rage")
        ...
        cooldowns_tree:render("Cooldowns", function()
            ...
            menu.use_mocking_blow_dance:render("Mocking Blow Dance", "Stance dance")
            menu.taunt_trash:render("Taunt Trash", "Auto-taunt trash")

            ps.header("Threat Management")
            menu.use_threat_equalization:render("Threat Equalization", "Balance threat")
            menu.threat_eq_threshold:render("Threat Eq Threshold", "% threshold")
        end)
        ...
        defensive_tree:render("Defensive", function()
            ...
            menu.last_stand_hp_pct:render("Last Stand HP Pct", "Alternative")
            menu.use_shield_wall:render("Shield Wall", "Emergency")
            menu.shield_wall_hp:render("Shield Wall HP %", "Below")
            menu.shield_wall_hp_pct:render("Shield Wall HP Pct", "Alternative")
        ...
        utility_tree:render("Utility", function()
            ...
            menu.use_shield_bash:render("Shield Bash", "Interrupt")
            menu.use_concussion_blow:render("Concussion Blow", "Stun")
            menu.use_concussion_blow_proactive:render("Concussion Proactive", "Preemptive")
            menu.use_disarm:render("Disarm", "Disarm enemy")

            ps.header("Peel & Protection")
            menu.use_peel_intercept:render("Peel Intercept", "Intercept to peel")
            menu.use_swing_manager:render("Swing Manager", "Queue next attack")
            menu.swing_queue_threshold:render("Swing Queue %", "Threshold")
            menu.heroic_strike_rage:render("HS Rage", "Rage threshold")
            menu.heroic_strike_rage_cap:render("HS Rage Cap", "Max rage")
        end)

        -- 6. Buffs
        buffs_tree:render("Buffs", function()
            ps.header("Self Buffs")
            menu.use_battle_shout:render("Battle Shout", "Maintain buff")
            menu.use_commanding_shout:render("Commanding Shout", "Alternative buff")

            ps.header("Group Support")
            menu.ooc_rez:render("Auto-Resurrect", "Accept and cast resurrection")
            menu.ooc_group_buff:render("Group Buffs", "Apply buffs to party")
        end)

        -- 7. Middleware (Automation)
        middleware_tree:render("Middleware", function()
            ...
            menu.trinket1_mode:render("Trinket 1", {"Off", "Offensive", "Defensive", "Auto"})
            menu.trinket2_mode:render("Trinket 2", {"Off", "Offensive", "Defensive", "Auto"})
        end)

        -- 8. PvP
        pvp_tree:render("PvP", function()
            ps.header("General")
            menu.pvp_enabled:render("Enable PvP", "PvP-specific logic")
        ...
        -- 9. Automation (Burst/Trinket)
        automation_tree:render("Automation", function()
            ps.header("AoE Settings")
            menu.aoe_enemy_count:render("AoE Enemy Count", "Minimum enemies")

            ps.header("Challenging Shout Thresholds")
            menu.challenging_boss_threshold:render("Boss Threshold", "Min bosses")
            menu.challenging_elite_threshold:render("Elite Threshold", "Min elites")
            menu.challenging_trash_threshold:render("Trash Threshold", "Min trash")
        end)
        ...
        dashboard_tree:render("Dashboard", function()
            ...
            menu.show_energy_tick:render("Energy Tick", "Show energy tick")
            menu.show_combo_points:render("Combo Points", "Show combo points")
        end)
        ...
        -- 12. Advanced (Targeting and Racial)
