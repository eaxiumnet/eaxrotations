...
**Tests**: 326 rotation suites registered in `run_rotation_tests.lua` + 21 leveling suites in `run_leveling_tests.lua` (347 total)
...
├── tests/                     # ~347 test files (326 rotation + 21 leveling suites)
...
**Scope**: 24 files (14 spec + 9 leveling + 1 shadow), ~170 locations guarded. All 326 rotation + 21 leveling suites pass.
...
- One spec per commit. Gate each with `luac -p` + full 326+21 suite.
...
- Run `lua EaxRotations/tests/run_rotation_tests.lua` — all 326 rotation suites must pass
