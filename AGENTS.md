...
**Tests**: 350 rotation suites registered in `run_rotation_tests.lua` + 21 leveling suites in `run_leveling_tests.lua` (371 total)
...
├── tests/                     # ~369 test files (348 rotation + 21 leveling suites)
...
**Scope**: 24 files (14 spec + 9 leveling + 1 shadow), ~170 locations guarded. All 348 rotation + 21 leveling suites pass.
...
- One spec per commit. Gate each with `luac -p` + full 348+21 suite.
...
- Run `lua EaxRotations/tests/run_rotation_tests.lua` — all 350 rotation suites must pass
