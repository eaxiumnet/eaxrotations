...
**Tests**: 323 rotation suites registered in `run_rotation_tests.lua` + 21 leveling suites in `run_leveling_tests.lua` (344 total)
...
├── tests/                     # ~344 test files (323 rotation + 21 leveling suites)
...
**Scope**: 24 files (14 spec + 9 leveling + 1 shadow), ~170 locations guarded. All 323 rotation + 21 leveling suites pass.
...
- One spec per commit. Gate each with `luac -p` + full 323+21 suite.
...
- Run `lua EaxRotations/tests/run_rotation_tests.lua` — all 323 rotation suites must pass
