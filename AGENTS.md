...
**Tests**: 321 rotation suites registered in `run_rotation_tests.lua` + 21 leveling suites in `run_leveling_tests.lua` (342 total)
...
├── tests/                     # ~342 test files (321 rotation + 21 leveling suites)
...
**Scope**: 24 files (14 spec + 9 leveling + 1 shadow), ~170 locations guarded. All 321 rotation + 21 leveling suites pass.
...
- One spec per commit. Gate each with `luac -p` + full 321+21 suite.
...
- Run `lua EaxRotations/tests/run_rotation_tests.lua` — all 321 rotation suites must pass
