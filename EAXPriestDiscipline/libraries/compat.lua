...
context.hp = (ok_hp and ok_max and hp and max_hp and max_hp > 0) and ((hp / max_hp) * 100) or 100
    local ok_max_hp2, max_hp2 = pcall(function() if me.get_max_health then return me:get_max_health() end return 0 end)
    context.max_hp = ok_max_hp2 and max_hp2 or 0
    local ok_cur_hp, cur_hp = pcall(function() if me.get_health then return me:get_health() end return 0 end)
    context.current_hp = ok_cur_hp and cur_hp or 0
    
    -- Mana/Energy/Rage/etc
    if me.get_power_percentage then
        local ok_pct, pct = pcall(function() return me:get_power_percentage() end)
        context.mana_pct = ok_pct and pct or 0
...
context.target_hp = (ok_hp and ok_max and hp and max_hp and max_hp > 0) and ((hp / max_hp) * 100) or 100
        ...
        local ok_m, me_x, me_y, me_z = pcall(function() return me:get_position() end)
        local ok_t, tgt_x, tgt_y, tgt_z = pcall(function() return target:get_position() end)
        if ok_m and ok_t and me_x and tgt_x then
            local dx = tgt_x - me_x
            local dy = tgt_y - me_y
