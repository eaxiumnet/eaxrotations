...
function smart_defensive.predict_burst(me, window_seconds)
    ...
    if not ok_hp or not ok_max then return false, 0, 0 end
...
---Get current damage mitigation from armor, buffs, and talents
---@param me game_object
...
function smart_defensive.get_current_mitigation(me)
    ...
    -- Get armor-based physical mitigation (plate tanks have high armor)
    ...
    -- Improved Righteous Fury: 6% damage reduction (talent)
    -- Note: This is a buff that should be checked via aura
    if me.has_aura then
        local ok_rf, has_rf = pcall(function() return me:has_aura(25780) end)  -- Righteous Fury
        if ok_rf and has_rf then
            -- Assume talented (3/3 Improved RF = 6% reduction)
            mitigation = mitigation + 0.06
        end
    end
    
    -- Divine Protection / Divine Shield: 100% mitigation (immunity)
    if me.has_aura then
        local ok_shield, has_shield = pcall(function() 
            return me:has_aura(498) or me:has_aura(642)  -- Divine Protection or Divine Shield
        end)
        if ok_shield and has_shield then
            mitigation = mitigation + 1.0  -- Full immunity
        end
    end
    
    -- Holy Shield: Block value mitigation (estimated)
    if me.has_aura then
        local ok_holy_shield, has_holy_shield = pcall(function() return me:has_aura(20925) end)
        if ok_holy_shield and has_holy_shield then
            mitigation = mitigation + 0.15  -- Estimated block mitigation
        end
    end
    
    -- Cap at 75% physical DR (or 100% if immune)
    return math.min(mitigation, 1.0)
end
...
function smart_defensive.should_use(me, defensive_type, ctx, settings)
    ...
    local ok_hp, hp = pcall(function() return me:get_health() end)
local ok_max, max_hp = pcall(function() return me:get_max_health() end)
if not ok_hp or not ok_max or not hp or not max_hp or max_hp == 0 then return false, "health_error" end
local hp_pct = (hp / max_hp) * 100
