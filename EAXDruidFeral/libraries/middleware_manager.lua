-- EAXDruidFeral Middleware Manager
-- Integrates libraries/middleware with Druid-specific registrations
...
--- Initialize and register all Druid-specific middleware
...
function middleware_manager.initialize(menu)
    ...
    local innervate_mana_threshold = (menu.innervate_mana_pct and menu.innervate_mana_pct:get()) or 20
        ...
        local innervate_mw = {
            ...
            execute = function(icon, ctx)
                if core.spell_queue and core.spell_queue.add then
                    core.spell_queue.add(DRUID_SPELLS.INNERVATE, "innervate", 80)
    ...
    if use_war_stomp then
        local war_stomp_threshold = (menu.war_stomp_hp_pct and menu.war_stomp_hp_pct:get()) or 25
        local war_stomp_attackers = (menu.war_stomp_attackers and menu.war_stomp_attackers:get()) or 3
        ...
        local war_stomp_mw = {
            ...
            matches = function(ctx)
                ...
                -- Check attacker count (simplified - would need enemy count check)
                -- For now, just check health threshold
            ...
            execute = function(icon, ctx)
                if core.spell_queue and core.spell_queue.add then
                    core.spell_queue.add(DRUID_SPELLS.WAR_STOMP, "war_stomp", 85)
