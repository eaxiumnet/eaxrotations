...
theme.col = {
    ...
    border_glow  = c(240,140, 50, 220),
    border_dim   = c(100, 48,  8, 160),
    accent       = c(220,120, 30, 255),
    accent_mid   = c(190, 85, 15, 255),
    text_on      = c(255,160, 70, 255),
    text_off     = c(110, 60, 15, 255),
    transparent  = c(  0,   0,   0,   0),
}
function theme.init(class_id, spec_id) theme.class_id, theme.spec_id = class_id, spec_id end
function theme.col_panel() return theme.col.panel end
function theme.col_panel_deep() return theme.col.panel_deep end
function theme.col_border_glow() return theme.col.border_glow end
function theme.col_border_dim() return theme.col.border_dim end
function theme.col_accent() return theme.col.accent end
function theme.col_accent_mid() return theme.col.accent_mid end
function theme.col_text_on() return theme.col.text_on end
function theme.col_text_off() return theme.col.text_off end
function theme.col_transparent() return theme.col.transparent end
function theme.col_spec_primary() return theme.col.accent end
function theme.col_spec_secondary() return theme.col.accent_mid or theme.col.accent end
function theme.col_spec_tertiary() return theme.col.border_dim end
function theme.col_resource() return theme.col.accent end
function theme.spec_header(label) local h = core.menu.header(); h:render(label, theme.col_spec_primary()) end
...
function theme.apply(win, id)
    ...
    -- -- Panel base fill ------------------------------------------------------
    win:render_rect_filled(v(0, 0), v(W, H), theme.col.panel, 0)
    ...
    local BL  = 18
    local gcol = theme.col.border_glow
    -- top-left
    ...
    local function gem(cx, cy, sz2)
        win:render_triangle_filled(
            ...
            theme.col.border_dim)
        win:render_triangle_filled(
            v(cx, cy + sz2), v(cx + sz2, cy), v(cx - sz2, cy),
            theme.col.border_dim)
        win:render_circle_filled(v(cx, cy), sz2 * 0.35, theme.col.accent)
    end
    local G = 9
    gem(G + 2,     G + 2,     G)
    gem(W - G - 2, G + 2,     G)
...
function theme.header(label)
    ...
    h:render("  " .. label, theme.col.accent)
end
...
function theme.checkbox(id, default)
    return core.menu.checkbox(default, id)
end
function theme.slider_int(mn, mx, def, id)
    return core.menu.slider_int(mn, mx, def, id)
end
function theme.slider_float(mn, mx, def, id)
    return core.menu.slider_float(mn, mx, def, id)
end
function theme.keybind(key, toggle, id)
    return core.menu.keybind(key, toggle, id)
end
function theme.combobox(default, id)
    return core.menu.combobox(default, id)
end
function theme.tree_node()
