...
- `eax_shared/` — shared runtime used by most live EAX specs.
...
- **Shared-runtime pattern**: common decision support is centralized in `eax_shared/` instead of duplicated across specs.
...
2. On each update tick, the spec builds or reads cached combat context from `eax_shared/`.
...
|-----------|------------------------|--------------|
| `eax_shared/` | Central shared runtime for combat context, managers, UI telemetry, gating, and reusable combat helpers consumed by many specs. | `eax_shared/codemap.md` |
...
## Navigation Notes
- Start with this file, then jump into `eax_shared/codemap.md` for shared runtime behavior.
