-- EAX Hunter Marksmanship | libraries/menu.lua | Project Sylvanas
local unified = require("EAX_Unified/menu")
if not unified then error("[EAX Hunter MM] EAX_Unified/menu not found!") end

local ROTATION_KEY = "hunter_mm"
local menu = {}
    ...
    categories = {
        {
            name = "Rotation",
            settings = {
                { key = "use_aimed_shot",      type = "checkbox", label = "Aimed Shot",      default = true,  tooltip = "Primary big hit — high focus cost" },
                { key = "use_chimera_shot",    type = "checkbox", label = "Chimera Shot",    default = true,  tooltip = "Refreshes sting + burst" },
                { key = "use_serpent_sting",   type = "checkbox", label = "Serpent Sting",   default = true,  tooltip = "Maintain Serpent Sting (refreshed by Chimera)" },
                ...
                { key = "use_silencing_shot",  type = "checkbox", label = "Silencing Shot",  default = true,  tooltip = "Interrupt on cast" },
            },
        },
        {
            name = "Pet",
        ...
        },
        {
            ...
            settings = {
                ...
                { key = "use_trueshot_aura",type = "checkbox",label = "Trueshot Aura",  default = true,  tooltip = "Raid-wide AP buff — maintain" },
...
local callbacks = {
    on_enabled  = function() print("|cFFAAD966[EAX Marksmanship]|r Rotation ENABLED") end,
    on_disabled = function() print("|cFFAAD966[EAX Marksmanship]|r Rotation disabled") end,
}

unified.register_rotation("Hunter", "MM", MENU_DEF, callbacks)
...
menu.use_aimed_shot       = proxy("use_aimed_shot",     true)
menu.use_chimera_shot     = proxy("use_chimera_shot",   true)
...
menu.use_trueshot_aura    = proxy("use_trueshot_aura",  true)
...
local _enabled_keybind  = core.menu.keybind(7, false, "eax_hunter_mm_enabled_kb")
local _toggle_key_elem  = core.menu.keybind(107, false, "eax_hunter_mm_toggle_key")
...
function menu.on_menu_render()
    _menu_tree:render("EAX Hunter MM", function()
        _enabled_keybind:render("Enable EAX Hunter MM", "Toggle rotation on/off")
